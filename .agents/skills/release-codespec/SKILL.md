---
name: release-codespec
description: >-
  Use this skill when releasing CodeSpec: audit merged work and changeset
...
Run the CodeSpec release workflow as a resumable state machine. Inspect live GitHub state on every invocation and take only the next safe action. Do not assume an earlier invocation completed.

## Principles

- Treat `Fission-AI/CodeSpec` and `origin/main` as the release source of truth.
...
1. Verify the repository and tools:
   ...
   - Stop before release mutations if the canonical repository is not `Fission-AI/CodeSpec`.
...
3. Find the latest stable GitHub release. Exclude drafts and prereleases; do not use `git describe`, because a beta tag may be newer than the stable baseline.
   ...
   ```bash
   gh release list --repo Fission-AI/CodeSpec \
...
4. Find open release-related PRs:
   ...
   ```bash
   gh pr list --repo Fission-AI/CodeSpec --state open \
...
5. Use the exact package name `"@fission-ai/codespec"`, the highest required semantic bump, only relevant headings, and user-focused descriptions.
...
2. Verify all three artifacts independently:
   - `npm view @fission-ai/codespec@<version> version`
