# CodeSpec docs: the style guide

Structure, voice, tone, and language for CodeSpec's user docs. This file is the primary style authority for the docs tree. The tree's own README owns structure (the page map and which page teaches what); when this file and that README disagree, the README wins. `no-ai-slop` owns the generic slop patterns; this file owns what CodeSpec's docs specifically look and sound like.
...
- **Place first, meaning second**: name each on-disk item by its path or folder ("an `codespec/` folder at the repo root"), never by concept alone ("the planning folder"). The concept gloss can wait for the item's expansion. When a location varies (per tool, per OS), anchor it with a real folder or two ("`.agents/`, `.claude/`") and link the full list.
...
- Plain judgment is welcome. The docs may tell the reader what to do and what to skip: "The `codespec/` folder: pause first."
- Address the reader as "you". CodeSpec, the CLI, and init do things. "We" appears only for project decisions ("we say skills"), never as a tour guide.
...
State the observable consequence of a limit. If CodeSpec ignores a misspelled field, say that validation passes and the field has no effect. If a value falls back, name the value CodeSpec uses.

## Two surfaces

CodeSpec spans the terminal and the AI chat, and readers mix them up. Label every snippet:
...
In your AI chat:
  /codespec:propose add-rate-limit
```
...
- Don't generalize. Where you're tempted to write what CodeSpec "helps" with, write what actually happens: which file appears, what the diff shows, what the agent does next.
...
- Examples invoke workflows by skill: the ask that triggers it ("ask your agent to propose a change") or the skill's name (`codespec-propose`), which is the same in every tool. A command spelling (`/codespec:propose`) appears only as a labeled per-tool example, never as the generic instruction; commands are headed for deprecation and their spellings vary per tool.
