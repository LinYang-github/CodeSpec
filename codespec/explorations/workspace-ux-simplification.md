...
> CodeSpec should have one default path, one escalation path, and fewer explicit concepts shown to the user unless the system actually needs a decision from them.
...
2. Cross-root work feels like a workflow restart:
   ...
   - CodeSpec says this is multi-repo
...
- "CodeSpec figures out whether this stays local or needs to expand"
- "If it expands, it carries me forward instead of making me restart"
- "I only see advanced concepts when CodeSpec needs a real decision from me"
...
3. CodeSpec plans locally unless it has a strong reason not to
...
> This work spans multiple owned areas strongly enough that CodeSpec needs to upgrade it into a coordinated initiative.
...
- "CodeSpec is upgrading this into a coordinated initiative"
...
### Concepts CodeSpec should keep implicit until needed
...
### Concepts CodeSpec should only show when a real decision is needed
...
If CodeSpec realizes the work is no longer local, it should escalate in one motion.
...
If escalation is needed, CodeSpec should do as much as possible automatically:
...
CodeSpec can upgrade this into a coordinated initiative.

Suggested next step:
- create a coordination workspace at ~/work/codespec-workspaces/add-3ds
...
When CodeSpec has to ask questions, it should ask the smallest useful set.
...
Only ask if CodeSpec has strong evidence of a cross-boundary contract.
...
To keep the UX intuitive, CodeSpec should aggressively choose defaults.
...
CodeSpec should infer affected areas from:
...
If an existing shared contract owner already exists, CodeSpec should suggest it instead of asking an abstract ownership question.
...
CodeSpec can start with a local coordination workspace and later offer:
...
2. CodeSpec upgrades to a coordinated initiative if needed
3. When the work becomes collaborative, CodeSpec offers to make it team-shared
...
CodeSpec should avoid asking users to classify work in abstract ways if it can infer a reasonable default.
...
By default, CodeSpec should feel like:
...
- progressive disclosure: only show advanced concepts when CodeSpec needs a real decision

If CodeSpec does this well, the same system can feel intuitive for:
