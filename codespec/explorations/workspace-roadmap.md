...
This document proposes a lightweight roadmap for workspace, monorepo, and multi-repo support in CodeSpec.
...
CodeSpec needs a better way to organize:
...
References help agents and humans navigate related specs without requiring CodeSpec to build a dependency graph system on day one.
...
1. Nested spec paths within one `codespec/` root
...
If CodeSpec uses local path caches or machine-specific mappings, they should be:
