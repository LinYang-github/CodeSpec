...
```
codespec profile set core|extended
codespec profile install <workflow>
codespec profile uninstall <workflow>
codespec profile list
codespec profile show
codespec config set delivery skills|commands|both
...
**After (simplified):**
```
codespec config profile          # interactive picker (delivery + workflows)
codespec config profile core     # preset shortcut
codespec config profile extended # preset shortcut
```
...
- **Global** = user's defaults (`~/.config/codespec/`)
- **Project** = repo-level config (`.codespec/` or committed to repo)
...
Keep it simple:
1. Global profile as default for `codespec init`
2. `codespec init` applies current profile to project
...
### Current CodeSpec Architecture

CodeSpec currently assumes:
- One `codespec/` per repo, always at root
...
## Part 4: Three Models for CodeSpec
...
├── services/
│   └── billing/
│       └── codespec/specs/billing/spec.md
├── clients/
│   ├── web/
│   │   └── codespec/specs/checkout/spec.md
│   ├── ios/
│   │   └── codespec/specs/checkout/spec.md
│   └── android/
│       └── codespec/specs/checkout/spec.md
└── codespec/           # Root-level for cross-cutting
    ├── specs/
...
- Works for multi-repo too (each repo has its own `codespec/`)

**Cons:**
- Cross-cutting specs are awkward (where do they go?)
- Changes that span multiple `codespec/` directories = ???
- Need a "workspace" concept to aggregate
- Multiple `codespec/` roots to manage

### Model D: Hybrid (Model B Inside Each Project + Model C Across Projects)

Use one `codespec/` root per project, but allow nested specs within that root for clear ownership and shared contracts.
...
~/work/
├── billing-service/
│   └── codespec/specs/billing/
├── web-client/
│   └── codespec/specs/checkout/
├── ios-client/
│   └── codespec/specs/checkout/
...
2. **Where do cross-repo changes live?**
   ...
   - In `~/.config/codespec/workspaces/my-platform/changes/`?
...
> **A workspace is a collection of CodeSpec roots that can be operated on together.**

```yaml
# ~/.config/codespec/workspaces.yaml (or similar)
workspaces:
  my-platform:
    roots:
...
- `codespec/specs/checkout/spec.md` is the shared kernel
- Platform specs nest under it

**Option B: Dedicated area**
- `codespec/specs/_shared/checkout-contract/spec.md`
- Or `codespec/specs/_contracts/checkout/spec.md`
...
| Concept | Definition |
|---------|------------|
| **Project** | Single CodeSpec root (one `codespec/` directory) |
...
### 4. Does CodeSpec need to understand dependencies?

If `checkout-web` depends on `checkout-contract`:
- Should CodeSpec know this relationship?
...
**Ambitious:**
> CodeSpec automatically understands your repo structure, detects cross-cutting specs, and helps you create changes that flow to the right places.

**Simpler:**
> You organize specs however you want. CodeSpec just works.
...
### What is a spec in CodeSpec?

For CodeSpec, a spec should be treated as a **verifiable behavior contract at a boundary**:
...
CodeSpec is often agent-authored from human exploration. To make that reliable:
...
2. `codespec/specs/codespec-conventions/spec.md` for normative spec conventions
3. `codespec/specs/docs-agent-instructions/spec.md` for agent-instruction authoring rules

---

## Part 10: Design Decisions (April 2026)

After evaluating the models above against real multi-repo use cases (see [#725](https://github.com/Fission-AI/CodeSpec/issues/725)), we converged on the following design direction.
...
- **Each repo keeps its own canonical `codespec/`** — no change to the fundamental storage model.
...
For larger multi-team work, repo-centric planning is the wrong primary abstraction. Teams and repos are many-to-many facets over the same work. CodeSpec should treat the **initiative / plan** as the first-class planning object, then link repo-local changes to it.
...
```yaml
# billing-service/codespec/changes/add-3ds/.codespec.yaml
schema: spec-driven
created: 2026-04-12
initiative: add-3ds
links:
...
2. **Local CodeSpec repo registry** — a persistent mapping in `~/.config/codespec/` or `~/.local/share/codespec/` (see `src/core/global-config.ts`).
...
```yaml
# web-client/codespec/specs/checkout/spec.md frontmatter
references:
...
This avoids accidentally committing CodeSpec to a full dependency graph system before the use cases justify it.
...
|---------|----------|------------|
| **Spec organization** | Nested specs inside one `codespec/` (Model B) | Each repo has its own `codespec/` |
...
|----------|--------|-------|
| Profile UX | Decided | `codespec config profile` with presets |
