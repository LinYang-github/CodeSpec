...
This exploration is also affected by the emerging workspace direction: for larger cross-team or cross-repo work, CodeSpec may need to treat the **initiative** as the first-class planning object and repo-local changes as execution artifacts. That means explore may sometimes be seeding an initiative, not just a single change.
...
2. **Where should exploration files live?**
   - `codespec/explorations/<name>.md`?
   - `codespec/changes/<change>/explorations/`?
   - `.codespec-workspace/initiatives/<initiative>/explorations/` for coordinated work?
   - Somewhere else?
...
- Saves to `codespec/explorations/<name>.md`
...
- `codespec/changes/simplify-skill-installation/` - current change updating core workflows
