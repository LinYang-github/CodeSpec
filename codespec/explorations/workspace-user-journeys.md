...
This document describes the exact user experience for CodeSpec across:
...
- How CodeSpec decides which specs to read
- How CodeSpec behaves when roots are missing, ambiguous, or partially available
...
CodeSpec should operate on **scopes**.
...
- CodeSpec reads only the relevant specs
- CodeSpec stores each artifact in the owning place
- If the work spans multiple owning places, CodeSpec creates a neutral coordination workspace
...
CodeSpec should model them explicitly as:
...
If there is no owner yet, the shared behavior can live temporarily as an initiative note during exploration, but CodeSpec should not promote it into a canonical spec until ownership is explicit.
...
Users should still recognize the existing CodeSpec workflow:
...
The new behavior is that CodeSpec may ask:
...
Instead, CodeSpec should create a neutral coordination workspace for the initiative.
...
CodeSpec may surface them to users and agents, but should not require them to resolve or validate across roots in the first version.

### 6. Shared contracts need explicit ownership

When the user describes behavior that spans multiple boundaries, CodeSpec should help them decide whether they are creating:

- a local spec
- a shared contract
- an initiative note that is not yet canonical

If the work is a real long-lived contract, CodeSpec should prompt for a canonical owner rather than burying it in whichever repo the user happened to be standing in.

### 7. Do not force "shared ownership" as implicit co-ownership

CodeSpec should help teams create **shared contract ownership**, not ambiguous co-ownership.
...
When a user starts in `web-client` and describes a checkout behavior spanning web, iOS, Android, and backend, CodeSpec should treat the current cwd as a clue about likely consumers, not as proof that `web-client` should own the shared contract.
...
A single CodeSpec root where canonical specs and changes can live.

In practice a project may be:

- a repo root with `codespec/`
- a monorepo root with one `codespec/`
- a nested owned scope if CodeSpec later supports nested roots
...
```text
~/work/codespec-workspaces/add-3ds/
  .codespec-workspace/
    workspace.yaml
    initiative.md
    links.yaml
...
```text
monorepo/
  apps/web/codespec.scope.yaml
  services/billing/codespec.scope.yaml
  packages/contracts/codespec.scope.yaml
```
...
CodeSpec needs to answer two separate questions:
...
CodeSpec discovers candidate scopes from:
...
CodeSpec resolves these to local paths using this order:

1. explicit path passed for this run
2. saved local CodeSpec registry
...
For team use, CodeSpec should separate:
...
CodeSpec should communicate this clearly:
...
CodeSpec should help teams choose the owner that can most credibly do all of the following:
...
When CodeSpec detects a likely shared contract, it should ask or infer from these questions:
...
CodeSpec should default to that existing owner.
...
CodeSpec should suggest that existing domain owner if there is no dedicated shared area.
...
CodeSpec should help create a new shared contract scope.
...
CodeSpec should allow the behavior to remain in initiative notes temporarily and explicitly mark it as non-canonical until ownership is chosen.

### What CodeSpec should suggest by default
...
When the user describes a likely cross-boundary behavior, CodeSpec should treat this as a first-class planning moment.
...
During `/opsx:propose` or `/opsx:explore`, CodeSpec should ask something like:
...
If the user chooses "create a new shared contract scope," CodeSpec should guide them.
...
- codespec/specs/contracts/checkout
- codespec/specs/shared/checkout
...
CodeSpec should avoid auto-creating a brand-new repo. It can scaffold the plan and record the decision, but repo creation may be organizationally sensitive and usually belongs outside the CLI.

### Temporary initiative-only mode

If ownership is unclear, CodeSpec should support:
...
4. Let CodeSpec determine whether this is:
   - local only
   - multi-scope in one root
   - cross-root coordination
5. CodeSpec creates artifacts in the right places
6. CodeSpec tells the user where to continue planning and where to implement
...
CodeSpec should support three agent capability levels.
...
- CodeSpec tells the user exactly which roots to attach
...
- User opens the coordination workspace
- CodeSpec writes agent-readable absolute paths into the workspace instructions
...
- CodeSpec guides the user into repo-local implementation sessions afterward
...
## Current CodeSpec Flow To Preserve

Today the flow is:

1. User enters a repo
2. User runs `codespec init`
3. User opens an agent in that repo
4. User runs `/opsx:explore` or `/opsx:propose`
5. CodeSpec stores changes in `codespec/changes/<change>/`
...
- if CodeSpec detects a cross-root initiative, it explicitly upgrades the workflow into a coordination workspace flow

---

## Journey 1: Single Repo, Standard CodeSpec Project
...
```text
Created codespec/changes/add-dark-mode/
Using scope: repo
...
CodeSpec archives the change in this repo, as today.
...
- User runs from subdirectory: CodeSpec should either walk up or clearly tell them to run from repo root
...
CodeSpec detects this is still one project root, but there are multiple candidate scopes.

If the user’s request clearly mentions one area, CodeSpec may infer:
...
1. create a single monorepo-local change under `codespec/changes/add-invoice-filtering/`
...
CodeSpec detects likely affected scopes:
...
```text
platform/codespec/changes/add-3ds-checkout/
```
...
CodeSpec ignores unrelated specs by default.
...
This means CodeSpec cannot assume:
...
```text
apps/web/codespec.scope.yaml
apps/mobile/codespec.scope.yaml
services/billing/codespec.scope.yaml
packages/contracts/codespec.scope.yaml
```
...
CodeSpec detects this is limited to `apps/web`.
...
At this point CodeSpec must make a product decision.
...
If the user chooses the coordination flow, CodeSpec creates:
...
They already use CodeSpec there.
...
At this point CodeSpec should not silently create one cross-repo change inside `web-client`.
...
### Expected CodeSpec behavior

CodeSpec interrupts the default single-repo flow and says:
...
For cross-repo work, CodeSpec recommends creating a coordination workspace.
...
If the user agrees, CodeSpec creates the workspace.
...
```bash
codespec workspace create add-3ds --at ~/work/codespec-workspaces/add-3ds
```
...
CodeSpec now resolves local paths for:
...
CodeSpec then tells the user:

```text
Next step:
1. Open your coding agent in ~/work/codespec-workspaces/add-3ds
...
CodeSpec has generated workspace instructions at:
.codespec-workspace/agents/claude.md
```
...
Or CodeSpec may have already scaffolded the initiative and tell the agent to continue it.
...
- `contracts/codespec/changes/add-3ds-contract/`
- `billing-service/codespec/changes/add-3ds-billing/`
- `web-client/codespec/changes/add-3ds-web/`
- `ios-client/codespec/changes/add-3ds-ios/`
...
```bash
codespec initiative new add-3ds
```
...
CodeSpec resolves local clones and writes the workspace files.

### Agent setup

The user opens the coordination workspace in their agent.

CodeSpec-generated agent instructions contain:
...
CodeSpec should support that directly.
...
Each teammate keeps local path mappings outside the shared repo, for example in CodeSpec local config/data:
...
2. runs something like `codespec workspace doctor` or `codespec workspace sync`
...
CodeSpec generates agent instructions using:
...
CodeSpec should support the initiative being created in a shared coordination repo owned by the sponsoring or driving team.
...
When another team member opens the shared coordination workspace, CodeSpec should make this explicit:
...
CodeSpec should still allow planning.
...
CodeSpec should never blindly read all specs from all roots.
...
```text
Related references:
- github.com/Fission-AI/contracts: codespec/specs/checkout/spec.md
- github.com/Fission-AI/web-client: codespec/specs/checkout/web/spec.md
```
...
When a selected change involves a shared contract, CodeSpec should prefer this read order:
...
If a cross-boundary behavior has not yet been assigned a canonical owner, CodeSpec should store it as initiative-level draft material rather than pretending it is already a canonical spec.
...
CodeSpec detects this likely requires a shared contract.
...
If the user selects an existing contracts repo, CodeSpec:
...
If the user keeps it initiative-only, CodeSpec:
...
CodeSpec checks linked repo changes:
...
4. run `codespec update`
5. open agent in monorepo root

During planning, CodeSpec asks for scope selection when needed.
...
1. each repo independently runs `codespec init`
...
When CodeSpec creates a coordination workspace, it should generate agent-facing files.
...
- CodeSpec recommends creating a coordination workspace
...
- `codespec workspace doctor` or equivalent relinks roots
...
If CodeSpec adopts these journeys, several design conclusions follow.

### 1. Multi-repo planning needs a first-class coordination workspace

Without this, CodeSpec has no honest answer to where the user should stand or where the cross-repo initiative should live.

### 2. Repo-local changes and canonical specs should stay in owners

Without this, CodeSpec creates duplicate or misleading sources of truth.

### 3. Large monorepos cannot be treated as always-simple single roots

CodeSpec must support both:
...
CodeSpec must separately handle:
...
Cross-root planning only works reliably if CodeSpec writes deterministic workspace context for the agent.

### 6. References should stay informational in v1

If CodeSpec validates cross-root references by default, it has effectively shipped a dependency graph system.
...
CodeSpec should help users choose among:
...
To support teams and orgs cleanly, CodeSpec should distinguish:
...
1. Should a coordination workspace always be user-visible on disk, or may it optionally live in global CodeSpec data directories?
...
3. Should CodeSpec support nested `codespec/` roots, or keep one root and model scopes separately?
