...
### Requirement: Migrating CodeSpec content out of a renamed tool's former directory

When a tool's directory is renamed, CodeSpec-managed content left in the former
...
- **WHEN** `codespec init` or `codespec update` runs and CodeSpec-managed content is found under a former root marked as needing no consent, such as `.kimi`
...
- **GIVEN** CodeSpec skills or command files under `.windsurf/`
- **WHEN** `codespec update` runs interactively without `--force`
...
- **WHEN** `codespec update` runs with `--force`, or non-interactively
- **THEN** perform the move without prompting, reporting what moved

#### Scenario: Selecting a renamed tool is consent

- **WHEN** `codespec init` configures a tool that has CodeSpec content under a former root
- **THEN** move that content as part of setup, rather than leaving the user with two installs of one tool

#### Scenario: Both directories already hold CodeSpec content

- **GIVEN** the same CodeSpec-managed skill or command exists under both the former and the current root
- **WHEN** the move runs
- **THEN** the copy under the current root SHALL win, rather than being merged or overwritten
- **AND** only the file CodeSpec generated SHALL be removed from the former root — for a skill directory that is `SKILL.md` alone, never the directory and whatever else it holds
...
- **GIVEN** every CodeSpec-managed file under the former root differs from its counterpart under the current one
- **WHEN** the move runs
...
- **WHEN** the move runs
- **THEN** move only the files CodeSpec generates — each skill's `SKILL.md` and command files named `opsx-*`
- **AND** delete the former directory only when the move leaves it empty

#### Scenario: A user file beside a generated skill is not carried into a directory CodeSpec prunes

- **GIVEN** a former skill directory holds `SKILL.md` alongside a file the user wrote
- **AND** CodeSpec removes whole skill directories it owns, as under commands-only delivery or for a workflow outside the active profile
- **WHEN** the move runs
...
- **WHEN** `codespec update` runs again after a completed move
...
- **AND** CodeSpec-managed skills remaining under the legacy `.kimi/skills` directory SHALL be migrated to `.kimi-code/skills` during init and update, preserving user files
...
- **AND** `codespec init` and `codespec update` SHALL display the note whenever `hermes` is configured
