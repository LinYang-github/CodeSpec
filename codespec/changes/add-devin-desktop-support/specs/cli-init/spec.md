...
- **THEN** rewrite `/opsx:<id>` references in the skill body to the matching `/codespec-<skill>` invocation, which both Devin agents accept
- **AND** the getting-started hint SHALL name `/codespec-propose` rather than a workflow
