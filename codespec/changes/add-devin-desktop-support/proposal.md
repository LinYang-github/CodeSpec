...
- The rebrand moved the config directory: `.devin/` is now the preferred read + write location and `.windsurf/` the legacy read-only fallback, for `rules/`, `workflows/`, `skills/`, and `plans/`. CodeSpec writes only `.windsurf/`, so every Devin install lands in the deprecated path.
- Devin ships two agents. Devin Desktop (Cascade) reads workflows; the [Devin Local agent does not](https://docs.devin.ai/desktop/devin-local) — its docs say to migrate workflows to skills, and it does not read `.windsurf/` at all. An existing Windsurf user's CodeSpec files are therefore invisible to Devin Local entirely.
- Adding `devin` as a *second* tool id alongside `windsurf` would list one product twice in the picker and leave existing users with two parallel installs. This follows the rename instead, matching what CodeSpec already did for Kimi CLI → Kimi Code.
...
- **Migrate existing installs, with consent.** CodeSpec-managed skills (`codespec-*`) and command files (`opsx-*`) under `.windsurf/` move to `.devin/`. `codespec update` explains the rebrand and asks first; `--force` and non-interactive runs take the move. Selecting the tool during `codespec init` is itself consent. Files the user wrote are never touched.
- Route Devin's **skill** bodies and the getting-started hint through the skill-reference transformer so they say `/codespec-*`, the one invocation both Devin agents accept.
...
- The `.devin/` directory also covers `rules/` and `plans/`. CodeSpec writes neither, so they are out of scope and untouched.
