...
- [x] 2.3 Re-key the pre-opsx `.windsurf/workflows/codespec-*.md` entry in `LEGACY_SLASH_COMMAND_PATHS` to `devin` — that map's keys are tool ids.
- [x] 2.4 In `getTransformerForTool`, give `devin` the skill-reference transformer whenever skills are generated, so skill bodies and the getting-started hint say `/codespec-*` — the Devin Local agent has no workflows. Under commands-only delivery, fall through to the invocation rewrite.
...
- [x] 3.4 `codespec update`: explain the rebrand, prompt interactively, migrate under `--force` or non-interactively, and say plainly what declining costs.
- [x] 3.5 `codespec init`: treat selecting the tool as consent and migrate for the selected tools only.
...
- [x] 5.5 `init`/`update`: both surfaces — `.devin/workflows/opsx-*.md` carry `/opsx-*`, `.devin/skills/codespec-*/SKILL.md` carry `/codespec-*`, and neither carries `/opsx:`.
...
- [x] 6.1 `codespec validate add-devin-desktop-support --strict`.
- [x] 6.2 `codespec archive add-devin-desktop-support --yes` merges cleanly and additively (run on a scratch copy, then reverted).
