...
- [x] 2.2 Remove (or fold) `codespec/changes/add-artifact-regeneration-support/` so the tree has a single update proposal.
- [x] 2.3 Update any generated-skill manifests/fixtures that enumerate workflow skills so `codespec-update-change` is included.

## 3. Tests

- [x] 3.1 Template generation snapshot for the skill and command templates.
- [x] 3.2 Assert the template's control flow contains NO hardcoded artifact-name branching (the anti-#777 guard): artifact ids must be read from `codespec status` JSON.
...
- [x] 4.1 `codespec validate add-update-workflow --strict` passes; `codespec status --change add-update-workflow` shows all artifacts complete.
