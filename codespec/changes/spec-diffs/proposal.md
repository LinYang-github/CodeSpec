...
When proposing a change, delta spec files under `codespec/changes/<name>/specs/` duplicate large portions of existing specs in `codespec/specs/`. A MODIFIED requirement must include the entire requirement block (all scenarios), making it hard to see what actually changed versus what was copied verbatim. This friction slows review and increases the risk of errors.

## What Changes

Add a `--diff` flag to `codespec show` (change type) that renders each delta spec as a unified diff against the corresponding main spec in `codespec/specs/`. This is the smallest viable improvement: it doesn't change the storage format or workflow, just adds a new way to view the deltas.
...
3. **Add `codespec show --diff` to render deltas against main specs.** (Chosen.) Leave the storage format unchanged. When displaying a change, compute the diff on the fly by comparing each delta spec file against its matching main spec. This gives reviewers the view they need with minimal code changes and zero workflow disruption.

### What this change delivers

- A `--diff` flag on `codespec show <change>` (and `codespec change show <change>`) that outputs a human-readable unified diff per delta spec
