...
`codespec show <change>` currently displays the raw proposal markdown (text mode) or a parsed JSON with deltas extracted from the proposal's Capabilities section. Delta spec files under `codespec/changes/<name>/specs/<cap>/spec.md` contain full requirement text including unchanged content copied from the base spec at `codespec/specs/<cap>/spec.md`.
...
**Goals:**
- Let users see per-requirement diffs of delta specs via `codespec show <change> --diff`
...
- **MODIFIED** — match by requirement name (`### Requirement: <name>`) against the base spec at `codespec/specs/<cap>/spec.md`, extract both blocks, and compute a unified diff of those blocks
...
The `diff` package provides `structuredPatch()`, which generates structured unified-diff hunks from two strings. CodeSpec renders those hunks without synthetic file headers.
...
**Alternative:** A separate `codespec diff` command. Rejected because the diff is about *viewing* a change, which is what `show` does. Adding a flag is more discoverable and consistent.
...
Add `--diff` to:
- `codespec show` (top-level, passed through as a change-only flag)
- `codespec change show` (direct)
