...
- **WHEN** executing `codespec show <item> --json`
...
- **WHEN** executing `codespec show <change-name>` in text mode without `--diff`
- **THEN** print the proposal markdown and nothing else, exactly as before `--diff` existed

#### Scenario: Diff output in text mode

- **WHEN** executing `codespec show <change-name> --diff` in text mode (no `--json`)
- **THEN** display the proposal markdown text
- **AND** for each delta spec file under `codespec/changes/<change-name>/specs/<cap>/spec.md`, display the parsed deltas grouped by capability
...
- **AND** for MODIFIED requirements, extract the matching requirement block from the main spec at `codespec/specs/<cap>/spec.md` by `### Requirement:` header name, compute a unified diff of the main block vs the delta block, and display it colorized (green for `+` lines, red for `-` lines, plain for context lines)
...
- **WHEN** executing `codespec show <change-name> --json --diff`
...
- **WHEN** executing `codespec show <change-name> --diff` and the change has no delta spec files
- **THEN** print a message reporting that the change has no delta specs to diff
- **AND** exit with code 0

#### Scenario: Diff flag on non-change item

- **WHEN** executing `codespec show <spec-name> --diff`
...
- **THEN** the system SHALL build `codespec/specs/<cap>/spec.md` under the same CodeSpec root the change was read from, so `--store <id>` diffs against that store's main specs rather than the working directory
