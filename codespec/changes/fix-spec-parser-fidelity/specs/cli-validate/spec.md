...
- **WHEN** running `codespec validate <id> --strict` for both a change delta and a main spec
...
- **WHEN** running `codespec validate <spec-id> --strict`
- **THEN** validation SHALL skip the metadata lines, detect `MUST`, and pass — matching `codespec validate <change-id>`
...
- **WHEN** running `codespec validate <change-id> --strict`
...
- **WHEN** running `codespec validate <id> --strict`
...
- **WHEN** running `codespec validate <change-id> --strict`
- **THEN** validation SHALL report the requirement as missing a scenario — the same result as `codespec validate <spec-id>`
...
- **WHEN** running `codespec validate` on each
