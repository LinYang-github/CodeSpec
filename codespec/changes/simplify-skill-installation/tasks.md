...
- [x] 3.6 Display post-update message: "Config updated. Run `codespec update` in your projects to apply."
- [x] 3.7 Detect if running inside an CodeSpec project and offer to run update automatically
...
- [x] 3.10 Update `codespec config list` to display profile, delivery, and workflows settings (indicate defaults vs explicit)
...
- [x] 5.3 Ensure propose creates `.codespec.yaml` via `codespec new change` before generating artifacts
...
- [x] 5.7 Add `codespec-propose` to `SKILL_NAMES` in `src/core/shared/tool-detection.ts`
...
- [x] 8.4 Display migration message: "Migrated: custom profile with N workflows" + "New in this version: /opsx:propose. Try 'codespec config profile core' for the streamlined experience."
- [x] 8.5 Add project check: exit with error if no `codespec/` directory exists
...
- [x] 10.1 Verify `codespec new change` creates `.codespec.yaml` with schema and created fields

<!-- Note: 10.2 and 10.3 below are potential follow-up work, not core to this change -->
<!-- - [ ] 10.2 Update ff skill to verify `.codespec.yaml` exists after `codespec new change` -->
<!-- - [ ] 10.3 Add guardrail to skills: "Never manually create files in codespec/changes/ - use codespec new change" -->
...
- [x] 11.3 Review explore → propose transition UX (see `codespec/explorations/explore-workflow-ux.md` for open questions)
...
- [x] 12.6 Manual: `codespec config profile` picker — verify delivery toggle, workflow toggles, pre-selection of current values, core preset shortcut
