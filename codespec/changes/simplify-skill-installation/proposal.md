...
Power users can customize via `codespec config profile`.
...
Stored in existing global config (`~/.config/codespec/config.json`). Not prompted during init.
...
codespec config profile core     # preset shortcut (core workflows, preserves delivery)
```
...
**Existing users keep their current setup.** When `codespec update` runs on a project with existing workflows and no `profile` in global config, it performs a one-time migration:
...
- `codespec init` on a **new** project (no existing workflows) uses global config, defaulting to `core`
...
- Migration message mentions `propose` and suggests `codespec config profile core` to opt in
- After migration, users can opt into `core` profile via `codespec config profile core`
...
```json
// ~/.config/codespec/config.json (extends existing)
{
  "telemetry": { ... },          // existing
...
| custom | user-defined | Pick exactly what you need via `codespec config profile` |
