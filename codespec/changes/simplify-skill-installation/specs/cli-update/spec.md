...
- **WHEN** user runs `codespec update`
...
- **WHEN** user runs `codespec update`
- **AND** a tool has generated CodeSpec command files
- **AND** that tool has no CodeSpec skill files (commands-only delivery)
...
- **THEN** the system SHALL display: "New in this version: /opsx:propose (combines new + ff). Try 'codespec config profile core' for the streamlined 4-workflow experience."
...
- **WHEN** user runs `codespec update` multiple times
...
- **WHEN** user runs `codespec update` non-interactively (e.g., in CI)
...
- **THEN** the system SHALL display: "Detected new tool: Windsurf. Run 'codespec init' to add it."
...
- **THEN** the system SHALL display one consolidated message listing all detected tools, for example: "Detected new tools: GitHub Copilot, Windsurf. Run 'codespec init' to add them."
...
### Requirement: Update requires an CodeSpec project
The update command SHALL only run inside an initialized CodeSpec project.

#### Scenario: Update outside a project
- **WHEN** user runs `codespec update`
- **AND** no `codespec/` directory exists in the current working directory
- **THEN** the system SHALL display: "No CodeSpec project found. Run 'codespec init' to set up."
...
- **WHEN** user runs `codespec update`
- **AND** project has workflows not in current profile (e.g., user switched from custom to core or deselected workflows via `codespec config profile`)
