...
- **WHEN** user runs `codespec init` interactively and tool directories are detected
...
- **WHEN** user runs `codespec init` interactively and no tool directories are detected
- **THEN** the system SHALL prompt for tool selection
- **THEN** the system SHALL use default profile (`core`) and delivery (`both`)

#### Scenario: Non-interactive with detected tools
- **WHEN** user runs `codespec init` non-interactively (e.g., in CI)
...
- **WHEN** user runs `codespec init` non-interactively
...
- **WHEN** user runs `codespec init --tools claude`
...
- **WHEN** user runs `codespec init --tools claude` interactively
...
- **THEN** the system SHALL display: "Done. Run 'codespec config profile' to configure your workflows."
...
- **WHEN** user runs `codespec init` on a project with existing workflow files
...
- **WHEN** user runs `codespec init` on a project with no existing workflow files
- **AND** global config does not contain a `profile` field
...
- **WHEN** user runs `codespec init --profile core`
...
- **WHEN** user runs `codespec init --profile <invalid>`
...
- **WHEN** user runs `codespec init` interactively
...
- **WHEN** user has custom profile with extra workflows and runs `codespec init` with core profile
...
- **WHEN** user runs `codespec init` on existing project
...
- **WHEN** user runs `codespec init` on an existing project
