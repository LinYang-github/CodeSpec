...
The propose workflow SHALL combine change creation and artifact generation into a single command, reducing friction for new users while teaching them the CodeSpec workflow through embedded guidance.
...
- **THEN** the system SHALL create `.codespec.yaml` in the change directory (via `codespec new change`)
