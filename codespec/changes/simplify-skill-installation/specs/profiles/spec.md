...
- **WHEN** user runs `codespec config profile`
...
- **THEN** the system SHALL display: "Config updated. Run `codespec update` in your projects to apply."

#### Scenario: Core preset shortcut
- **WHEN** user runs `codespec config profile core`
...
- **THEN** the system SHALL NOT modify any project files
- **THEN** the system SHALL display: "Config updated. Run `codespec update` in your projects to apply."
- **THEN** the new profile takes effect on the next `codespec init` or `codespec update` run

#### Scenario: Config profile run inside a project
- **WHEN** user runs `codespec config profile` inside an CodeSpec project directory
- **THEN** after updating global config, the system SHALL prompt: "Apply to this project now? (y/n)"
- **WHEN** user confirms
- **THEN** the system SHALL run `codespec update` automatically
- **THEN** the system SHALL still display: "Run `codespec update` in your other projects to apply."
...
- **WHEN** user runs `codespec config profile` non-interactively (e.g., in CI, no TTY)
- **THEN** the system SHALL display an error: "Interactive mode required. Use `codespec config profile core` or set config via environment/flags."
- **THEN** the system SHALL exit with code 1

### Requirement: Profile settings stored in global config
Profile and delivery settings SHALL be stored in the existing global config file (`~/.config/codespec/config.json`) alongside telemetry and feature flags.
...
- **WHEN** user runs `codespec config list`
...
- **WHEN** user updates config via `codespec config profile`
- **THEN** the system SHALL only update global config (`~/.config/codespec/config.json`)
- **THEN** the system SHALL NOT modify any project skill/command files
- **THEN** existing projects retain their current workflow files until user runs `codespec update`

### Requirement: Config changes applied via update command
The existing `codespec update` command SHALL apply the current global config to a project. See `specs/cli-update/spec.md` for detailed update behavior.

#### Scenario: Config changes require explicit project sync
- **WHEN** user updates profile or delivery via `codespec config profile`
- **THEN** the global config SHALL be updated immediately
- **AND** project files SHALL remain unchanged until `codespec update` is run for that project
