...
CodeSpec installation paths are currently inconsistent:
...
- Add install scope control in `codespec config profile` interactive flow.
- Extend `codespec config list` output with install scope source (`explicit`, `new-default`, `legacy-default`).
