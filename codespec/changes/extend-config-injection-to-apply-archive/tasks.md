...
- [x] 4.1 Route `codespec instructions archive --change <name>` to a dedicated read-only archive instruction handler using existing repo/store root resolution and change validation
...
- [x] 5.5 Before archive-driven spec sync writes a main spec, fetch `codespec instructions specs` once for the selected change/root, apply its rules to the semantic merge, and pass the specs-rule snapshot into inline sync; stop before any main-spec write or change move on lookup failure
...
- [x] 6.6 Run `codespec validate extend-config-injection-to-apply-archive --strict` and reconcile every task with the final implementation diff
