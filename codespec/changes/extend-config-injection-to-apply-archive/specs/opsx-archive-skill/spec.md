...
- **THEN** the skill calls `codespec instructions archive --change "<name>" --json` with the selected-root context
...
- **WHEN** `codespec instructions archive --change "<name>" --json` exits non-zero or does not return valid archive-instruction JSON
...
- **THEN** the skill requests `codespec instructions specs --change "<name>" --json` once using the selected change and planning root
...
- **AND** `codespec instructions specs --change "<name>" --json` exits non-zero or does not return valid artifact-instruction JSON
- **THEN** the skill reports the instruction lookup error
