...
The CLI SHALL provide `codespec instructions archive --change <name>` as a read-only workflow instruction surface for current archive operation inputs.

#### Scenario: Archive JSON contains context and guidance

- **WHEN** a user runs `codespec instructions archive --change <name> --json`
...
- **WHEN** a user runs `codespec instructions archive --change <name>` with configured inputs
