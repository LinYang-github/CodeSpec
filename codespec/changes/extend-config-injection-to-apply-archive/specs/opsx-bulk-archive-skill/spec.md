...
- **THEN** it calls `codespec instructions archive --change "<selected-change>" --json` once for that root
...
- **WHEN** `codespec instructions archive --change "<selected-change>" --json` exits non-zero or does not return valid archive-instruction JSON
...
- **WHEN** a required `codespec instructions specs --change "<name>" --json` lookup exits non-zero or does not return valid artifact-instruction JSON
