...
- **THEN** standalone sync requests `codespec instructions specs --change "<name>" --json` once using the selected change and planning root
...
- **WHEN** `codespec instructions specs --change "<name>" --json` exits non-zero or does not return valid artifact-instruction JSON
