...
the sentence `codespec archive` writes for a new capability, or a marker left in
...
Validation performed inside `codespec archive` SHALL be unaffected, because
...
- **WHEN** `codespec validate --specs` runs
...
- **WHEN** `codespec validate --specs --strict` runs
...
- **WHEN** `codespec archive` validates the rebuilt spec before writing it
...
- **WHEN** `codespec validate --specs` runs on each
