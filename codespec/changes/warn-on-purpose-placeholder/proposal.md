...
[#369](https://github.com/Fission-AI/CodeSpec/issues/369) reported agents leaving
...
- `codespec validate` reports a `## Purpose` that is still the archive
...
- Not breaking: the finding is a warning, so a project that already carries
  placeholders keeps validating by default and only `--strict` fails. `codespec
...
- **Affected behavior**: `codespec validate` on main specs — `validate <spec>`,
...
- **Unaffected**: `codespec archive`, which validates rebuilt specs non-strictly;
