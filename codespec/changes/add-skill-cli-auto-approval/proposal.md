...
Every generated CodeSpec skill drives the `codespec` CLI (`codespec list`, `status`, `instructions`, …). Today the skill frontmatter never pre-approves those calls, so agents that gate Bash on permission prompt the user on every single `codespec` invocation. The workflow stalls on approvals for a first-party, read-mostly CLI the user already opted into by installing CodeSpec.
...
- Every generated `SKILL.md` gains `allowed-tools: Bash(codespec:*)` in its YAML frontmatter, so agents run `codespec` commands from the skill without prompting. Emitted centrally in `generateSkillContent`, so `init`, `update`, every tool's skills directory, and every current and future skill get it uniformly.
...
- Scope is deliberately narrow: only the `codespec` CLI is pre-approved. Per the standard, `allowed-tools` pre-approves rather than restricts — so any other tool a skill or command uses (Read, Write, or arbitrary Bash for builds/tests in `apply`/`onboard`) stays available under the user's normal permission settings, still prompting as before.
...
- `src/core/shared/allowed-tools.ts` — the shared `CODESPEC_CLI_ALLOWED_TOOLS` constant (single source for both surfaces).
