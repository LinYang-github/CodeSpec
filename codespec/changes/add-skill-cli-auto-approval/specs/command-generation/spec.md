...
- **AND** the `allowed-tools` field SHALL have the value `Bash(codespec:*)` so Claude Code runs `codespec` commands from the slash command without prompting for approval
