...
CodeSpec currently assumes command delivery maps directly to command adapters. That assumption does not hold for all tools.

Some tools expose CodeSpec workflows via skill entries rather than adapter-generated command files. Kimi CLI is a concrete example: it invokes skills with forms such as `/skill:codespec-new-change`. In this model, skills are the command surface.
...
- `none`: no CodeSpec command surface
