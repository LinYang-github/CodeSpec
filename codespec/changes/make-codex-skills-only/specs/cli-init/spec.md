...
`codespec init` SHALL treat Codex as a skills-invocable tool, not as an adapter-backed command-file tool.

#### Scenario: Codex command surface resolution
- **WHEN** a user runs `codespec init` and selects Codex
...
`codespec init` SHALL configure Codex through generated CodeSpec skills without generating Codex custom prompt files.

#### Scenario: Initializing Codex with default delivery
- **WHEN** a user runs `codespec init` and selects Codex
- **AND** the active delivery mode is `both`
- **THEN** the command SHALL create the selected CodeSpec skill files under `.codex/skills/`
...
- **THEN** the command SHALL still create the selected CodeSpec skill files under `.codex/skills/`
...
`codespec init` SHALL remove previously managed global Codex prompt files only after replacement Codex skills exist, without deleting user-authored Codex prompts.
...
- **WHEN** `codespec init` runs without interaction and without `--force`
...
- **WHEN** `codespec init` detects managed global Codex prompt files before tool setup
