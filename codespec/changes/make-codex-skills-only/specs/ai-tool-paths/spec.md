...
The system SHALL identify `.codex/skills/` as the supported Codex CodeSpec workflow path.
...
- **THEN** the system SHALL present Codex as a skills-only CodeSpec integration
...
- **THEN** the system SHALL match only exact historical CodeSpec-owned filenames directly under that resolved directory
...
#### Scenario: Repo-local codespec compatibility prompt names
- **WHEN** detecting legacy Codex prompt files in the project tree
- **THEN** the system SHALL match repo-local files named `.codex/prompts/codespec-*.md`
