...
- [x] 2.3 Rename the entry to `Shared .agents skills` so it names the directory instead of a file CodeSpec never writes
...
- [x] 4.2 Validate with `codespec validate --strict`, and confirm `codespec archive` applies cleanly against a scratch copy of `codespec/`
