...
- **WHEN** running legacy artifact detection on a project with files matching `.opencode/command/opsx-*.md` or `.opencode/command/codespec-*.md`
...
- **WHEN** a user runs `codespec init` in a project with old `.opencode/command/` artifacts
...
- **WHEN** a user runs `codespec init` in non-interactive mode (e.g., CI) and legacy artifacts are detected
- **THEN** the system SHALL auto-cleanup legacy artifacts without requiring `--force`
- **AND** legacy slash command files (100% CodeSpec-managed) SHALL be removed
- **AND** config file cleanup SHALL only remove CodeSpec markers (never delete user files)
