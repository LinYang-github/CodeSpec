...
A raw `--root` or `--cwd` flag was rejected because it would bypass registry validation, store identity checks, canonicalization, and existing diagnostics. Asking an Agent to run `cd <root.path> && codespec schemas` was rejected because generated tool permissions and working-directory support differ across Agents.
...
1. Explicit `--store`.
2. Nearest CodeSpec root, including resolution of a config-only `store:` pointer.
