...
`codespec schemas` discovers project-local schemas from the shell's current directory, while commands that consume a schema resolve an authoritative CodeSpec root first. When an explicit store, a local `store:` pointer, or `defaultStore` selects a different root, discovery can recommend a schema that is unavailable where the change will actually be created. Agents currently have to work around this mismatch by resolving a path and trying to change their shell working directory, which is not reliable across supported tools.

## What Changes

- Make `codespec schemas` resolve its project root through the same root-selection contract used by normal CodeSpec commands before listing schemas.
- Add `--store <id>` to `codespec schemas`, including the standard hidden `--store-path` rejection path, so explicit store selection is carried directly by the CLI.
...
- `schema-resolution`: `codespec schemas` resolves and lists schemas from the authoritative CodeSpec root, including explicitly selected and configured stores.

## Impact

- Affected CLI surface: `codespec schemas [--json] [--store <id>]`.
