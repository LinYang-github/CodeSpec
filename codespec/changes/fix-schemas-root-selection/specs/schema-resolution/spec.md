...
- **WHEN** the user runs `codespec schemas --json --store <id>`
...
- **GIVEN** the nearest `codespec/config.yaml` is a config-only root declaring `store: <id>`
- **WHEN** the user runs `codespec schemas --json` without an explicit store flag
- **THEN** the result SHALL include schemas from the declared store root

#### Scenario: Global default store supplies schemas

- **GIVEN** no nearer CodeSpec root or pointer exists
...
- **WHEN** the user runs `codespec schemas --json` without `--store`
...
- **GIVEN** no CodeSpec root, pointer, global default, or registered store exists
- **WHEN** the user runs `codespec schemas --json`
...
- **GIVEN** no CodeSpec root, pointer, or global default exists
...
- **THEN** `codespec schemas` SHALL report the existing root-selection diagnostic and exit non-zero
...
- **WHEN** the user runs `codespec schemas --store-path <path>`
