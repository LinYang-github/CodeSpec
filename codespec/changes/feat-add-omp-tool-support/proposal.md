...
Oh My Pi (OMP) is a terminal AI coding agent whose users expect CodeSpec workflows to be available as slash commands. Without an adapter, users who have OMP configured in their project cannot generate OMP-native command files or get the correct skill transformations from `codespec init` or `codespec update`.
