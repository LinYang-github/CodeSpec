...
The `codespec update` command SHALL refresh Oh My Pi skill files and command files when Oh My Pi is configured, using Oh My Pi's hyphen-based command reference convention.

#### Scenario: Updating Oh My Pi skill files
- **WHEN** `codespec update` runs and Oh My Pi is a configured tool
- **THEN** skill files in `.omp/skills/codespec-<id>/SKILL.md` are refreshed with the latest templates
