...
- **WHEN** a user runs `codespec change graph`
...
- **WHEN** a user runs `codespec change next`
...
- **WHEN** a user runs `codespec change split <change-id>`
...
- **WHEN** a user runs `codespec change split <change-id>` for a parent whose generated child directories already exist
