...
- [ ] 3.1 Add `codespec change graph` to display dependency order for active changes
- [ ] 3.2 Add `codespec change next` to suggest unblocked changes in recommended order
- [ ] 3.3 Add tests for topological ordering and deterministic tie-breaking (lexicographic by change ID at equal depth)

## 4. Split Scaffolding

- [ ] 4.1 Add `codespec change split <change-id>` to scaffold child slices
...
- [ ] 4.5 Implement and test explicit overwrite mode for `codespec change split` (`--overwrite` / `--force`) for controlled re-splitting
...
- [ ] 5.4 Document migration guidance for `codespec/changes/IMPLEMENTATION_ORDER.md` as optional narrative, not dependency source of truth
