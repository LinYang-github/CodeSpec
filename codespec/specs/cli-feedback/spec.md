...
## Purpose
Define `codespec feedback` behavior for creating GitHub issues safely via `gh`, with a manual fallback when automation is unavailable.

## Requirements
### Requirement: Feedback command

The system SHALL provide an `codespec feedback` command that creates a GitHub Issue in the codespec repository using the `gh` CLI. The system SHALL use `execFileSync` with argument arrays to prevent shell injection vulnerabilities.

#### Scenario: Simple feedback submission

- **WHEN** user executes `codespec feedback "Great tool!"`
...
- **AND** the issue is created in the codespec repository
...
- **AND** the issue is created in the codespec repository without the `feedback` label
- **AND** the system displays the created issue URL
...
- **WHEN** user executes `codespec feedback "Title here" --body "Detailed description..."`
...
- **AND** the issue body includes metadata (CodeSpec version, platform, timestamp)

#### Scenario: Long or multiline feedback message

- **WHEN** user executes `codespec feedback` with a long or multiline message
...
- **WHEN** user runs `codespec feedback "message"`
...
- **THEN** the issue body includes:
  - CodeSpec CLI version
  - Platform (darwin, linux, win32)
  - Submission timestamp
  - Separator line: "---\nSubmitted via CodeSpec CLI"
...
- **WHEN** user has disabled telemetry via `CODESPEC_TELEMETRY=0`
- **AND** user runs `codespec feedback "message"`
...
- **AND** submits via `codespec feedback` command on user confirmation
...
- **WHEN** user types `codespec fee<TAB>`
- **THEN** the shell completes to `codespec feedback`

#### Scenario: Flag completion

- **WHEN** user types `codespec feedback "msg" --<TAB>`
