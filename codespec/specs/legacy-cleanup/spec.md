...
## Purpose
Define detection and cleanup behavior for legacy CodeSpec artifacts during initialization and update workflows.

## Requirements
### Requirement: Legacy artifact detection

The system SHALL detect legacy CodeSpec artifacts from previous init versions.

#### Scenario: Detecting legacy config files

- **WHEN** running `codespec init` on an existing project
- **THEN** the system SHALL check for config files with CodeSpec markers:
  - `CLAUDE.md`
  - `.cursorrules`
  - `.windsurfrules`
  - `.clinerules`
...
- **THEN** the system SHALL check for old slash command directories:
  ...
  - `.cursor/commands/codespec/` (note: old format used `codespec-*.md` in commands root)
  - `.windsurf/workflows/codespec-*.md`
  - And equivalent directories for all tools in the legacy SlashCommandRegistry

#### Scenario: Detecting legacy CodeSpec structure files

- **WHEN** running `codespec init` on an existing project
- **THEN** the system SHALL check for:
  - `codespec/AGENTS.md`
  - `codespec/project.md` (for migration messaging only, not deleted)
  - Root `AGENTS.md` with CodeSpec markers
...
The system SHALL preserve user content when removing CodeSpec markers from config files.

#### Scenario: Config file with only CodeSpec content

- **WHEN** a config file contains only CodeSpec marker block (whitespace outside is acceptable)
- **THEN** the system SHALL remove the CodeSpec marker block
...
- **WHEN** a config file contains content outside CodeSpec markers
- **THEN** the system SHALL remove only the `<!-- CODESPEC:START -->` to `<!-- CODESPEC:END -->` block
...
- **WHEN** root `AGENTS.md` contains CodeSpec markers AND other content
- **THEN** the system SHALL remove only the CodeSpec marker block
...
- **WHEN** a legacy slash command directory exists (e.g., `.claude/commands/codespec/`)
...
- **WHEN** `codespec/AGENTS.md` exists
- **THEN** the system SHALL delete the file
- **AND** NOT delete the `codespec/` directory itself
...
- **THEN** the system SHALL display a summary section:
  ...
  Cleaned up legacy files:
    ✓ Removed CodeSpec markers from CLAUDE.md
    ✓ Removed .claude/commands/codespec/ (replaced by CodeSpec skills and commands)
    ✓ Removed codespec/AGENTS.md (no longer needed)
  ```
- **AND IF** `codespec/project.md` exists
