...
This spec defines how CodeSpec collects anonymous usage telemetry to help improve the tool. It governs the `src/telemetry/` module, which handles PostHog integration, privacy-preserving event design, user opt-out mechanisms, and first-run notice display. The spec ensures telemetry is minimal, transparent, and respects user privacy.

## Requirements

### Requirement: Command execution tracking
The system SHALL send a `command_executed` event to PostHog when any CLI command executes, including only the command name and CodeSpec version as properties.

#### Scenario: Standard command execution
- **WHEN** a user runs any codespec command
- **THEN** the system sends a `command_executed` event with `command` and `version` properties

#### Scenario: Subcommand execution
- **WHEN** a user runs a nested command like `codespec change apply`
...
- **WHEN** a user runs `codespec init my-project --force`
...
The system SHALL disable telemetry when `CODESPEC_TELEMETRY=0` or `DO_NOT_TRACK=1` environment variables are set.

#### Scenario: CODESPEC_TELEMETRY opt-out
- **WHEN** `CODESPEC_TELEMETRY=0` is set in the environment
...
- **AND** the user sets `CODESPEC_TELEMETRY=0`
...
- **WHEN** `CI=true` is set
- **AND** `CODESPEC_TELEMETRY=1` is explicitly set
...
- **WHEN** a user runs their first codespec command
- **AND** telemetry is enabled
- **THEN** the system displays: "Note: CodeSpec collects anonymous usage stats. Opt out: CODESPEC_TELEMETRY=0"
