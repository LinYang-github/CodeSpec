...
The `codespec init` command SHALL create a complete CodeSpec directory structure in any project, enabling immediate adoption of CodeSpec conventions with support for multiple AI coding assistants.
## Requirements
...
- **AND** display progress with ora spinners:
  - Show spinner: "⠋ Creating CodeSpec structure..."
  - Then success: "✔ CodeSpec structure created"
  - Show spinner: "⠋ Configuring AI tools..."
  - Then success: "✔ AI tools configured"

### Requirement: Directory Creation

The command SHALL create the CodeSpec directory structure with config file.

#### Scenario: Creating CodeSpec structure

- **WHEN** `codespec init` is executed
...
- **THEN** display animated welcome screen with CodeSpec logo
...
- **AND** create `codespec/config.yaml` with default schema setting
...
- **WHEN** the `codespec/` directory already exists
- **THEN** inform the user that CodeSpec is already initialized, skip recreating the base structure, and enter an extend mode
...
- **AND** spell each command the way the configured tool registers it: `/opsx-<id>` for tools whose command files are named `opsx-<id>`, and the tool's skill invocation (`$codespec-<skill>` for Codex, `/skill:codespec-<skill>` for Kimi Code, `/codespec-<skill>` otherwise) for tools that receive no command files
...
- **THEN** return appropriate exit code:
  - 0: Success
  - 1: General error (including when CodeSpec directory already exists)
  - 2: Insufficient permissions (reserved for future use)
  - 3: User cancelled operation (reserved for future use)

### Requirement: Additional AI Tool Initialization
`codespec init` SHALL allow users to add configuration files for new AI coding assistants after the initial setup.

#### Scenario: Configuring an extra tool after initial setup
- **GIVEN** an `codespec/` directory already exists and at least one AI tool file is present
- **WHEN** the user runs `codespec init` and selects a different supported AI tool
- **THEN** generate that tool's configuration files with CodeSpec markers the same way as during first-time initialization
...
`codespec init` SHALL summarize tool actions when initialization or extend mode completes.
...
`codespec init` SHALL treat extend mode without new native tool selections as a successful refresh.

#### Scenario: Allowing empty extend runs
- **WHEN** CodeSpec is already initialized and the user selects no additional natively supported tools
- **THEN** complete successfully without requiring additional tool setup
- **AND** preserve the existing CodeSpec structure and config files
...
- **THEN** create only the codespec directory structure
...
- **WHEN** user runs `codespec init` without `--tools`
- **THEN** fail with exit code 1
...
- **WHEN** the user runs `codespec init --language <language>` and no CodeSpec config exists
- **THEN** create `codespec/config.yaml` with context instructing agents to write artifacts in the selected language
- **AND** keep CodeSpec structural headings and `SHALL`/`MUST` requirement keywords in English
- **AND** make the language context available to artifact instructions

#### Scenario: Protecting existing project context

- **WHEN** the user runs `codespec init --language <language>` and an CodeSpec config already exists without the same generated language guidance
...
- **THEN** fail before creating CodeSpec files
- **AND** explain why the value is invalid

#### Scenario: Language config cannot be written

- **WHEN** the user runs `codespec init --language <language>` and the new config cannot be written
...
#### Scenario: Running codespec experimental

- **WHEN** user runs `codespec experimental`
- **THEN** delegate to `codespec init`
- **AND** the command SHALL be hidden from help output

## Why

Manual creation of CodeSpec structure is error-prone and creates adoption friction. A standardized init command ensures:
