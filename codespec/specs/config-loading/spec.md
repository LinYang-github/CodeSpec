...
## Purpose
Define how `codespec/config.yaml` is discovered, parsed, validated, and exposed to callers with safe fallbacks.

## Requirements
### Requirement: Load project config from codespec/config.yaml

The system SHALL read and parse the project configuration file located at `codespec/config.yaml` relative to the project root.

#### Scenario: Valid config file exists
- **WHEN** `codespec/config.yaml` exists with valid YAML content
- **THEN** system parses the file and returns a ProjectConfig object

#### Scenario: Config file does not exist
- **WHEN** `codespec/config.yaml` does not exist
- **THEN** system returns null without error

#### Scenario: Config file has invalid YAML syntax
- **WHEN** `codespec/config.yaml` contains malformed YAML
- **THEN** system logs a warning message and returns null

#### Scenario: Config file has valid YAML but invalid schema
- **WHEN** `codespec/config.yaml` contains valid YAML that fails Zod schema validation
...
- **WHEN** `codespec/config.yml` exists and `codespec/config.yaml` does not exist
- **THEN** system reads from `codespec/config.yml`

#### Scenario: Both .yaml and .yml exist
- **WHEN** both `codespec/config.yaml` and `codespec/config.yml` exist
- **THEN** system prefers `codespec/config.yaml`
...
- **WHEN** config file has syntax errors and user runs `codespec new change`
