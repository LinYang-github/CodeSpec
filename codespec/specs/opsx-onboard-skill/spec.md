...
## Purpose
Define `/opsx:onboard` behavior for guiding users through an end-to-end CodeSpec workflow on their real codebase.

## Requirements
### Requirement: OPSX Onboard Skill

The system SHALL provide an `/opsx:onboard` skill that guides users through their first complete CodeSpec workflow cycle with narration and real codebase work.

#### Scenario: Skill invocation

- **WHEN** user invokes `/opsx:onboard`
- **THEN** agent checks if CodeSpec is initialized
- **AND** if not initialized, prompts user to run `codespec init` first
...
- **THEN** agent runs `codespec new change "<name>"` with derived kebab-case name
