...
- **WHEN** user runs `codespec status --change <id>`
...
- **WHEN** user runs `codespec status --change <id> --json`
...
- **WHEN** user runs `codespec status --change <id>` (text or `--json`)
...
- **WHEN** user runs `codespec status --change <id>` on a change with no artifacts
...
- **WHEN** user runs `codespec status` without `--change`
...
- **WHEN** user runs `codespec status --change unknown-id`
- **AND** directory `codespec/changes/unknown-id/` does not exist
- **THEN** the system displays an error listing all available change directories

### Requirement: Next Artifact Discovery

The workflow SHALL use `codespec status` output to determine what can be created next, rather than a separate next-command surface.

#### Scenario: Discover next artifacts from status output

- **WHEN** a user needs to know which artifact to create next
- **THEN** `codespec status --change <id>` identifies ready artifacts with `[ ]`
...
- **WHEN** user runs `codespec instructions <artifact> --change <id>`
...
- **WHEN** user runs `codespec instructions <artifact> --change <id> --json`
- **THEN** the system outputs JSON matching ArtifactInstructions interface

#### Scenario: Unknown artifact

- **WHEN** user runs `codespec instructions unknown-artifact --change <id>`
...
- **WHEN** user runs `codespec instructions proposal --change <id>` on a scaffolded change
...
- **WHEN** user runs `codespec templates`
...
- **WHEN** user runs `codespec new change add-feature`
- **THEN** the system creates `codespec/changes/add-feature/` directory

#### Scenario: Invalid change name
- **WHEN** user runs `codespec new change "Add Feature"` with invalid name
- **THEN** the system displays validation error with guidance

#### Scenario: Duplicate change name
- **WHEN** user runs `codespec new change existing-change` for an existing change
- **THEN** the system displays an error indicating the change already exists

#### Scenario: Create with description
- **WHEN** user runs `codespec new change add-feature --description "Add new feature"`
...
- **WHEN** user runs `codespec status --change <id> --schema tdd`
...
The system SHALL generate schema-aware apply instructions via `codespec instructions apply`.

#### Scenario: Generate apply instructions

- **WHEN** user runs `codespec instructions apply --change <id>`
...
- **WHEN** user runs `codespec instructions apply --change <id> --json`
- **THEN** the system outputs JSON with:
...
- **WHEN** user runs `codespec artifact-experimental-setup --tool cursor`
...
- **WHEN** user runs `codespec artifact-experimental-setup` without `--tool`
...
- **WHEN** user runs `codespec artifact-experimental-setup --tool unknown-tool`
...
- **WHEN** a user runs `codespec status --change <id> --json`
...
- **GIVEN** the change is repo-local
- **WHEN** a user runs `codespec instructions <artifact> --change <id> --json`
- **THEN** instruction output SHALL preserve existing repo-local paths

### Requirement: Workflow skills use CLI artifact context
Generated workflow skills SHALL use CodeSpec CLI output as the source of truth for artifact locations.
...
- **THEN** it SHALL instruct the agent to run `codespec status --change <id> --json`
...
- **THEN** it SHALL instruct the agent to run `codespec instructions <artifact> --change <id> --json`
