...
## Purpose
Define `codespec schema init` behavior for creating project-local schema skeletons in interactive and non-interactive modes.
## Requirements
### Requirement: Schema init command creates project-local schema
The CLI SHALL provide an `codespec schema init <name>` command that creates a new schema directory under `codespec/schemas/<name>/` with a valid `schema.yaml` file and default template files.

#### Scenario: Create schema with valid name
- **WHEN** user runs `codespec schema init my-workflow`
- **THEN** system creates directory `codespec/schemas/my-workflow/`
...
- **WHEN** user runs `codespec schema init "My Workflow"` (contains space)
...
- **WHEN** user runs `codespec schema init existing-schema` and `codespec/schemas/existing-schema/` already exists
...
- **WHEN** user runs `codespec schema init my-workflow` in an interactive terminal
...
- **WHEN** user runs `codespec schema init my-workflow --description "My workflow" --artifacts proposal,tasks`
...
- **WHEN** user runs `codespec schema init my-workflow` in interactive mode
- **AND** user confirms setting as default
- **THEN** system updates an existing `codespec/config.yaml` or `codespec/config.yml` in place with `schema: my-workflow`
- **AND** removes the legacy `defaultSchema` key when updating an existing configuration
- **AND** creates `codespec/config.yaml` when neither configuration file exists

#### Scenario: Set as default via flag
- **WHEN** user runs `codespec schema init my-workflow --default`
- **THEN** system creates the schema and updates an existing `codespec/config.yaml` or `codespec/config.yml` in place with `schema: my-workflow`
- **AND** removes the legacy `defaultSchema` key when updating an existing configuration
- **AND** creates `codespec/config.yaml` when neither configuration file exists

#### Scenario: Skip setting default
- **WHEN** user runs `codespec schema init my-workflow --no-default`
- **THEN** system creates schema without modifying `codespec/config.yaml`

#### Scenario: Invalid config prevents schema creation
- **GIVEN** `codespec/config.yaml` or `codespec/config.yml` is invalid YAML, is not a YAML object, is not a regular file, or is not writable
- **WHEN** user runs `codespec schema init my-workflow --default`
- **THEN** the command exits with a non-zero status
- **AND** does not create `codespec/schemas/my-workflow/`
- **AND** leaves the config byte-for-byte unchanged

#### Scenario: Config failure preserves a schema during forced replacement
- **GIVEN** `codespec/schemas/my-workflow/` already contains user-authored files
- **AND** the project config cannot be validated or atomically replaced
- **WHEN** user runs `codespec schema init my-workflow --force --default`
- **THEN** the command exits with a non-zero status
...
- **WHEN** user runs `codespec schema init my-workflow --json --description "Test" --artifacts proposal`
...
- **WHEN** user runs `codespec schema init "invalid name" --json`
...
- **GIVEN** `codespec/schemas/tdd-driven/` already exists with user-authored files
