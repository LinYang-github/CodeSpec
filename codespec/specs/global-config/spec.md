...
- **THEN** `getGlobalConfigDir()` returns `/custom/config/codespec`
...
- **THEN** `getGlobalConfigDir()` returns `~/.config/codespec` (expanded to absolute path)

#### Scenario: Windows platform
- **WHEN** the platform is Windows
- **AND** `%APPDATA%` is set to `C:\Users\User\AppData\Roaming`
- **THEN** `getGlobalConfigDir()` returns `C:\Users\User\AppData\Roaming\codespec`
