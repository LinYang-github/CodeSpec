...
The `codespec view` command provides a comprehensive dashboard view of the CodeSpec project state, displaying specifications, changes, and progress metrics in a unified, visually appealing format to help developers quickly understand project status.
## Requirements
...
- **WHEN** user runs `codespec view`
- **THEN** system displays a formatted dashboard with sections for summary, active changes, completed changes, and specifications

#### Scenario: No CodeSpec directory

- **WHEN** user runs `codespec view` in a directory without CodeSpec
- **THEN** system displays error message "✗ No codespec directory found"
