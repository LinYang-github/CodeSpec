...
## Purpose
Define top-level `codespec show` behavior for interactive and direct display of change and spec content.
...
- **WHEN** executing `codespec show` without arguments
...
- **WHEN** executing `codespec show` without arguments
- **THEN** do not prompt
- **AND** print a helpful hint with examples for `codespec show <item>` or `codespec change/spec show`
- **AND** exit with code 1

#### Scenario: Direct item display

- **WHEN** executing `codespec show <item-name>`
...
- **AND** if it matches both, print an ambiguity error and suggest `--type change|spec` or using `codespec change show`/`codespec spec show`
- **AND** if it matches neither, print not-found with nearest-match suggestions

#### Scenario: Explicit type override

- **WHEN** executing `codespec show --type change <item>`
- **THEN** treat `<item>` as a change ID and show it (skipping auto-detection)

- **WHEN** executing `codespec show --type spec <item>`
...
- **WHEN** showing a change with `codespec show <change-name> --deltas-only`
...
- **WHEN** showing a spec with `codespec show <spec-id> --requirements`
