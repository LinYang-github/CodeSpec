...
## Purpose
Define `codespec validate` behavior for validating changes and specs with actionable remediation guidance and structured output.
...
- **THEN** show error "No deltas found" with guidance:
  ...
  - Remind authors that files must live under `codespec/changes/{id}/specs/<capability-path>/spec.md`
  - Include an explicit note: "Spec delta files cannot start with titles before the operation headers"
  - Suggest running `codespec change show {id} --json --deltas-only` for debugging
...
- **THEN** include expected header names and a minimal skeleton:
  ...
  - Mention the quick-reference section in `codespec/AGENTS.md` as the authoritative template
...
- **THEN** emit an error explaining that `### Requirement:` lines must be followed by narrative text before any `#### Scenario:` headers
  ...
  - Reference the pre-validation checklist in `codespec/AGENTS.md`
...
- Source file path (`codespec/changes/{id}/proposal.md`, `.../specs/{cap}/spec.md`)
...
- **THEN** print "Next steps" with 2-3 targeted bullets and suggest `codespec change show <id> --json --deltas-only`
...
Validation run inside `codespec archive` SHALL NOT report these issues, because archive enforces the same check when it applies the deltas.

#### Scenario: MODIFIED omits an existing scenario

- **GIVEN** the main spec's requirement has scenarios "A" and "B"
- **WHEN** a change MODIFIES that requirement with only scenario "A" and `codespec validate <change>` runs
...
- **WHEN** `codespec validate <change>` runs
...
- **WHEN** executing `codespec validate` without arguments
...
- **WHEN** executing `codespec validate <item-name>`
...
- **WHEN** executing `codespec validate --all`
- **THEN** validate all changes in codespec/changes/ (excluding archive)
- **AND** validate all specs in codespec/specs/
...
- **THEN** include all change proposals under `codespec/changes/`
- **AND** exclude the `codespec/changes/archive/` directory

- **WHEN** validating with `--specs`
- **THEN** include all specs that have a `spec.md` under `codespec/specs/<capability-path>/spec.md`

#### Scenario: Validate all changes

- **WHEN** executing `codespec validate --changes`
- **THEN** validate all changes in codespec/changes/ (excluding archive)
...
- **WHEN** executing `codespec validate --specs`
- **THEN** validate all specs in codespec/specs/
...
- **WHEN** executing `codespec validate --all --strict`
...
- **WHEN** executing `codespec validate --all --json`
...
- **WHEN** executing `codespec validate --all --json` (or `--changes` / `--specs`)
...
- **AND** suggest passing `--type change` or `--type spec`, or using `codespec change validate` / `codespec spec validate`
...
- **WHEN** executing `codespec validate --type change <item>`
- **THEN** treat `<item>` as a change ID and validate it (skipping auto-detection)

- **WHEN** executing `codespec validate --type spec <item>`
...
- **WHEN** `codespec validate` is executed with `--no-interactive` or with environment `OPEN_SPEC_INTERACTIVE=0`
...
- **WHEN** running `codespec validate <change-id>`
