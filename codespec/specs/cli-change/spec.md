...
## Purpose
Define `codespec change` command behavior for showing, listing, and validating change proposals and deltas.
...
- **WHEN** executing `codespec change show update-error --json`
...
- **WHEN** executing `codespec change list`
- **THEN** scan the codespec/changes directory
- **AND** return list of all pending changes
- **AND** support JSON output with `--json` flag

#### Scenario: Show only requirement changes

- **WHEN** executing `codespec change show update-error --requirements-only`
...
- **WHEN** executing `codespec change validate update-error`
...
- **WHEN** executing `codespec list`
- **THEN** display current list of changes (existing behavior)
- **AND** show deprecation notice: "Note: 'codespec list' is deprecated. Use 'codespec change list' instead."

#### Scenario: Legacy list with --all flag

- **WHEN** executing `codespec list --all`
...
- **WHEN** executing `codespec change show` without arguments
...
- **WHEN** executing `codespec change show` without a change name
...
- **WHEN** executing `codespec change validate` without arguments
- **THEN** display an interactive list of available changes
...
- **GIVEN** stdin is not a TTY or `--no-interactive` is provided or environment variable `OPEN_SPEC_INTERACTIVE=0`
- **WHEN** executing `codespec change validate` without a change name
- **THEN** do not prompt interactively
- **AND** print the existing hint including available change IDs
- **AND** set `process.exitCode = 1`
