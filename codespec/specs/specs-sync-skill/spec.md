...
- **THEN** the agent reads delta specs from `codespec/changes/<name>/specs/`
- **AND** reads corresponding main specs from `codespec/specs/`
...
- **THEN** create new main spec file at `codespec/specs/<capability-path>/spec.md`, preserving the delta's path relative to `specs/`
- **AND** copy the delta's `## Purpose` body into it when the delta has one, matching what `codespec archive` does
