...
## Purpose
Provide shell completion scripts for the CodeSpec CLI, enabling tab-completion for commands, flags, and dynamic values (change IDs, spec IDs) across multiple shells. Supports Zsh, Bash, Fish, and PowerShell.
## Requirements
...
- **WHEN** user executes `codespec completion --help`
...
- **WHEN** user executes `codespec completion generate zsh`
...
- **WHEN** user executes `codespec completion generate bash`
...
- **AND** support dynamic completion for change and spec IDs via `codespec __complete`

#### Scenario: Generating Fish completion

- **WHEN** user executes `codespec completion generate fish`
- **THEN** output a complete Fish completion script to stdout
- **AND** use `complete -c codespec` with conditions
...
- **WHEN** user executes `codespec completion generate powershell`
- **THEN** output a complete PowerShell completion script to stdout
- **AND** use `Register-ArgumentCompleter -CommandName codespec`
...
- **THEN** discover active changes from `codespec/changes/` directory
- **AND** exclude archived changes in `codespec/changes/archive/`
- **AND** return change IDs as completion suggestions
- **AND** only provide suggestions when inside an CodeSpec-enabled project

#### Scenario: Completing spec IDs

- **WHEN** completing arguments for commands that accept spec names (show, validate)
- **THEN** discover specs from `codespec/specs/` directory
...
- **WHEN** user requests completions outside an CodeSpec project
...
- **WHEN** user executes `codespec completion install zsh`
...
- **AND** write completion script to `~/.oh-my-zsh/custom/completions/_codespec`
...
- **WHEN** user executes `codespec completion install zsh` and Oh My Zsh is not detected
- **THEN** create completions directory at `~/.zsh/completions/` if it doesn't exist
- **AND** write completion script to `~/.zsh/completions/_codespec`
...
- **WHEN** user executes `codespec completion install bash`
- **THEN** detect if bash-completion is installed by checking for `/usr/share/bash-completion` or `/etc/bash_completion.d`
- **AND** if bash-completion is available, write to `/etc/bash_completion.d/codespec` (with sudo) or `~/.local/share/bash-completion/completions/codespec`
- **AND** if bash-completion is not available, write to `~/.bash_completion.d/codespec` and source it from `~/.bashrc`
...
- **WHEN** user executes `codespec completion install fish`
- **THEN** create Fish completions directory at `~/.config/fish/completions/` if it doesn't exist
- **AND** write completion script to `~/.config/fish/completions/codespec.fish`
...
- **WHEN** user executes `codespec completion install powershell`
...
- **WHEN** user executes `codespec completion install` without specifying a shell
...
- **WHEN** user executes `codespec completion uninstall zsh`
...
- **AND** if user confirms, remove `~/.oh-my-zsh/custom/completions/_codespec` if Oh My Zsh is detected
- **AND** remove `~/.zsh/completions/_codespec` if standard Zsh setup is detected
- **AND** remove fpath modifications from `~/.zshrc` using marker-based removal
- **AND** display success message

#### Scenario: Uninstalling Bash completion

- **WHEN** user executes `codespec completion uninstall bash`
...
- **WHEN** user executes `codespec completion uninstall fish`
- **THEN** prompt for confirmation (unless `--yes` flag provided)
- **AND** if user confirms, remove `~/.config/fish/completions/codespec.fish`
- **AND** display success message (no config file modification needed)

#### Scenario: Uninstalling PowerShell completion

- **WHEN** user executes `codespec completion uninstall powershell`
- **THEN** prompt for confirmation (unless `--yes` flag provided)
...
- **WHEN** user executes `codespec completion uninstall` without specifying a shell
...
- **AND** implement methods:
  ...
  - `isCodeSpecProject(): boolean` - Checks if current directory is CodeSpec-enabled
...
- **WHEN** `codespec completion install` cannot detect current shell
- **THEN** display error: "Could not auto-detect shell. Please specify shell explicitly."
- **AND** display usage hint: "Usage: codespec completion <operation> [shell]"
- **AND** exit with code 1
...
- **AND** allow redirection to files: `codespec completion generate zsh > /path/to/_codespec`
