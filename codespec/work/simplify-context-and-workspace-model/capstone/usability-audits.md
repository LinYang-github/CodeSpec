...
- **F1 (P1)** Unparseable `codespec/config.yaml` in a real root dumps
...
- Retired `context store` forms: zero hits in the enforced live sweep
  ...
  when present). Planning-history artifacts under `codespec/` are
...
1. `codespec store setup team-plans --path ~/codespec/team-plans` —
   creates the store, registers it, prints the next command.
2. `codespec new change my-first-change --store team-plans` — the
