...
You install CodeSpec and run two commands:

```bash
codespec store setup team-plans --path ~/codespec/team-plans
codespec new change my-first-change --store team-plans
```
...
line (`store: team-plans` in `codespec/config.yaml`) after which the
...
`codespec doctor` answers "is my setup healthy"; `codespec context`
answers "what CodeSpec roots are related by declarations"; and personal
...
- **One root model.** A single resolution precedence (explicit
  ...
  command entry points. Stores are standalone CodeSpec repos in a typed
  local registry.
...
3. `codespec doctor` is top-level and root-scoped; health findings of
   any severity exit 0 (3.6).
4. 4.1's surface is `codespec context` (not `view`/`open`); opening is
...
6. Capstone fixes: the nearest walk now requires a QUALIFYING
   `codespec/` (planning shape or config); every `--json` failure
