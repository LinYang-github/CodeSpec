...
CodeSpec work should live in normal Git files. Those files can live inside the
project repo, or they can live in a separate CodeSpec repo that points at one or
more project repos.
...
1. CodeSpec can live in this project repo or in its own Git repo.
2. If CodeSpec lives in its own repo, users can register that repo locally.
3. Normal CodeSpec commands can create, read, validate, and archive work in that
   selected CodeSpec repo.
4. A project repo with its own CodeSpec root can reference standalone CodeSpec
...
6. The assembled CodeSpec context can show the root plus referenced stores; it
...
- **CodeSpec root**: the `codespec/` folder with `config.yaml`, `specs/`, and
  `changes/`.
- **CodeSpec inside a project repo**: the `codespec/` folder lives inside the
  code repo.
- **Standalone CodeSpec repo**: the `codespec/` folder lives in its own Git
  repo.
- **Store**: a standalone CodeSpec repo registered on this machine. It has a
  thin `.codespec-store/store.yaml` identity file, but the real planning work
  lives in normal files under `codespec/`. (Renamed from the beta noun
  "context store" on 2026-06-11; the CLI group rename lands in slice 1.4.)
- **Reference store**: a standalone CodeSpec repo that a project repo's work
...
- **View**: a local convenience for opening the CodeSpec repo and project repos
  together. It is not the source of truth.

## Rules We Should Not Forget

- Keep the normal `codespec/specs/` and `codespec/changes/` lifecycle working.
- When context stores are used, treat them as standalone CodeSpec repos, not as
...
- [x] **Phase 0. Make the active direction easy to find.**
  ...
  active direction.
- [ ] **Phase 1. Make a standalone CodeSpec repo useful.**
...
- [x] **Phase 3. Say how roots relate: references.**
  ...
  fallback, canonical remotes, and the `codespec doctor`
...
- [ ] **Phase 4. Assemble the working context.**
  Complete (merge to `main` pending): `codespec context` ships the
...
- [x] **0.3** Reframe local agent guidance around CodeSpec roots.
...
### 0.3 Reframe Local Agent Guidance Around CodeSpec Roots
...
- Agent instructions that start with "where is the CodeSpec root?" instead of
  "which beta workspace/context-store mode is this?"

What changed:

- Local guidance was reframed around CodeSpec roots, artifact placement, and
...
```text
I can keep CodeSpec work in its own Git repo and still use normal CodeSpec
commands.
```

Phase checklist:

- [x] **1.1** Create or register a standalone CodeSpec repo.
  Implemented in draft PR #1190.
- [ ] **1.2** Let normal commands use a named standalone CodeSpec repo.
...
### 1.1 Create Or Register A Standalone CodeSpec Repo
...
- Run `context-store setup` and get a normal CodeSpec root in a standalone repo.
- Clone a teammate's standalone CodeSpec repo and register it locally.
- Run `context-store doctor` and see whether the CodeSpec root is healthy.

Why it matters:

- A context store should not feel like a special beta planning system.
- It should be a normal CodeSpec root plus a small identity file.
...
- Register requires an existing healthy CodeSpec root.
- Register can add `.codespec-store/store.yaml` only after confirmation.
...
- `context-store doctor --json` includes a separate `codespec_root` section.
...
```text
When I am in an app repo, I can tell CodeSpec to create or read work in my
registered standalone CodeSpec repo.
```

Example user flow:

```bash
codespec new change add-billing --store team-context
codespec status --store team-context
codespec instructions apply --store team-context
```

What the user can do:

- Stay in the project repo they are working on.
- Pick a registered standalone CodeSpec repo by name.
- Create, inspect, validate, and archive normal CodeSpec work in that selected
  repo.

Why it matters:

- Without this, users can create/register a standalone CodeSpec repo, but normal
  commands still mostly act on the nearest local `codespec/` folder.
- The user should not need initiative links or workspace planning state just to
  put work in a standalone CodeSpec repo.

What changes in commands or files:

- Add `--store <id>` as the way to choose the CodeSpec root for normal
  commands.
- First command set: `new change`, `status`, `instructions`, `list`, `show`,
  `validate`, and `archive`, behind one shared root resolver.
- The selected command writes normal `codespec/changes/` and reads normal
  `codespec/specs/`.
...
- `--store` is repurposed as root selection with exactly one meaning. Phase
  ...
  `--store-path` are removed, and `codespec set change` is removed because
...
- When the current directory has no CodeSpec root and registered stores
...
- Without `--store`, commands keep using the nearest/current CodeSpec root.
- With `--store team-context`, `codespec/changes/<id>` is created in the
  registered store root.
- JSON output shows which CodeSpec root was used.
...
```text
Show that a registered standalone CodeSpec repo can do the same basic lifecycle
as an CodeSpec root inside a project repo — including cloning it and continuing
the work from a second checkout.
```

What the user can do:

- Set up a standalone CodeSpec repo that is a real Git repo (initialized, with
...
- This proves standalone CodeSpec repos are not just setup plumbing.
...
- A clone of a freshly set-up store is immediately a healthy CodeSpec root.
- The final files are normal `codespec/specs/`, `codespec/changes/`, and
  `codespec/changes/archive/` files in both checkouts.
...
- [x] Terminology decided (2026-06-11): the noun is **store**, defined
  everywhere as "a store — a standalone CodeSpec repo you've registered."
  ...
  (`.codespec-store/store.yaml`, registry shape) stay. "Planning repo" and
...
CodeSpec repo and use it without the human spelling out flags — and is no
...
- Read top-level help and recognize the context-store commands as the
  standalone CodeSpec repo feature.
- Follow generated guidance without being pointed at `codespec initiative` or
...
- Two renames riders: remove the second live meaning of `--store` (legacy
  ...
  `codespec store new change <id>` (pointing at
  `codespec new change <id> --store <id>`).
...
- The checked-in `.codex/skills/use-codespec/` guidance, which still
...
```text
Normal CodeSpec work should not require an initiative.
```
...
- `new change` stops creating new initiative links as part of the main product
  path.
- `codespec set change` is removed because initiative linking was its only
  behavior.
- Existing `.codespec.yaml` initiative metadata remains parseable if needed.
- Store/root selection points to normal CodeSpec roots, not initiative
  collections.
...
- A project repo can **reference** the standalone CodeSpec repos its work
  ...
  CodeSpec root, with the store as cited context).

Root resolution precedence is fixed and stated once: explicit `--store` wins,
then the nearest local `codespec/` root, then (only when no local root
...
- **Declarations live in `codespec/config.yaml` (3.1, 3.2).** Both
  `references:` and the fallback `store:` pointer share one home. The
  fallback case is a config-only `codespec/` directory (no `specs/` or
  ...
  top-level marker file was rejected: `.codespec.yaml` is already taken as
...
- Declare in the project repo's `codespec/config.yaml` (for example a
...
- In a repo whose planning is fully externalized (no local `codespec/`),
...
- Optional canonical remote in `.codespec-store/store.yaml` (the shared,
...
- `codespec --help`, instructions, doctor, context, docs, and the agent
...
- [x] Spec written.
- [x] Plan written.
- [x] Implementation done (the root-scoped `codespec doctor` — pure
...
- Ask CodeSpec whether the roots this work relates to — referenced stores and
  the resolved CodeSpec root — are available on the current machine.

Why it matters:

- Agents need to know whether they can read the referenced context and
  trust the resolved CodeSpec root.
- This should be diagnostic only; it should not clone or sync anything.

What changes in commands or files:

- Doctor output reports root, store, and reference health.
- The report clearly separates CodeSpec root health, store metadata health,
...
the CodeSpec root and the stores it references.
```
...
- [x] Implementation done (CP1 deleted the workspace machinery —
  27 files, −2,196 lines; CP2 added `codespec context` with the JSON
...
- From any root, get the full working set its declarations describe: the
  CodeSpec root itself and its referenced stores.
...
- [x] Cleanup done. First tranche complete 2026-06-11: the `workspace`
  ...
  `codespec schemas`); the four `workspace-*` beta change folders
...
- Follow the simple CodeSpec root path without being distracted by obsolete beta
  workflows.
...
no code-repo relationship machinery in the current product path. `codespec
context` remains focused on CodeSpec roots and references.
...
- No changes to `codespec context`, project config parsing, or any committed
  file format.
...
Keep these out of the main queue until the simpler standalone CodeSpec repo path
is working:
...
- **L8** Review local `use-codespec` skill guidance and decide whether it should be an
  ignored local skill, generated artifact, checked-in source, or productized
  default.
- **L9** Fix small baseline quirks, such as JSON support for `codespec list --specs`,
...
- 2026-06-07: Started the active reorientation experiment under
  `codespec/work/` instead of continuing the context-store initiative roadmap.
...
- 2026-06-09: Reframed checked-in `use-codespec` guidance around CodeSpec roots
...
- 2026-06-09: Deferred public concept docs until the simplified model is more
  solid.
- 2026-06-09: Reordered the roadmap around standalone CodeSpec repos and local
  views.
...
- 2026-06-10: Settled the model question behind 1.2: the CodeSpec root is the
...
- 2026-06-11: Locked the open decisions after parallel product-level and
  ...
  declarations in `codespec/config.yaml`, one typed id namespace, and the
...
- 2026-06-11: Wrote the store-rename-and-guidance slice spec (1.4) and
  ...
  proof (`codespec init` in the scratch repo).
...
- 2026-06-11: Decided autonomously (review me): discovered during 1.4
  ...
  use-codespec guidance the roadmap called "checked-in" is actually the
...
- 2026-06-11: Wrote the delete-legacy-command-groups slice spec (the
  ...
  workspace-profile integration (which even executes `npx codespec
...
- 2026-06-11: Capstone (6.1) COMPLETE. The whole-delta gauntlet ran
  ...
  highest-impact being the ~/codespec layout turning $HOME into a
  phantom nearest root (the walk now requires a qualifying codespec/),
...
- 2026-06-11: Capstone (6.1) technical audits done
  ...
  gross insertions dominated by codespec/work planning artifacts.
...
- 2026-06-11: Executed the Phase 5 remainder, closing out 5.1
  ...
  named it after 4.1, but `codespec schemas` still ADVERTISED it — a
...
- 2026-06-11: Implemented slice 4.1 in two checkpoints plus a
  ...
  `codespec context` — the JSON agent brief, the human working-set
...
- 2026-06-11: Decided autonomously (review me): 4.1's surface is a new
  top-level `codespec context` (JSON agent brief / human listing /
...
- 2026-06-11: Implemented slice 3.6 (relationship health) in two
  ...
  root-scoped `codespec doctor`. Later review removed the code-repo
...
- 2026-06-11: Implemented slice 3.3 (store canonical remote) in two
  ...
  reporting edge). Capstone notes recorded: the `~/codespec/<id>`
...
- 2026-06-11: Wrote the store-canonical-remote plan (3.3, two
  ...
  `~/codespec/<id>` form is amended); setup's origin probe must reach
...
- 2026-06-11: Wrote the store-canonical-remote slice spec (3.3) and
  ...
  the clone fix made pasteable verbatim via the `~/codespec/<id>`
...
- 2026-06-11: Implemented slice 3.2 (declared-store fallback) in two
  ...
  `codespec/config.yaml`, the resolver classification (directory-typed
  ...
  alongside the shared `classifyCodeSpecDir` (resolver and init can
...
- 2026-06-11: Wrote the declared-store-fallback slice spec (3.2) and
  ...
  archive's absolute cross-root paths). Also folded: `codespec init`
...
- 2026-06-11: Implemented slice 3.1 (store references) in two
  ...
  `codespec/config.yaml` (raw-string parsing), the
  ...
  `test/helpers/codespec-fixtures.ts`, deleted a dead defensive
...
- 2026-06-11: Decided autonomously (review me): ground truth uncovered
  ...
  library (`codespec/specs/cli-config`, `workspace-*`,
...
- 2026-06-11: Wrote the store-rename-and-guidance plan (four green
  ...
  roots exclude the `codespec/` planning history by design; old-data-dir
  negative fixtures (valid and corrupt) and exact-equality
  `--store`-description tests were added; the dogfood pins
  `codespec init --tools claude --profile core`. Spec updated in the
...
- 2026-06-12: User-directed (owner review of the 4.1 autonomous
  ...
  renames `codespec context` → `codespec workset`, anchored on the work
  ...
  (`codespec map`), with point-of-need prompts and diagnostic fix
...
- 2026-06-12: Ran the 7.1 capstone dogfood
  ...
  session with no insider knowledge — told only that "the codespec
...
- 2026-06-12: Continued owner design review replaced the change-anchored
  ...
  run invocation. `codespec context` is explicitly independent of 7.1.
