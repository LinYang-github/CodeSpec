...
direction into a simpler CodeSpec model that is easier to explain, implement,
and dogfood.
...
CodeSpec artifacts should live in Git. That Git repo may be the project repo,
...
```text
CodeSpec can live in this project repo or in its own Git repo.
...
Agents and commands should be able to assemble the relevant CodeSpec root and
...
- Preserve the current `specs/` and `changes/` baseline while the simpler model
  is introduced.
- Make the placement choice explicit: in-project CodeSpec or standalone
  CodeSpec repo.
- Support layered planning by reference, not redirection: high-level
  ...
  keeps its own CodeSpec root for implementation-level work, drawing on the
...
- Treat the future `work/` layout as a later evolution, not a prerequisite for
  making standalone CodeSpec repos useful.

## Constraints

- Keep the current `codespec/changes/` and `codespec/specs/` lifecycle working.
...
- The next product slices are about preserving the baseline, clarifying
  placement, supporting standalone CodeSpec repos, references, and personal
  worksets.
