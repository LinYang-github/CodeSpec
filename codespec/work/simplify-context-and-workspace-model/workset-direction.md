...
> names folders in a workset or gives an explicit path, not because CodeSpec
...
## 1. `codespec context` becomes `codespec workset`, anchored on the change

- Rename the 4.1 surface to `codespec workset`. "Context" names the data,
...
- Primary form: `codespec workset <change-name>` — the anchor is the work
...
- Bare `codespec workset` remains the root-union view (everything the
