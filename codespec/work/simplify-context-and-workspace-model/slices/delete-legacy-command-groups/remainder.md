...
died with planning-home's collapse), but `codespec schemas` still
...
templates) is deleted; `codespec schemas` now lists `spec-driven`
alone.

## 2. Obsolete beta change folders — the four `workspace-*` DELETED

`codespec/changes/{workspace-agent-guidance, workspace-apply-repo-slice,
...
- **Incidental mentions elsewhere are recorded, not rewritten**:
  ...
  `codespec-conventions`, `schema-resolution` mention workspace
