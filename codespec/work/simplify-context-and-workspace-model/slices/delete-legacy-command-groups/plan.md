...
- `docs/cli.md`: workspace section (~179-349), the six
  `codespec workspace ...` rows in the agent-compatible table (51-56),
  ...
  `codespec workspace update` instructions in the Configuration
...
- New tests (in an existing suitable file or a small
  ...
  - `codespec workspace list` / `codespec initiative list` → unknown
  ...
  - Update fall-through: view-state dir, `codespec update` → standard
  ...
  - Planning-home mode pin: `status --json` over a
    `.codespec-workspace/view.yaml` fixture asserts
...
4. `pnpm run build`, full `pnpm test`, built-binary smoke
   ...
   `grep -rn "codespec workspace\|codespec initiative" docs/ src/ .codex/`
