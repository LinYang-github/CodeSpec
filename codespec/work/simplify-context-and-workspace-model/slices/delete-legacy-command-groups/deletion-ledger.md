...
- **Minor error-fidelity change in `codespec update`**: pre-deletion, an
  unreadable `codespec/` entry (EACCES) surfaced the raw fs error via
...
- **The accepted spec library still describes deleted behavior**:
  `codespec/specs/cli-config`, `workspace-open`, `workspace-foundation`,
