...
The `codespec workspace` and `codespec initiative` command groups no
...
CodeSpec roots, stores, and the normal lifecycle commands. What survives
...
2. **Surviving commands stop pointing at the dead groups — two included
   behavior changes.** (a) `codespec update`'s workspace detection
   (`src/cli/index.ts:~205-210` via `findWorkspaceRoot`) errors with
   "Run `codespec workspace update`…", a dead command after this slice;
   ...
   `codespec workspace update` (`src/commands/config.ts:228-261`), the
   workspace context resolution (`:199-211`), and the interactive
   apply-to-workspace flow that **executes**
   `npx codespec workspace update` (`:674-697`) — is deleted whole.
...
$ codespec workspace list
error: unknown command 'workspace'
$ codespec initiative list
...
- Any change to surviving command behavior beyond the two named in
  decision 2 (`codespec update` detection-block removal; `config`
...
- **WHEN** the user runs `codespec workspace <anything>` or
  `codespec initiative <anything>`
- **THEN** the CLI fails with Commander's unknown-command error, exit 1,
  no alias, no redirect stub
- **AND** `codespec --help` lists neither group
...
- **WHEN** the surviving CLI prints any help, error, hint, or fix text,
  ...
  `codespec workspace` and `codespec initiative`
...
- **WHEN** the representative surviving command set runs — `store list`,
  ...
  `codespec update`, `status`, and `new change` in that store
...
- **GIVEN** the working directory is a workspace view dir with no
  CodeSpec project
- **WHEN** the user runs `codespec update`
