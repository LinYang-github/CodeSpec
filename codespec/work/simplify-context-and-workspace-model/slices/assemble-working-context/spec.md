...
From any root, one command produces the CodeSpec working context its
declarations describe: the resolved CodeSpec root plus referenced stores.
...
The earlier code-repo declaration/map experiment is removed. `codespec context`
...
1. **Assembly is a local convenience, not a planning system.** The selected
   CodeSpec root remains the source of truth; references provide read-only
   upstream context.
...
```json
{
  "root": { "path": "/abs/root", "source": "store|declared|nearest", "store_id": "...", "role": "codespec_root" },
  "members": [
    { "role": "referenced_store", "id": "upstream-context", "path": "/abs/store", "fetch": "codespec show <spec-id> --type spec --store upstream-context", "status": [] },
...
Referenced stores
  upstream-context  /Users/dev/codespec/upstream-context
    Fetch: codespec show <spec-id> --type spec --store upstream-context

Not available on this machine
  - design-system: not registered
    Fix: git clone -- https://github.com/acme/design-system.git /Users/dev/codespec/design-system && codespec store register '/Users/dev/codespec/design-system' --id design-system
```
...
- **GIVEN** a store-backed root with one resolvable and one unresolvable
  reference
- **WHEN** `codespec context` runs in human and JSON modes
...
- **WHEN** `codespec context --code-workspace out.code-workspace` runs
