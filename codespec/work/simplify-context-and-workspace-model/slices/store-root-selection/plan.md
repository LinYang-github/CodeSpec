...
```text
--store <id> selects an CodeSpec root.
```

A context store remains local registration and identity for a standalone
CodeSpec repo. Normal command behavior should read and write ordinary
`codespec/specs/`, `codespec/changes/`, and `codespec/changes/archive/` files in
the resolved root.
...
landed, because `src/core/codespec-root.ts` and the registry health behavior in
...
- "I am in an app repo, but the CodeSpec work lives in my standalone planning
  repo."
- "Use the registered store I named, not a nearby accidental `codespec/` folder."
...
- Which CodeSpec root every command resolved.
- Whether the root came from `--store`, the nearest `codespec/`, or preserved
  implicit-root behavior.
- Whether a selected store is unknown, unhealthy, or mismatched with its
  `.codespec-store/store.yaml` identity.
- Whether a command wrote only the selected root's CodeSpec artifacts.

How the user knows it worked:

- With `--store team-context`, commands use the registered store's root.
- Human mode writes `Using CodeSpec root: team-context (/abs/path)` to stderr.
- JSON mode includes an additive `root` block with the shared shape.
- No new initiative metadata is created, and `codespec set change` is gone.
...
- Add `--store <id>` to the supported top-level commands:
  ...
  `archive`.
- Route those commands through one shared CodeSpec-root resolver.
- Demote leftover workspace view state for those commands. A
  `.codespec-workspace-view.yaml` ancestor is not a normal command root.
...
- Remove `codespec set change` from CLI registration, help, completions metadata,
...
- Do not change deprecated noun-form commands such as `codespec change show` or
  `codespec spec show`; they remain cwd-based and do not gain `--store`.
...
- `src/core/codespec-root.ts` inspects healthy CodeSpec root shape.
...
- `src/core/list.ts`, `src/core/archive.ts`, `src/commands/show.ts`,
  ...
  and `src/utils/item-discovery.ts` still contain cwd-based `codespec/...`
  assumptions.
...
Add a shared resolver for normal CodeSpec commands. It can live in a new module
...
```ts
type CodeSpecRootSource = 'store' | 'nearest' | 'implicit';
...
interface ResolveCodeSpecRootOptions extends StoreSelectorOptions {
  startPath?: string;
  allowImplicitRoot?: boolean;
  commandName: string;
}

interface ResolvedCodeSpecRoot {
  path: string;
  changesDir: string;
  specsDir: string;
  archiveDir: string;
...
- If `storePath` is present, reject deliberately with guidance:
  `codespec context-store register <path>` and then use `--store <id>`.
...
- Selected store roots must be inspected as healthy CodeSpec roots. Do not
  scaffold or repair them.
- Selected store metadata id must match the registry id.
- Store health and metadata errors should point to `codespec context-store
  doctor`.
...
- Resolver check order is: validate store id format, read registry entry, verify
  store metadata identity, then inspect the CodeSpec root shape. Metadata
  missing or mismatched errors win before root-health diagnostics.
- If no store is selected, find the nearest ancestor containing `codespec/` and
  ignore workspace view state.
- If no nearest root exists and registered stores exist, fail with a hint naming
  the registered store ids plus `--store <id>` or `codespec init`.
...
- Commands that currently fail for missing `codespec/changes` or
  `codespec/specs` should keep failing in that no-store/no-root case.
...
- Workflow commands still expect the existing planning-home shape. Provide a
  small adapter from `ResolvedCodeSpecRoot` to the existing `PlanningHome`
...
- When `--store` is selected, write exactly one root banner to stderr before or
  ...
  `Using CodeSpec root: team-context (/abs/path)`.
...
- Resolve the CodeSpec root before validating schema or writing files.
...
- Write changes under the resolved root's `codespec/changes/`.
...
- Update top-level `codespec list` to resolve the root before listing.
- Make `ListCommand` accept an absolute root or directories instead of assuming
  cwd.
- Preserve deprecated noun-form `codespec change list` and `codespec spec list`
  behavior.
...
- Resolve the root in top-level `codespec show`.
...
- Resolve the root in top-level `codespec validate`.
...
- Update successful human next steps to show normal command usage:
  `codespec new change <id> --store <store-id>`.
...
- `areas_option_removed`: tells users that workspace affected areas are not part
  of the normal CodeSpec root path.
...
- Temporary app repo root with no `codespec/`.
- Temporary app repo root with its own `codespec/`.
...
- [ ] Remove `codespec set change` registration and normal command completion
  metadata.
...
- `codespec set change` is not registered, not in help, and not in completion
  metadata.
- JSON success payloads include the shared root block.
