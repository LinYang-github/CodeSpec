...
Normal CodeSpec commands can act on a registered standalone CodeSpec root
selected by name:
...
Selecting a store resolves to an ordinary CodeSpec root. Everything downstream
behaves exactly as if the command had been run from inside that root: the same
`codespec/specs/`, `codespec/changes/`, and `codespec/changes/archive/` files,
...
CodeSpec root should this command use.
...
1. **`--store` means root selection, and only that.** The old initiative
   ...
   Initiative linking was `set change`'s only behavior, so `codespec set
...
4. **No silent implicit-root scaffold when stores are registered.** When the
   current directory has no CodeSpec root and registered stores exist, the
...
```bash
codespec context-store list --json
codespec new change add-billing --store team-context
codespec status --change add-billing --store team-context
codespec instructions proposal --change add-billing --store team-context
codespec archive add-billing --store team-context
```
...
```text
Using CodeSpec root: team-context (/Users/alice/src/team-context)
```

Without `--store`, commands keep using the nearest CodeSpec root when one
...
- One shared CodeSpec-root resolver behind those commands, replacing the
  per-command `cwd + codespec/changes` path joins.
...
- Removing `codespec set change` from the CLI, help, completions metadata,
...
- Deprecated noun-form commands (`codespec change show`, `codespec spec
...
CodeSpec root and runs the command against that root.

#### Scenario: Creating A Change In A Selected Store

- **GIVEN** a registered context store `team-context` with a healthy CodeSpec
  root
- **AND** the current directory is a project repo without its own `codespec/`
  root
- **WHEN** the user runs `codespec new change add-billing --store team-context`
- **THEN** CodeSpec creates `codespec/changes/add-billing/` inside the
  `team-context` store root
- **AND** CodeSpec writes no CodeSpec artifacts under the current directory
...
- **THEN** each command reads the store's `codespec/changes/` and
  `codespec/specs/`
- **AND** `archive` moves the change into the store's
  `codespec/changes/archive/`
- **AND** no CodeSpec artifacts under the current directory are read or
  written
...
- **THEN** CodeSpec uses the `team-context` store root
- **AND** CodeSpec does not read or write the nearby local root
...
- **THEN** CodeSpec fails with an error naming the unknown id
- **AND** the error lists the registered store ids
- **AND** CodeSpec creates no files

#### Scenario: Rejecting An Unhealthy Store Root

- **GIVEN** a registered store whose CodeSpec root is missing or incomplete
- **WHEN** the user runs a supported command with `--store` for that id
- **THEN** CodeSpec fails with an error describing the root problem
- **AND** the error points to `context-store doctor`
- **AND** CodeSpec does not scaffold or repair the store root

#### Scenario: Rejecting A Mismatched Store Identity

- **GIVEN** a registered store whose `.codespec-store/store.yaml` id does not
  match its registry id
- **WHEN** the user runs a supported command with `--store` for that id
- **THEN** CodeSpec fails with an error describing the identity mismatch
- **AND** the error points to `context-store doctor`

#### Scenario: Path Selection Is Not Available

- **WHEN** the user passes `--store-path` to a supported command
- **THEN** CodeSpec rejects the option
...
- **WHEN** the user runs a supported command without `--store`
- **THEN** CodeSpec fails without scaffolding a new local root
- **AND** the error names the registered store ids
- **AND** the error suggests `--store <id>` or `codespec init`
...
- **GIVEN** an ancestor directory contains leftover
  `.codespec-workspace-view.yaml` state
- **AND** the current directory is inside a repo with an `codespec/` root
- **WHEN** the user runs a supported command without `--store`
- **THEN** CodeSpec uses the nearest `codespec/` root
- **AND** CodeSpec does not route to a workspace-owned changes directory
- **AND** CodeSpec does not switch to the workspace-planning schema
...
- **GIVEN** an ancestor directory contains leftover workspace view state
- **AND** no ancestor directory contains an `codespec/` root
- **WHEN** the user runs a supported command without `--store`
- **THEN** CodeSpec treats the directory as having no CodeSpec root
...
- **WHEN** the user runs `codespec new change add-billing --areas api`
- **THEN** CodeSpec rejects `--areas`
- **AND** CodeSpec does not switch to the workspace-planning schema
- **AND** CodeSpec does not create affected workspace-link metadata

#### Scenario: Goal Metadata Does Not Select Workspace Planning

- **WHEN** the user runs `codespec new change add-billing --goal "Improve billing"`
- **THEN** CodeSpec uses the same root resolution it would use without `--goal`
- **AND** `--goal` may write the existing change goal metadata
- **AND** CodeSpec does not create workspace-owned planning state
...
- **THEN** CodeSpec creates no initiative link or initiative metadata
...
- **WHEN** the user runs `codespec set change` or `codespec set change --help`
- **THEN** the command is no longer available
- **AND** CodeSpec does not print deprecated command guidance for initiative
  linking
- **AND** CodeSpec creates or modifies no files
...
- **THEN** CodeSpec does not modify or delete that metadata in this slice
...
- **AND** `implicit` is used only for preserved no-store behavior where a
  command is allowed to treat the current directory as an implicit CodeSpec root
...
- **THEN** CodeSpec never opens an interactive picker or confirmation prompt
- **AND** if a change id or confirmation is required, CodeSpec fails
