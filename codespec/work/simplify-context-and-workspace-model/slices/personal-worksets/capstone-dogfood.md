...
under `/tmp/codespec-7.1-dogfood/`), three member folders (a planning
...
test is the built `dist/cli/index.js` via an `codespec` wrapper on the
...
```text
$ codespec workset create platform --member src/team-context --member src/web-app --member notes --tool claude
...
platform  (opens in Claude Code)
  team-context  /private/tmp/codespec-7.1-dogfood/src/team-context
  web-app       /private/tmp/codespec-7.1-dogfood/src/web-app
  notes         /private/tmp/codespec-7.1-dogfood/notes
exit=0

$ codespec workset open platform --tool code
Opening 'platform' in VS Code (a window opens; this command returns).
exit=0

$ codespec workset open platform   # saved tool: claude (attach-dirs)
Handing this terminal to Claude Code for 'platform' (the session ends when you exit).
exit=0

$ codespec workset open platform --tool codex
...
{
  "folders": [
    { "name": "team-context", "path": "/private/tmp/codespec-7.1-dogfood/src/team-context" },
    { "name": "web-app", "path": "/private/tmp/codespec-7.1-dogfood/src/web-app" },
    { "name": "notes", "path": "/private/tmp/codespec-7.1-dogfood/notes" }
  ]
}
```
...
```json
{"tool":"code","cwd":".../src/team-context","args":[".../data/codespec/worksets/platform.code-workspace"]}
...
```text
$ codespec workset open platform --tool zed   # unknown tool: the strand test
Error: Unknown tool 'zed'.
Fix: Known tools: code, cursor, claude, codex. Add new tools under "openers" in /tmp/codespec-7.1-dogfood/config/codespec/config.json.
Open manually:
  Workspace file: /tmp/codespec-7.1-dogfood/data/codespec/worksets/platform.code-workspace
  Members:
    team-context  /private/tmp/codespec-7.1-dogfood/src/team-context
    web-app       /private/tmp/codespec-7.1-dogfood/src/web-app
    notes         /private/tmp/codespec-7.1-dogfood/notes
...
$ rm -rf notes && codespec workset open platform --tool code   # missing member
Skipped 'notes' (/private/tmp/codespec-7.1-dogfood/notes is not available).
Opening 'platform' in VS Code (a window opens; this command returns).
exit=0

$ codespec workset remove platform --yes
Removed workset 'platform'. Member folders were not touched.
exit=0

$ codespec workset list
No worksets saved. Create one with: codespec workset create
exit=0
```
...
? Folder path: .
  Added 'codespec-7.1-dogfood' (/private/tmp/codespec-7.1-dogfood)
...
Open it any time with: codespec workset open platform-two
exit=0
```
...
three folders daily, was told "the codespec CLI can keep a named view
of folders and open them together", knows no commands, and must start
from `codespec --help`. The agent's own report of its path:

```sh
codespec --help
codespec workset --help
codespec workset create --help
codespec workset open --help
codespec workset list --help
codespec workset create daily-context --member ./src/team-context --member ./src/web-app --member ./notes --tool claude --json
codespec workset open daily-context --tool claude
```
...
`/tmp/codespec-7.1-dogfood/` during the run; the durable record is
this file.
