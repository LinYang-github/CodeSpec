...
- `test/commands/workspace-initiative-open.test.ts:~93-121` —
  ...
  `CODESPEC_FAKE_OPEN_LOG`); resurrect as a shared helper
...
- `test/commands/context.test.ts:20-27` — the XDG isolation block
  ...
  `CODESPEC_TELEMETRY: '0'`, `getGlobalDataDir({env})`).
...
4. Tests:
   ...
   - `test/cli-e2e/workset-journey.test.ts`: compose→list→open(both
     ...
     (fs-snapshot); `codespec context`/`doctor` byte-identical
     before/after.
5. Full `pnpm test` green; commit.
...
- No changes to `codespec context`, doctor, project config parsing, or
