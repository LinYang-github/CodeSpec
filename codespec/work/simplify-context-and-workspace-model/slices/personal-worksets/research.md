...
`$XDG_DATA_HOME/codespec` when set on any platform, else win32
`%LOCALAPPDATA%/codespec` (with a homedir fallback), else
`~/.local/share/codespec`. Fully injectable via
...
code appended `WORKSPACE_OPEN_MINIMAL_PROMPT = 'Open this CodeSpec
...
- CLI e2e harness: `runCLI` spawns the built `dist/cli/index.js` with
  ...
  `CODESPEC_TELEMETRY: '0'`, and `getGlobalDataDir({ env })` so fixtures
...
- The capstone's fake-executable machinery exists fully formed one commit
  ...
  `{ cwd, args }` to `$CODESPEC_FAKE_OPEN_LOG`, a posix `#!/bin/sh` shim
