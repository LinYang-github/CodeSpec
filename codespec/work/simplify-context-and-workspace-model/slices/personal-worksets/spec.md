...
8. **No changes** to `codespec context`, project config parsing, or any
...
1. **Command surface**: a new `workset` command group —
   `codespec workset create [name]` (guided compose; non-interactive
   via flags), `codespec workset list`, `codespec workset open <name>
   [--tool <id>]`, `codespec workset remove <name>`. "create" over
...
8. **`workset open` does not support `--json`** (recorded as a
   ...
   `codespec workset --json` probe, which the store group's
...
10. **Fallback is the failure path** (FR2.4), and the rule is
    ...
    *every* open failure except a prompt cancellation — tool not on
    ...
    (`codespec workset open <name> --tool <id>`) — including on
...
11. **Missing members degrade, absent worksets fail**: the open-time
    ...
    `codespec workset create` instead.
...
13. **Compose flow** (house `@inquirer` idiom; dynamic imports;
    ...
    **available** tools only, FR2.2; when none of the known tools is
    ...
    `codespec workset open <name>` line; the offer is skipped when no
...
$ codespec workset open platform --tool cursor
Error: Cursor ('cursor') is not on PATH.
Fix: Install 'cursor' or run: codespec workset open platform --tool code
Open manually:
  Workspace file: /Users/dev/.local/share/codespec/worksets/platform.code-workspace
  Members:
    team-context  /Users/dev/src/team-context
    web-app       /Users/dev/src/web-app
```
...
```text
$ codespec workset create ci-triage --member ~/src/ci --member runner=~/src/ci-runner --tool codex --json
{
  "workset": {
    "name": "ci-triage",
    "tool": "codex",
...
$ codespec workset remove ci-triage --yes
...
- Any change to `codespec context`, `codespec doctor`, reference parsing, or
...
- **WHEN** the user runs `codespec workset create` interactively,
...
- **AND** the flow offers to open immediately; declining prints the
  `codespec workset open platform` next step
...
- **WHEN** `codespec workset create ci --member <pathA>
...
- **WHEN** `codespec workset list` runs
...
- **GIVEN** a saved workset whose `.code-workspace` was generated by a
  prior open
- **WHEN** `codespec workset remove platform` runs interactively and
...
- **WHEN** each composes a different workset over that root — one
  adding an unrelated plain folder (no CodeSpec anything), one a
...
- **GIVEN** workset `platform` and a fake `code` on PATH (recorder
  shim)
- **WHEN** `codespec workset open platform --tool code` runs
...
- **THEN** `codespec context`, `codespec doctor`, and the store registry behave
