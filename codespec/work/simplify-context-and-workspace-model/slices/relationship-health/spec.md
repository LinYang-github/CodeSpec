...
One read-only question, one place: is the resolved CodeSpec root healthy, and
are its referenced stores available on this machine? `codespec doctor` answers
...
2. **The report separates** CodeSpec root health, store metadata health,
   reference health, and top-level relationship warnings.
3. **The surface is top-level `codespec doctor`.** It is root-scoped, not
...
4. **No new health machinery.** Reference health reuses the reference index
   diagnostics; root health reuses `inspectCodeSpecRoot`; store-backed roots
...
References
  - upstream-context: ok (/Users/dev/codespec/upstream-context)
...
- **WHEN** `codespec doctor` runs in human and JSON modes
