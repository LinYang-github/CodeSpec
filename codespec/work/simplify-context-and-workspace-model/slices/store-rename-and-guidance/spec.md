...
The product noun for a registered standalone CodeSpec repo is **store**,
...
1. **The noun is "store"**, defined everywhere as "a store — a standalone
   CodeSpec repo you've registered." "Planning repo" and "contracts repo"
...
4. **Committed store-repo formats stay:** the `.codespec-store/` metadata
...
5. **Two riders land with the rename:**
   ...
   - Add an unknown-subcommand hint under the `store` group for the
     inevitable `codespec store new change <id>` mistake, pointing at
...
- **Total token rename.** After this slice, no live surface — help,
  ...
  (`.codespec-store/` directory name, `store.yaml` filename) and the
  `codespec/` planning-history tree (`work/`, `changes/`,
...
6. **The checked-in beta guidance is cut, not updated.**
   `.codex/skills/use-codespec/references/shared-context-beta.md`
...
```bash
codespec store setup team-context --path ~/src/team-context
codespec store list
codespec store doctor
```

Top-level help describes the group as the standalone CodeSpec repo
feature, in the locked vocabulary:

```text
store      Create and manage stores - standalone CodeSpec repos you register on this machine
```

The `--store` flag on lifecycle commands reads "Store id to use as the
CodeSpec root (a store is a standalone CodeSpec repo you've registered)".
...
work: discover with `codespec store list --json`, then carry
`--store <id>` on every lifecycle command. The agent runs:

```bash
codespec store list --json          # finds id: team-context
codespec new change billing-rework --store team-context
```
...
```text
$ codespec store new change billing-rework
Error: unknown command 'new' for 'codespec store'.
...
--store, for example:
  codespec new change billing-rework --store <id>
```
...
- **Machine tokens, repo-wide per the rename rule**: every diagnostic
  ...
  root-selection, codespec-root); every JSON output key
...
- **Preserved formats, guarded by tests**: `.codespec-store/` directory
...
- **Rider 2**: an unknown-subcommand handler on the `store` group naming
  ...
  lifecycle-shaped mistakes at `codespec <command> --store <id>`. The
...
- **Generated workflow skills** (`src/core/templates/workflows/`, 12
  ...
  `codespec store list --json` and carry `--store <id>` on every
  `codespec` command the skill issues; note that printed hints carry the
...
- **Checked-in agent guidance**: `.codex/skills/use-codespec/SKILL.md`
  rewritten around store discovery (`codespec store list --json` as the
...
- **Dogfood acceptance** (runbook): a headless agent session in a scratch
  ...
  `codespec init` in the scratch repo), with isolated XDG state and a
...
- Changes to `.codespec-store/store.yaml` or registry content shapes.
...
- **GIVEN** the built CLI
- **WHEN** the user runs `codespec store setup|register|unregister|remove|list|ls|doctor`
- **THEN** each behaves exactly as its `context-store` counterpart did
  before this slice
- **AND** `codespec context-store <anything>` fails as an unknown command
  with no alias or redirect
- **AND** `codespec --help` lists `store` with a one-liner using the
...
- **THEN** the only matches are the committed format literals
  (`.codespec-store/`, `store.yaml` where it names that file), the
  `codespec/work/` planning-history folder, and archived/changelog
  history
...
- **AND** the store root still carries `.codespec-store/store.yaml` with
...
- **WHEN** the suite runs
- **THEN** explicit assertions pin `.codespec-store` and `store.yaml` as
...
- **WHEN** the user runs `codespec workspace open --store x` or
  `--store-path /tmp/x`
...
- **WHEN** the user runs `codespec store new change add-x` (or another
...
- **AND** points at the normal command with `--store`, for example
  `codespec new change add-x --store <id>`
...
- **WHEN** any of their reachable errors, hints, or fix texts names an
  `codespec` command (for example initiative resolution's
...
- **WHEN** a skill instructs the agent to run root-resolving `codespec`
  commands
- **THEN** the skill teaches discovering stores with
  `codespec store list --json` and carrying `--store <id>` on every
...
- **WHEN** an agent reads any file under `.codex/skills/use-codespec/`
- **THEN** store inspection is `codespec store list --json`
...
- **WHEN** the user reads `codespec --help` or completions metadata
...
- **GIVEN** a scratch project repo prepared with `codespec init` (so the
