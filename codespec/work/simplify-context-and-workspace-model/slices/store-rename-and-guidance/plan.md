...
- A generated skill that says how to discover stores
  (`codespec store list --json`) and to carry `--store <id>` on every
...
- Regenerate guidance: store teaching in all workflow skill templates,
  ...
  `.codex/skills/use-codespec/`, docs accuracy pass.
...
- No `.codespec-store/store.yaml` or registry shape changes.
...
- `src/core/context-store/` → becomes `src/core/store/`:
  - `foundation.ts` — constants at lines 12–15:
    `CONTEXT_STORE_METADATA_DIR_NAME = '.codespec-store'` and
...
- `src/core/codespec-root.ts` — dotted diagnostic fields (110, 119).
...
- `.codex/skills/use-codespec/SKILL.md` — description (3), beta routing
  (41–48, 67–68), invariants (73–80).
- `.codex/skills/use-codespec/references/shared-context-beta.md` —
  deleted.
- `.codex/skills/use-codespec/references/artifact-placement.md` — beta
...
5. Command registration rename (`store` group, subcommands untouched) and
   ...
   now name `codespec store ...` commands. Completions registry entries
...
2. Unknown-subcommand hint on the `store` group: Commander 14's
   ...
   `codespec <command> --store <id>` redirect, then exit 1 via
...
3. Tests: workspace-open unknown-option rejection (rewrite the four
   ...
   spec's negative assertions — `codespec context-store <anything>`
   fails as unknown with no alias, and `codespec --help` lists `store`
...
- **Templates**: add one shared store-selection block (single exported
  ...
  discover with `codespec store list --json`, carry `--store <id>` on
...
- **Checked-in guidance + docs**: rewrite
  `.codex/skills/use-codespec/SKILL.md` (store discovery as the
  ...
  stream by extracting the fenced `codespec` invocations from the
...
1. **Sweep-as-test**: new `test/vocabulary-sweep.test.ts` walking
   ...
   file:line. The `codespec/` tree (planning history: `work/`,
   ...
   literals (`.codespec-store`, `store.yaml`) don't match the patterns
   at all.
2. **Format guards**: explicit test pinning `.codespec-store` and
...
3. **Telemetry**: one assertion that the tracked command path for a
   ...
   `.codex/skills/use-codespec/` contains no `initiative list`/
   `workspace list` steps.
4. **Dogfood proof**: scratch project repo,
   `codespec init <scratch> --tools claude --profile core` (the
   `--tools` flag disables prompting, `src/core/init.ts:175–177`),
   isolated `XDG_*` state, `codespec store setup team-context --path
   ...
   the store's `codespec/changes/` and no initiative/workspace command
