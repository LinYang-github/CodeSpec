...
2. **Declarations live in `codespec/config.yaml`.** A `references:` list
...
1. **The index lives in both instruction surfaces, both modes.**
   Artifact instructions (`codespec instructions <artifact> --change
   ...`) and apply instructions (`codespec instructions apply`) both
...
3. **Problems degrade to warnings, never to silence or failure.**
   ...
   - `reference_unresolved` — the id has no registry entry; fix names
     ...
     codespec store register <path> --id <the-referenced-id>" (naming
   ...
   - `reference_root_unhealthy` — the registry resolved the id but
     ...
     mismatched store metadata, unhealthy CodeSpec root per
     `inspectCodeSpecRoot().healthy === false`); fix:
     `codespec store doctor <id>`.
...
```text
$ codespec instructions design --change billing-rework
...
<referenced_stores>
...
The agent fetches `codespec show billing --type spec --store
...
Store team-context: not registered on this machine.
  Fix: get a checkout from a teammate and run: codespec store register <path> --id team-context
</referenced_stores>
```
...
- **One shared index assembler** (new module under `src/core/`, e.g.
  ...
  `codespec/specs/`, extract first-line summaries tolerantly
...
- **GIVEN** `codespec/config.yaml` with `references: [team-context,
...
- **WHEN** `codespec instructions <artifact> --change <id> --json` runs
  in the app repo
- **THEN** the JSON carries `references: [{store_id: "team-context",
  root: <absolute path>, specs: [{id, summary}, ...], fetch: "codespec
...
- **THEN** generation succeeds, and the index entry carries
  ...
  referenced id: `codespec store register <path> --id <id>`
- **AND** a registered referenced root that is unhealthy — or whose
  checkout path no longer exists on disk — yields
  `reference_root_unhealthy` with the `codespec store doctor <id>` fix
...
- **AND** the design lands in the app repo's `codespec/changes/`, the
