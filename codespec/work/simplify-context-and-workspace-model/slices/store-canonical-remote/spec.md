...
1. **Optional canonical remote in `.codespec-store/store.yaml`** (the
...
2. **How each gets populated.** `store setup` gains `--remote <url>`,
   ...
   ("Edit <abs path>/.codespec-store/store.yaml and commit it") —
...
3. **The unresolved-reference clone source rides the declaration.** For
   ...
   (`<home>/codespec/<id>` via `os.homedir()` — `~` does not expand
   outside a shell, and agent JSON consumers execute argv directly):
   `git clone <remote> /home/me/codespec/<id> && codespec store register /home/me/codespec/<id> --id <id>`;
...
6. **Register guidance upgrades where the remote is knowable.** The
   ...
   ("Share it: teammates clone <remote> and run codespec store
...
Store team-context: not registered on this machine.
  Fix: git clone git@github.com:acme/team-context.git /Users/dev/codespec/team-context && codespec store register /Users/dev/codespec/team-context --id team-context
</referenced_stores>
```
...
```text
$ codespec store doctor team-context
Store doctor
...
- **References**: `references:` entries accept `string | {id, remote?}`
  ...
  (`git clone <remote> <home>/codespec/<id> && codespec store register <home>/codespec/<id> --id <id>`,
...
- **THEN** `<p>/.codespec-store/store.yaml` contains `remote: <url>`
...
- **THEN** the `reference_unresolved` fix is
  `git clone <url> <home>/codespec/team-context && codespec store register <home>/codespec/team-context --id team-context`
