...
- **Tests**: `test/core/store/foundation.test.ts` (metadata
  ...
  `test/helpers/` (run-cli, store-git, codespec-fixtures,
  fs-snapshot).
...
2. `references.ts`: `AssembleReferenceIndexInput.references:
   ...
   (`git clone <remote> <home>/codespec/<id> && codespec store
   register <home>/codespec/<id> --id <id>`) when remote present,
