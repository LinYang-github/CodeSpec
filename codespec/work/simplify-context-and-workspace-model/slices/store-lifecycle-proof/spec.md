...
A registered standalone CodeSpec repo provably supports the same basic
lifecycle as an CodeSpec root inside a project repo, including the sharing
...
```bash
codespec context-store setup team-context --path ~/src/team-context
```
...
put work in the store, and the one thing CodeSpec will not do for them:
...
codespec context-store register team-context
```

Because setup committed the store shape, the clone is immediately a healthy
CodeSpec root and register succeeds without ceremony. From then on, both
...
- **AND** store directories that would otherwise be empty (for example
  `codespec/specs/` and `codespec/changes/archive/`) contain a tracked
...
- **AND** the placeholder files appear in `created_files` and the initial
  commit
- **AND** a clone of the store is immediately a healthy CodeSpec root

#### Scenario: Committing Only What Setup Created

- **GIVEN** setup runs against an existing Git repository it accepts (for
  example a healthy CodeSpec root missing only identity metadata)
...
- **GIVEN** setup first accepts an existing healthy CodeSpec root that is
  not yet registered
- **AND** its `codespec/specs/` or `codespec/changes/archive/` directories
  are empty
...
- **AND** the editable suggestion is a user-visible path (for example
  `~/codespec/<id>`), not the managed XDG data directory
...
- **AND** no files, directories, Git repository, or registry entries are
  created
- **AND** setup does not commit using an invented CodeSpec-local identity
...
- **AND** `codespec/specs/` or `codespec/changes/archive/` exists but
...
- **GIVEN** a folder whose `.codespec-store/store.yaml` id differs from the
  requested `--id`
...
- **GIVEN** a directory that is a Git repository without a healthy CodeSpec
  root (for example a clone of a commitless store)
- **WHEN** the user runs register against it
- **THEN** the refusal names the missing CodeSpec root pieces
...
- **GIVEN** register creates `.codespec-store/store.yaml` after confirmed
...
- **WHEN** its output includes a hint naming a follow-up `codespec` command
...
- **WHEN** the command then fails (for example `instructions apply` with no
  active changes)
- **THEN** stderr still includes the `Using CodeSpec root` banner
...
- **GIVEN** simulated machine A with isolated global state and a project
  repo without its own CodeSpec root
...
- **AND** the change ends in the store's `codespec/changes/archive/` with
  the store's `codespec/specs/` updated
...
- **AND** the final files are normal artifacts in the clone's `codespec/`
  root

#### Scenario: End State Is Just Normal Files

- **WHEN** the journey completes
- **THEN** each checkout contains only normal `codespec/` artifacts, the
  thin `.codespec-store/store.yaml` identity file, and Git state
