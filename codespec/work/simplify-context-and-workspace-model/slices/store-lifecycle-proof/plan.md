...
- The journey test passes against the built binary with isolated global
  state, ending in nothing but normal CodeSpec files.
...
- No retrofit of placeholders into stores created before this slice, and no
  change to `codespec init` baseline roots (their clone fragility is an L9
...
- `src/core/context-store/operations.ts` (916 lines) owns setup/register/
  ...
  `.codespec-store/store.yaml` is written inside
  ...
  Register errors live here: `requires an existing healthy CodeSpec root`
...
- `src/commands/workflow/status.ts` prints `Planning home: <label>`
  ...
  codespec new change <name>` (line ~75).
- `src/commands/workflow/shared.ts` throws storeless hints at lines ~148
  and ~169 (`No changes found. Create one with: codespec new change
  <name>`).
...
1. Resolve input. Non-interactive or JSON without `--path` fails with new
   ...
   user-visible path such as `~/codespec/<id>`. Setup never silently picks
...
4. Create all in-store files: the root shape, a tracked placeholder file
   (`.gitkeep`) inside `codespec/specs/` and `codespec/changes/archive/`
   ...
   `.codespec-store/store.yaml` when missing. This requires restructuring:
...
5. `git init` when needed, then an index-preserving pathspec commit
   ...
   CodeSpec context store <id>" -- <pathspecs>`). The commit set depends
   on who owns the repository: when setup initialized it, the pathspecs
   are the full store shape (`codespec/` plus `.codespec-store/`) so a
...
- Warning when `codespec/specs/` or `codespec/changes/archive/` exists but
...
- `registry.ts` already-registered error: replace "choose a different
  ...
  names the registered path and `codespec context-store unregister <id>`
...
- Add a small helper (likely in `root-selection.ts`) that formats a
  follow-up `codespec ...` suggestion and appends `--store <id>` when the
  ...
  other supported-command hint found by grepping for `codespec new change`
  / `codespec ` literals in supported command paths.
...
- `new change`: after the created-change lines, print a next-step line
  ...
  (`codespec instructions <artifact> --change <id> --store <id>` when
  selected); fall back to `codespec status --change <id>` if the first
...
4. Assert: change in `changes/archive/`, spec promoted into
   `codespec/specs/`, project repo byte-identical (hash the tree before and
   after), banners on stderr, stdout payloads clean.
5. Commit machine A's work (the test acts as the user; CodeSpec must not
   commit here).
...
9. Both checkouts contain only normal `codespec/` artifacts,
   `.codespec-store/store.yaml`, placeholders, and Git state. No
...
- The two-checkout journey passes against the built CLI with isolated
  global state, ending in normal CodeSpec files only, and the full suite is
  green.
