...
- **Resolver**: `resolveCodeSpecRoot` (`src/core/root-selection.ts:258-314`);
  the nearest-root arm at 277-280 (`findRepoPlanningRootSync` returns
  the project root whose `codespec/` exists and terminates at the
...
- **Init**: `InitCommand.execute` → `createDirectoryStructure`
  ...
  existing `codespec/`; the guard goes before that.
- **Tests**: `test/core/root-selection.test.ts` (resolver unit),
  ...
  `test/helpers/codespec-fixtures.ts` (shared fixtures from 3.1).
...
2. `src/core/root-selection.ts`:
   - `CodeSpecRootSource` gains `'declared'`.
   - New `isStoreSelectedRoot(root)` predicate (`storeId !== undefined`);
     `emitStoreRootBanner` and `withStoreFlag` switch to it.
   - In the nearest-root arm: stat `codespec/specs` and
     `codespec/changes` as directories. Planning shape → today's path,
     plus the both-shapes check: a targeted, warning-silent read of
     `codespec/config.{yaml,yml}` (small local helper: read file, YAML
     parse in try/catch, pluck `store`) and one stderr warning when a
     `store` key exists ("codespec/config.yaml declares store 'x', but
     this directory is a real CodeSpec root; the declaration is
     ignored.").
...
- **planning-home untouched**: `findRepoPlanningRootSync` semantics
  stay; only `resolveCodeSpecRoot` classifies the found dir. The
