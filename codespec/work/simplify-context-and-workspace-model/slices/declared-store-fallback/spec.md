...
A repo whose planning is fully externalized — no local CodeSpec root —
...
1. **The declaration lives in `codespec/config.yaml`** — the fallback
   `store:` pointer shares one home with `references:`. The fallback
   case is a **config-only `codespec/` directory** (no `specs/`, no
   ...
   marker file was rejected (`.codespec.yaml` is taken; dot-only
...
1. **Detection mechanics.** The walk is unchanged: nearest ancestor
   carrying `codespec/` wins and terminates the walk. On that one
   directory, two stats (`codespec/specs`, `codespec/changes`, each
   ...
   **no `store:` key → today's behavior is preserved** (the config-only
   ...
   working). The walk never continues past the nearest `codespec/`
...
5. **The both-shapes warning lives in resolution, on stderr** (the
   ...
   CodeSpec root; the declaration is ignored." (implementation
   amendment: the absolute path replaces the spec draft's relative
   `codespec/config.yaml`, per the absolute-paths quality bar) — in
...
6. **The pointer directory is never scaffolded by normal commands; only
   `codespec init` may convert it, deliberately.** No lifecycle command
   creates `specs/` or `changes/` inside a config-only pointer
   directory; work lands in the declared store's root. `codespec init`
   run in a pointer repo **refuses** with an actionable error ("this
   repo's planning is externalized to store 'x' (codespec/config.yaml);
...
```text
$ codespec new change billing-rework
Using CodeSpec root: team-context (/Users/dev/src/team-context)
Created change 'billing-rework' at /Users/dev/src/team-context/codespec/changes/billing-rework/
...
$ codespec status --change billing-rework --json
...
`codespec/specs/`, the same `store:` line changes nothing except one
...
```text
Error: Declared in /Users/dev/src/app-repo/codespec/config.yaml: Unknown store
'team-context'. No stores are registered. Run codespec store setup team-context
or codespec store register <path> first.
```
...
- **Resolver**: in `resolveCodeSpecRoot`
...
- Any change to explicit `--store` behavior, the stores-hint error, or
  the implicit-root scaffold for directories without `codespec/`.
...
- **GIVEN** a repo whose `codespec/` contains only `config.yaml` with
...
- **AND** the banner prints `Using CodeSpec root: team-context (…)`
...
- **GIVEN** a repo with a real root (`codespec/specs/` or
  `codespec/changes/` present)
...
- **GIVEN** a config-only `codespec/` directory whose config has no
  `store:` key
...
- **WHEN** the user runs `codespec init`
...
- **GIVEN** a directory with no `codespec/` anywhere up the walk
