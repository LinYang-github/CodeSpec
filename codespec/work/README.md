...
This directory is experimental. Current CodeSpec CLI validation, archive, and
spec update behavior still centers on `codespec/changes/` and
`codespec/specs/`.

Use `/work` to coordinate and learn. When a slice needs today's executable
CodeSpec lifecycle, project that slice into a normal `codespec/changes/<id>/`
