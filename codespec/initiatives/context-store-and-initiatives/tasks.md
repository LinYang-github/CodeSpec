...
- [x] Decide that the initiative link lives in repo-local `.codespec.yaml`.
...
- [x] Decide not to add `codespec initiative resolve`, now or later.
...
- [ ] Update initiative direction language so context stores are CodeSpec-aware
  shared project roots, not only cross-team/cross-repo coordination folders.
- [ ] Decide the minimal context-store CodeSpec structure:
  `.codespec-store/store.yaml`, `codespec/config.yaml`,
  `codespec/schemas/`, and collection mounts.
...
- [ ] Decide how context-store setup creates, preserves, or repairs
  store-local `codespec/config.yaml`.
...
- [ ] Decide whether schema CLI commands need `--store` or `--store-path`
  selectors.
- [ ] Guard planning-home resolution so context stores with `codespec/config.yaml`
...
- [ ] Define how `codespec new change` selects and reports the artifact home,
