...
initiative artifacts can become executable, target-bound CodeSpec changes.
...
The current initiative direction assumes CodeSpec changes usually live in the
...
Decide whether CodeSpec should support initiative-hosted artifacts that can
...
output should prefer clearer phrases like "plan lives in repo-local CodeSpec",
...
- A targetless initiative artifact is a brief, work item, or proposal, not an
  implementation-ready CodeSpec change.
...
- How does local resolution map a target repo identity to a checkout path,
  CodeSpec root, branch, and allowed edit roots?
...
- How does `codespec new change` report work target, artifact home,
...
- Should archive for an initiative-hosted target-bound change archive centrally,
  ...
  exists?
- Which command and skill surfaces still hardcode `codespec/changes/`, current
...
Do not implement initiative-hosted executable changes until CodeSpec has one
