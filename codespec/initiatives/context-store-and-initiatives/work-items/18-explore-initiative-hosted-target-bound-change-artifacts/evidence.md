...
- Older command surfaces still assume `codespec/changes/` under a local
  CodeSpec project and need an explicit audit before any implementation slice.
...
CodeSpec should keep that separation while making the agent handoff sharper
...
briefs, or proposals, not executable CodeSpec changes, until Item 18 defines a
...
- Use `initiative show` plus workspace/repo context as the agent handoff
  backbone.
- Do not implement context-store-backed CodeSpec changes in Item 8.
...
- The sharper product question is whether initiative-hosted artifacts can
  become executable CodeSpec changes after they are bound to a target repo or
  spec root.
...
```text
Plan lives in: repo-local CodeSpec | initiative context
...
- `src/commands/validate.ts` validates changes and specs from
  `process.cwd()/codespec/...`.
- `src/core/archive.ts` archives by reading `codespec/changes`, applying deltas
  to `codespec/specs`, and moving the change into `codespec/changes/archive`.
...
- Generated skills and workflow templates still contain repo-local path
  assumptions such as `codespec/changes/<name>/`.
