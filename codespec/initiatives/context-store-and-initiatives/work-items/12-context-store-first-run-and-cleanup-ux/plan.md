...
The beta pass found that `codespec context-store setup` feels like a first-run
...
- `codespec context-store setup` with no arguments should start a guided setup
...
- The default location should be the managed CodeSpec context-store directory,
  not the current working directory. Users can still choose any explicit safe
  local path; CodeSpec stores that machine-local path in the local registry, not
...
- Git initialization is an optional local convenience only. When requested,
  CodeSpec may run `git init`, but it must not stage, commit, push, create
...
```text
Store id
Target path, defaulting to the managed CodeSpec context-store location
...
```bash
codespec context-store unregister team-context
codespec context-store remove team-context
```
...
- A fresh user can run `codespec context-store setup` in a terminal and be led
