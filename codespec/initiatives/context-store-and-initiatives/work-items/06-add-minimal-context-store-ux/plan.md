...
- Portable store metadata is exactly `.codespec-store/store.yaml`. It should be
...
```bash
codespec context-store setup <id> [--path <path>] [--init-git|--no-init-git] [--json]
codespec context-store register <path> [--id <id>] [--json]
codespec context-store list [--json]
codespec context-store doctor [id] [--json]
codespec initiative list [--store <id>] [--store-path <path>] [--json]
```
...
- File paths fail.
- `.codespec-store/store.yaml` is written when missing.
...
Metadata: /Users/me/work/team-context/.codespec-store/store.yaml
Registry: /Users/me/.local/share/codespec/context-stores/registry.yaml
Git: initialized
```
...
- If `.codespec-store/store.yaml` exists, use its id.
...
Metadata: /Users/me/src/team-context/.codespec-store/store.yaml
Registry: /Users/me/.local/share/codespec/context-stores/registry.yaml
```
...
Next:
  codespec context-store setup team-context
  codespec context-store register /path/to/context-store
```
...
```text
CodeSpec initiatives (3 across 2 stores)
...
Run: codespec context-store doctor
```
...
```text
CodeSpec initiatives in platform (2)
...
- Added `codespec context-store setup/register/list/doctor`.
