...
- A store can start as a local Git-backed folder. CodeSpec can help create the
...
- Portable `.codespec-store/store.yaml` metadata should be checked in and should
  not include local paths.
- `.codespec-store/store.yaml` is the identity file itself, not a bundle beside
