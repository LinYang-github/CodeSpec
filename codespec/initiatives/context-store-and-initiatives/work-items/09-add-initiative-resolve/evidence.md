...
After review, the standalone `codespec initiative resolve <id>` command should
...
codespec context-store register ./context --id platform
...
- Item 8 stores initiative links in repo-local `.codespec.yaml` as
  `{ store, id }`.
