...
Final decision: do not implement a standalone `codespec initiative resolve`
...
Do not ship `codespec initiative resolve <id>` as a user-facing command in this
...
If a user only has a context store clone, CodeSpec can already resolve the
canonical initiative with:

```bash
codespec initiative show billing-launch --json
```
...
- CodeSpec should not infer repo ownership, scan arbitrary repos, clone repos,
...
- Use `codespec initiative show <id> --json` to locate canonical shared context.
- Use workspace commands to set up, link, relink, list, open, update, and doctor
  local views.
- Use repo-local `codespec new change ... --initiative ...` and
  `codespec set change ... --initiative ...` to create durable links from repo
  work to initiative context.
- Use `codespec status --change <id> --json` inside the owning repo to inspect
...
Decision: do not add `codespec initiative resolve`, now or later. Initiative
