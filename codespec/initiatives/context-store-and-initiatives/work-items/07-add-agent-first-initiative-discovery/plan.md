...
```text
CodeSpec initiative: Billing Launch
...
- Not found: say the initiative was not found in registered context stores and
  suggest `codespec initiative list`.
- Ambiguous: show matching stores and paths, then suggest
  `codespec initiative show <id> --store <store>`.
- Incomplete lookup: say some context stores could not be read, include partial
  matches when present, then suggest `codespec context-store doctor`.
...
- Top-level `codespec show` integration.
