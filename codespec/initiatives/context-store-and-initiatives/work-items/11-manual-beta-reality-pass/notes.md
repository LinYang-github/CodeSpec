...
- After changing the default, rerunning
  `codespec context-store setup team-context --init-git` from inside the
  CodeSpec repo created the store at
  `~/.local/share/codespec/context-stores/team-context` instead of nesting it in
  the repo.
- Minimal fresh-agent handoff worked for initiative creation. A subagent given
  ...
  `~/.local/share/codespec/context-stores/team-context/initiatives/agent-trace-hooks`.
...
- `codespec context-store setup` with no args feels like it should start an
  interactive setup, but it does not. The command name itself creates that
  expectation.
- `codespec context-store setup team-context --init-git` created
  `team-context/` inside the current CodeSpec repo because the default path is
...
- `context-store setup --init-git` initializes Git, but leaves
  `.codespec-store/` and new initiatives untracked. That may be fine, but the
  beta flow does not tell the user or agent whether to stage/commit the shared
  context store.
- `codespec workspace open` with no arguments prompts only for known local
  ...
  like the broad "open something CodeSpec-related" entrypoint.
...
- Need clearer guidance for whether a beta pass should use existing local
  CodeSpec state or create a normal local test context store. Avoid requiring
...
- Better minimal beta path: keep lazy workspace creation, but make bare
  interactive `codespec workspace open` initiative-aware. The picker should show
...
- The first agent step has a bootstrapping problem. `context-store setup` does
  ...
  mini-playbook, installed CodeSpec skills, or CLI output that prints the exact
...
- "Commands only" is product-ambiguous for this beta. The implementation treats
  ...
  likely expect their coding agent to understand CodeSpec concepts, context
  stores, initiatives, and workspace handoff.

## Delivery UX Model

- Split the concept into two layers:
  - Baseline CodeSpec literacy: "Does the agent understand CodeSpec concepts and
...
- In UI copy, avoid "commands only" if it implies no skills at all. Prefer
  ...
  only" with helper text that baseline CodeSpec guidance is still installed
...
- Product distinction:
  ...
  - Repo-local CodeSpec changes are implementation plans owned by the repo that
...
- Agent guidance should not assume repo-local is preferred just because work
  ...
  CodeSpec artifacts outside the repo, when a monorepo has multiple teams with
...
- If a request is ambiguous, the agent should inspect first:
  `codespec initiative list --json`, `codespec list --json`, and workspace
  state when available. If still ambiguous, ask: "Should these CodeSpec
...
- `codespec initiative create` currently creates a full six-file planning
...
- Step 3 says "Open Your Local Workbench," but the command is actually
  ...
  should also warn that bare `codespec workspace open` selects existing
...
- Make `codespec context-store setup` interactive when no id is provided:
...
- Reconsider the default context-store setup path. Options: use the managed
  CodeSpec data directory by default, or keep `./<id>` only after an interactive
  confirmation that names the full target path.
  - Implemented during the pass: use the managed CodeSpec data directory by
    default and keep `--path` for explicit locations.
- Add `codespec context-store unregister <id>` or `remove <id>` for local
...
- Add interactive workspace creation for initiative views. Candidate surfaces:
  extend `codespec workspace setup` with initiative selection, add
  `codespec workspace setup --initiative <store>/<initiative>`, or introduce a
...
- Add an initiative-aware `workspace open` picker as the smallest product fix:
  ...
  - Follow-up fix: when that lazy initiative view is new, `workspace open`
    ...
    user expects it.
- Consider splitting baseline CodeSpec literacy from workflow delivery. A
  small default `use-codespec` skill could be installed whenever a selected
  ...
  agent gets no CodeSpec context."
- Simpler possible slice: treat `use-codespec` as a normal managed skill bundled
...
- Rethink `codespec initiative create` as a sparse, schema-led container
...
- Promoted the broader fix into
  ...
  context stores should behave like CodeSpec roots for shared context, with
...
- Guide correction: after opening a workspace, the user should ask the agent to
  ...
  CodeSpec commands from the owning repo. The user-facing flow should not make
  humans type `codespec new change` or `cd` into implementation repos.
