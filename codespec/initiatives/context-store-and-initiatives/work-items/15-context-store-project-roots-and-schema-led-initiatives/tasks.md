...
- [ ] Record the product decision that context stores should behave like
      CodeSpec roots for config and schema resolution, but not as implementation
      repos by default.
- [ ] Define the context-store root layout, including `.codespec-store/`,
      `codespec/config.yaml`, `codespec/schemas/`, and `initiatives/`.
...
- [ ] Decide whether initiative schemas share `codespec/schemas/` with a
...
- [ ] Guard planning-home resolution so context stores with
      `codespec/config.yaml` do not silently become repo-local implementation
      homes.
