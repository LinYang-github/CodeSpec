...
- `codespec initiative show team-context/agent-trace-hooks --json` and
  `codespec initiative list --store team-context --json` continued to resolve,
...
- Project-local schema resolution already uses
  `<projectRoot>/codespec/schemas/<name>/schema.yaml` in
  `src/core/artifact-graph/resolver.ts`.
- Project config already reads `<projectRoot>/codespec/config.yaml` in
...
- Planning-home detection currently treats an ancestor containing `codespec/`
...
```text
context store = shared CodeSpec context project
...
work. A user may choose a context store simply because they want CodeSpec
...
2. Create `codespec/config.yaml` during context-store setup.
...
- Model a context store as an CodeSpec planning root:
...
- Keep `.codespec-store/store.yaml` as store identity and
  `codespec/config.yaml` as behavior/configuration.
...
- Guard planning-home discovery so a context store with `codespec/config.yaml`
  does not become an accidental implementation repo.

UX/product pass:

- Describe a context store as an CodeSpec-managed planning home. It may be used
  for cross-repo coordination, but also simply to keep CodeSpec artifacts out of
