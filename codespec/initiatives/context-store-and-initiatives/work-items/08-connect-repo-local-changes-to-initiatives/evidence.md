...
The initiative link should live in the repo-local change `.codespec.yaml`.
...
- `createChange()` already writes `.codespec.yaml` for every change.
...
Initiative-linked creation should use `codespec new change` with `--initiative`.

Supported first-slice forms:

```bash
codespec new change add-billing-api --initiative billing-launch --json
codespec new change add-billing-api --initiative platform/billing-launch --json
codespec new change add-billing-api --initiative billing-launch --store platform --json
```
...
```bash
codespec new change add-billing-api --initiative platform/billing-launch
codespec new change add-billing-api --initiative billing-launch --store platform
codespec new change add-billing-api --initiative billing-launch --store-path ./context
```
...
If `codespec new change <id> --initiative ...` runs from a workspace planning
...
```json
{
  "change": {
    "id": "add-billing-api",
    "path": "/repo/codespec/changes/add-billing-api",
    "metadataPath": "/repo/codespec/changes/add-billing-api/.codespec.yaml",
    "schema": "spec-driven"
  },
  "initiative": {
    "store": "platform",
...
```bash
codespec set change add-billing-api --initiative billing-launch --json
codespec set change add-billing-api --initiative platform/billing-launch --json
codespec set change add-billing-api --initiative billing-launch --store platform --json
codespec set change add-billing-api --initiative billing-launch --store-path ../context --json
```
...
file it may mutate is `codespec/changes/<id>/.codespec.yaml`.
...
- `set change` matches the actual side effect: writing validated change metadata
  to `.codespec.yaml`.
- Keeping the command scoped to `.codespec.yaml` avoids creating a broad change
  editing surface.
- `codespec change ...` is currently deprecated, `edit` implies opening an
  editor, and `update` already means refreshing local CodeSpec tooling or
  guidance.
...
- `codespec new change <id> --initiative ...` for repo-local changes, with
  `--json`, `--store`, and `--store-path` support.
- `codespec set change <id> --initiative ...` for existing repo-local changes.
