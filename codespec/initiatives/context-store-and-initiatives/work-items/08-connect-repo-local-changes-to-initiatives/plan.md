...
Let an agent create or link a repo-local CodeSpec change to a shared
...
Decision: Store the initiative link in the repo-local change `.codespec.yaml`.
...
- `.codespec.yaml` is already the per-change machine-readable metadata file.
...
- `codespec new change <id> --initiative ...` creates an initiative-linked
...
- Creating a repo-local change with an initiative link records participation in
  the initiative.
- The link does not claim that CodeSpec verified repo ownership, repo impact, or
...
- `codespec set change <id> --initiative ...` is a validated setter for
  repo-local change metadata.
- In Item 8, the only supported settable field is the initiative link.
- The command only mutates `codespec/changes/<id>/.codespec.yaml`.
...
- The command uses the same initiative lookup semantics as
  `codespec new change <id> --initiative ...`.
...
- Keeping the command limited to `.codespec.yaml` avoids a broad edit surface.
- Avoid `codespec change ...` because that namespace is currently deprecated.
- Avoid `edit` because it implies opening an editor, and avoid `update` because
  CodeSpec already uses update for local guidance/tool refresh.
