...
The GSD workspace comparison highlighted a UX gap: CodeSpec has increasingly
...
CodeSpec literacy. It needs to understand context stores, initiatives,
workspaces, repo-local changes, and where artifacts should live. A small
`use-codespec` skill may be the simplest first slice.
...
```bash
codespec initiative next <id> --json
```
...
produce `/opsx:use-codespec`, should not appear as an implementation workflow,
...
- Should baseline CodeSpec guidance be modeled as a default skill, a profile
