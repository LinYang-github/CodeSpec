...
```bash
codespec workspace open --initiative platform/billing-launch
codespec workspace open --initiative billing-launch --store platform
codespec workspace open --initiative billing-launch
codespec workspace open team-a-billing --initiative platform/billing-launch
```
...
- If no exact match exists, do not silently open the closest match. Show a small
  list of likely matches when available, plus a hint to run `codespec
  initiative list`.
...
Store each private workspace view under the user's CodeSpec global data
...
Item 10. The other files are generated runtime support owned by CodeSpec. They
...
- Generated files are CodeSpec-owned and may be overwritten each time.
...
for tools that need structured facts after CodeSpec has prepared the workspace
...
`.codespec-workspace/workspace.yaml`, `.codespec-workspace/local.yaml`, and a
