...
- Repo-local CodeSpec changes remain the implementation artifacts; initiatives
...
Before adding `codespec initiative create/list`, add a context-store
...
```bash
codespec initiative create <id> --store <store-id> --title <title> --summary <summary>
codespec initiative create <id> --store-path <path> --title <title> --summary <summary>
codespec initiative list --store <store-id>
codespec initiative list --store-path <path>
```

Use `initiative create/list` as a deliberate noun namespace, similar to
`workspace` and `schema`, even though newer CodeSpec conventions generally
...
- Registered `codespec initiative create` and `codespec initiative list` from
  the top-level CLI.
- Added `codespec initiative ls` as an alias for list.
