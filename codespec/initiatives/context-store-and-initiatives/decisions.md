...
`codespec/initiatives/context-store-and-initiatives/` rather than creating an
CodeSpec change for each roadmap item.

Why: The initiative is the durable coordination object for this work. Repo-local
CodeSpec changes should be reserved for implementation slices owned by a repo or
...
- Link repo-local CodeSpec changes back to the initiative later when
...
Decision: Do not use the initial direction lock to rewrite CodeSpec specs.
...
cross-team or cross-repo intent, repo-local CodeSpec changes own implementation
...
proposal"; CodeSpec should provide small CLI primitives the agent can compose.
