...
The user should not need to know every command. CodeSpec should expose small,
...
- create or link a repo-local CodeSpec change
...
of initiative prose. If an agent needs a compact context pack, CodeSpec can
...
```text
CodeSpec stores agreed implementation context.
Jira tracks work.
...
An initiative can span one or many CodeSpec changes.

Those changes may live:

- in the same repo as the initiative
- in different repos
- in multiple context stores or CodeSpec roots later
...
repo and create a linked CodeSpec change.
...
Product/program/architect creates initiative
  ...
  -> repo team creates linked CodeSpec change
...
const store = createStore({
  ...
  backend: gitBackend({
    ...
    localPath: "~/.codespec/stores/acme-context",
    autoSync: true,
  }),
  collections: [
...
- Where should enterprise context stores live by default: customer GitHub,
  CodeSpec-managed Git, or later hosted cloud?
