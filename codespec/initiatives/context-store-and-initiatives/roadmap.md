...
1. `codespec/work/simplify-context-and-workspace-model/goal.md`
2. `codespec/work/simplify-context-and-workspace-model/roadmap.md`
...
- Start from `codespec/work/simplify-context-and-workspace-model/goal.md` and
  `codespec/work/simplify-context-and-workspace-model/roadmap.md` for current
  product authority.
- Use `codespec/initiatives/context-store-and-initiatives/direction.md` as
  historical beta direction, not as the current product authority.
- Treat `codespec/changes/workspace-reimplementation-roadmap/HISTORICAL_DIRECTION.md` and
  `codespec/changes/workspace-reimplementation-roadmap/` as historical reference
...
- CodeSpec can create and manipulate files inside a local context store without
...
Decision: do not add `codespec initiative resolve`, now or later.
...
- `codespec initiative resolve <id>`
...
```bash
codespec initiative next billing-launch --json
```
...
```json
{
  ...
  "suggested_command": "codespec new change add-billing-api --initiative billing-launch"
}
```
...
- Use `codespec workspace open --initiative <initiative>`.
- Support `<store>/<initiative>` and `<initiative> --store <store>`.
- Support `codespec workspace open <workspace-name> --initiative <initiative>`
...
- On no exact match, show likely matches when available and suggest `codespec
...
- How delivery wording should distinguish baseline CodeSpec guidance from
...
Agents run CodeSpec work commands.
```
...
Goal: let context stores behave like CodeSpec roots for shared planning config
...
- A context store can have `codespec/config.yaml` and `codespec/schemas/` like a
...
- Context-store setup that creates or supports store-local CodeSpec config.
...
- Initiative artifact status and instructions output rooted in the initiative
  directory.
- Guardrails so `codespec new change` does not accidentally create executable
...
CodeSpec can upgrade it into a coordinated initiative and carry the current
...
CodeSpec changes only after they are bound to a target repo or spec root,
...
- Audit commands, templates, validation, archive, apply, completion, and docs
  for repo-local `codespec/changes/` assumptions.
