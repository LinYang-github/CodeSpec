# CodeSpec + Superpowers Overall Migration Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the current slug-based code-spec Change workflow with the multi-Change CodeSpec + Superpowers protocol, including stable Requirement identity, lifecycle gates, fresh verification, STALE/Rebase, and transactional archive.

**Architecture:** Add a focused `src/core/codespec-workflow/` domain layer that owns the new YAML/Markdown contracts and lifecycle operations. Existing generic artifact-graph infrastructure remains available for non-code-spec schemas, while the code-spec commands and generated skills route through the new domain layer and reject legacy `.codespec.yaml` Changes. Archive uses a read/validate/prepare/stage/commit pipeline so no current specification is changed until every delta and gate passes.

**Tech Stack:** TypeScript ESM, Node.js >=20.19.0, Zod, YAML, Commander.js, Vitest, pnpm.

**Spec:** `docs/superpowers/specs/2026-09-01-codespec-superpowers-overall-migration-design.md`

## Global Constraints

- This is an intentional breaking migration: old slug-based Changes and `.codespec.yaml` metadata are unsupported by the new code-spec workflow.
- `codespec/business.md` is the stable module registry; `codespec/archive/specs` is the current specification; `codespec/archive/changes` is immutable Change history.
...
Create the new domain layer in `src/core/codespec-workflow/` with one responsibility per file: contracts, loaders, Change management, state gates, Requirement parsing, traceability, verification, stale detection, rebase, and archive transaction. Add focused tests in `test/core/codespec-workflow/`. Update `src/commands/workflow/*`, `src/cli/index.ts`, `schemas/code-spec/*`, generated `skills/codespec-*/SKILL.md`, and user documentation only where the new code-spec workflow requires it. Keep the generic `src/core/artifact-graph/*` implementation unchanged unless a code-spec adapter explicitly needs a small shared hook.
...
**Files:**
- Create: `src/core/codespec-workflow/types.ts`
- Create: `src/core/codespec-workflow/schemas.ts`
- Create: `src/core/codespec-workflow/paths.ts`
- Create: `test/helpers/codespec-workflow.ts`
- Test: `test/core/codespec-workflow/contracts.test.ts`
...
- `getWorkspacePaths(codespecDir, config)` returns absolute paths for `business`, `changes`, `changeIndex`, `archive`, `currentSpecs`, and `archivedChanges`.
- `createWorkflowFixture()` in `test/helpers/codespec-workflow.ts` returns `{ tempDir, codespecDir, paths, workspace, changeId, latestSpecs, metadataAt, cleanup }`; `writeBusinessFile`, `writeCurrentRequirement`, `writeActiveReservation`, `writeDelta`, and `readCurrentRequirement` are exported helpers, and each test registers `cleanup` with `afterEach`.
...
it('rejects a legacy .codespec.yaml-shaped metadata object', () => {
...
Run: `pnpm exec vitest run test/core/codespec-workflow/contracts.test.ts`

Expected: FAIL because the new domain modules do not exist.

- [ ] **Step 3: Implement the minimal schemas and path resolver**

Use Zod object schemas with strict enum validation for state, mode, relation arrays, task statuses, and archive flags. `getWorkspacePaths` must resolve every configured relative path under `codespecDir` and reject absolute paths, `..`, and null bytes.
...
```bash
git add src/core/codespec-workflow/types.ts src/core/codespec-workflow/schemas.ts src/core/codespec-workflow/paths.ts test/core/codespec-workflow/contracts.test.ts
git commit -m "feat: define codespec workflow contracts"
```

### Task 2: Load the workspace, module registry, Change index, and artifacts

**Files:**
- Create: `src/core/codespec-workflow/loaders.ts`
- Create: `src/core/codespec-workflow/business-registry.ts`
- Create: `src/core/codespec-workflow/change-index.ts`
- Create: `src/core/codespec-workflow/artifacts.ts`
...
**Interfaces:**
- `loadWorkspace(codespecDir): Promise<WorkspaceContext>` reads config first and then resolves all configured paths.
...
it('loads configured paths before reading business and change data', async () => {
  const workspace = await loadWorkspace(codespecDir);
  expect(workspace.paths.changeIndex).toBe(path.join(codespecDir, 'changes', 'index.yaml'));
...
Run: `pnpm exec vitest run test/core/codespec-workflow/loaders.test.ts test/core/init.test.ts -t "canonical|module|duplicate"`
...
Read `config.yaml` before resolving any configured path. Parse `business.md` with explicit module-row lookup, preserve IDs on display-name changes, reject duplicate IDs/names, and emit a clear missing-registry error. Reject a directory containing only legacy `.codespec.yaml` Changes as unsupported when loading code-spec Changes.
...
```bash
git add src/core/codespec-workflow/loaders.ts src/core/codespec-workflow/business-registry.ts src/core/codespec-workflow/change-index.ts src/core/codespec-workflow/artifacts.ts src/core/project-config.ts src/core/init.ts test/core/codespec-workflow/loaders.test.ts test/core/init.test.ts
git commit -m "feat: load canonical codespec workspace"
```

### Task 3: Implement Change IDs, creation, deterministic resolution, and resume

**Files:**
- Create: `src/core/codespec-workflow/change-manager.ts`
- Create: `src/core/codespec-workflow/change-resolver.ts`
- Modify: `src/commands/workflow/new-change.ts`
- Modify: `src/commands/workflow/shared.ts`
- Modify: `src/cli/index.ts`
- Test: `test/core/codespec-workflow/change-manager.test.ts`
...
Run: `pnpm exec vitest run test/core/codespec-workflow/change-manager.test.ts test/commands/artifact-workflow.test.ts -t "allocates|guess|Change"`

Expected: FAIL because the canonical ID and resolver are absent.

- [ ] **Step 3: Implement canonical Change management**

Use the local timezone date formatter already used by the CLI. Reject user-supplied slug IDs for code-spec creation, update `changes/index.yaml` as navigation, and write `metadata.yaml` as the only status authority. Replace `--schema` and `.codespec.yaml` creation in the code-spec path; do not add a legacy fallback.
...
```bash
git add src/core/codespec-workflow/change-manager.ts src/core/codespec-workflow/change-resolver.ts src/commands/workflow/new-change.ts src/commands/workflow/shared.ts src/cli/index.ts test/core/codespec-workflow/change-manager.test.ts test/commands/artifact-workflow.test.ts
...
**Files:**
- Create: `src/core/codespec-workflow/state-machine.ts`
- Create: `src/core/codespec-workflow/gates.ts`
- Create: `src/core/codespec-workflow/relations.ts`
...
Run: `pnpm exec vitest run test/core/codespec-workflow/state-machine.test.ts`
...
Run: `pnpm exec vitest run test/core/codespec-workflow/state-machine.test.ts test/commands/artifact-workflow.test.ts`
...
```bash
git add src/core/codespec-workflow/state-machine.ts src/core/codespec-workflow/gates.ts src/core/codespec-workflow/relations.ts src/commands/workflow/status.ts src/commands/workflow/instructions.ts test/core/codespec-workflow/state-machine.test.ts test/commands/artifact-workflow.test.ts
git commit -m "feat: enforce codespec change lifecycle gates"
```

### Task 5: Parse modules, Requirement deltas, scenarios, and traceability

**Files:**
- Create: `src/core/codespec-workflow/module-resolver.ts`
- Create: `src/core/codespec-workflow/requirement-allocator.ts`
- Create: `src/core/codespec-workflow/delta-parser.ts`
- Create: `src/core/codespec-workflow/traceability.ts`
- Modify: `src/core/validation/validator.ts`
- Test: `test/core/codespec-workflow/module-resolver.test.ts`
- Test: `test/core/codespec-workflow/delta-parser.test.ts`
- Test: `test/core/codespec-workflow/traceability.test.ts`
...
Run: `pnpm exec vitest run test/core/codespec-workflow/module-resolver.test.ts test/core/codespec-workflow/delta-parser.test.ts test/core/codespec-workflow/traceability.test.ts`
...
```bash
git add src/core/codespec-workflow/module-resolver.ts src/core/codespec-workflow/requirement-allocator.ts src/core/codespec-workflow/delta-parser.ts src/core/codespec-workflow/traceability.ts src/core/validation/validator.ts test/core/codespec-workflow/module-resolver.test.ts test/core/codespec-workflow/delta-parser.test.ts test/core/codespec-workflow/traceability.test.ts
...
**Files:**
- Create: `src/core/codespec-workflow/verification.ts`
- Create: `src/core/codespec-workflow/baseline.ts`
- Create: `src/core/codespec-workflow/stale.ts`
- Create: `src/core/codespec-workflow/rebase.ts`
- Modify: `src/commands/workflow/instructions.ts`
- Modify: `src/cli/index.ts`
- Test: `test/core/codespec-workflow/verification.test.ts`
...
Run: `pnpm exec vitest run test/core/codespec-workflow/verification.test.ts test/core/codespec-workflow/stale-rebase.test.ts`
...
```bash
git add src/core/codespec-workflow/verification.ts src/core/codespec-workflow/baseline.ts src/core/codespec-workflow/stale.ts src/core/codespec-workflow/rebase.ts src/commands/workflow/instructions.ts src/cli/index.ts test/core/codespec-workflow/verification.test.ts test/core/codespec-workflow/stale-rebase.test.ts
...
**Files:**
- Create: `src/core/codespec-workflow/archive-transaction.ts`
...
Run: `pnpm exec vitest run test/core/codespec-workflow/archive-transaction.test.ts`
...
Run: `pnpm exec vitest run test/core/codespec-workflow/archive-transaction.test.ts test/core/archive.test.ts`
...
```bash
git add src/core/codespec-workflow/archive-transaction.ts src/core/archive.ts src/cli/index.ts src/core/business-archive.ts test/core/codespec-workflow/archive-transaction.test.ts test/core/archive.test.ts
git commit -m "feat: archive changes transactionally"
```

### Task 8: Route CLI actions and generated skills through codespec-workflow

**Files:**
- Create: `src/core/templates/workflows/codespec-workflow.ts`
...
- Generate: `skills/codespec-workflow/SKILL.md` through `pnpm generate:skills`

**Interfaces:**
- The generated workflow guidance names `codespec-workflow` as the adapter and routes artifacts to the bound `CHG-*` directory.
...
it('routes CodeSpec code-spec work through codespec-workflow', () => {
  const content = getCodespecProposeSkillTemplate().instructions;
...
Keep Superpowers skill names and rules intact. Add only routing/context instructions for `brainstorming`, `writing-plans`, TDD, debugging, fresh verification, review, and branch finishing. Add the new skill to the generator's canonical list and ensure the generated distribution is the only source of committed `skills/codespec-*/SKILL.md` content.

- [ ] **Step 4: Generate skills and verify parity**

Run: `pnpm build && pnpm generate:skills && pnpm run regen:parity-hashes && pnpm exec vitest run test/core/templates/codespec-workflow.test.ts test/core/shared/skill-content-equivalence.test.ts`
...
```bash
git add skills src/core/templates src/core/config.ts src/commands/workflow/instructions.ts test/core/templates/codespec-workflow.test.ts test/core/shared/skill-content-equivalence.test.ts
git commit -m "feat: route generated skills through codespec workflow"
```
...
- Modify: `codespec/config.yaml`
- Create: `codespec/business.md`
- Create: `codespec/changes/index.yaml`
- Create: `codespec/archive/README.md`
- Modify: `docs/overview.md`
- Modify: `docs/workflows.md`
- Modify: `docs/concepts.md`
- Modify: `docs/cli.md`
- Test: `test/core/codespec-workflow/templates.test.ts`

**Interfaces:**
- `code-spec` declares the canonical artifact files and no `.codespec.yaml`, `skip_specs`, `spec-driven`, `codespec/specs`, or `changes/archive` fallback.
...
Run: `pnpm exec vitest run test/core/codespec-workflow/templates.test.ts`
...
Run: `pnpm exec vitest run test/core/codespec-workflow/templates.test.ts test/core/artifact-graph/schema.test.ts test/commands/schema.test.ts`
...
```bash
git add schemas/code-spec codespec/config.yaml codespec/business.md codespec/changes/index.yaml codespec/archive/README.md docs/overview.md docs/workflows.md docs/concepts.md docs/cli.md test/core/codespec-workflow/templates.test.ts
git commit -m "feat: migrate code-spec documents to codespec protocol"
```
...
- Modify: `src/core/codespec-root.ts`
...
- Legacy `.codespec.yaml` or slug Changes fail with a diagnostic naming the canonical replacement command.
...
Run: `pnpm exec vitest run test/cli-e2e/codespec-workflow-journeys.test.ts test/core/codespec-workflow/legacy-rejection.test.ts`

Expected: FAIL against the old slug-based CLI behavior.

- [ ] **Step 3: Remove code-spec legacy fallbacks and update fixtures**

Delete `.codespec.yaml` resolution from code-spec paths, remove old `spec-driven` defaulting and `skip_specs` behavior from the canonical path, and update tests to create `metadata.yaml`/`CHG-*` workspaces. Leave generic non-code-spec schema support only where its tests explicitly require it.

- [ ] **Step 4: Run focused and full verification**

Run: `pnpm lint && pnpm exec vitest run test/core/codespec-workflow test/cli-e2e/codespec-workflow-journeys.test.ts test/commands/artifact-workflow.test.ts`
...
```bash
git add src test
git commit -m "test: verify complete codespec workflow migration"
```
...
Record the new canonical directory layout, `CHG-*` creation command, explicit archive requirement, and the unsupported `.codespec.yaml` behavior in `README.md` and `CHANGELOG.md`.
...
git commit -m "docs: document codespec workflow migration"
```
