...
**Goal:** 将 CodeSpec 根目录 `skills/` 收敛为三个公开入口：`codespec-workflow`、`codespec-rebase-change` 和 `codespec-archive-change`，并把 Change、Requirement、Baseline、STALE、Traceability、Canonical Spec 与归档事务等能力统一留在 CodeSpec Core；工程方法继续交由 Superpowers 承担。

**Architecture:** 保留 `codespec-*` 公开命名，采用“方案 A 的迁移路径 + 已确认的三 Skill 最终结构”。生成器使用独立的 public workflow surface；旧 workflow id 只用于迁移和清理，不再用于生成新的 Skill 或 AI command。三个 Skill 负责意图识别、入口路由和状态门禁，Core 负责所有领域状态变更与事务，Superpowers 负责 brainstorming、plan、TDD、debug、verify、review 和分支收尾。

**Tech Stack:** TypeScript、Vitest、pnpm、现有 `src/core/templates` Skill 生成器、现有 `src/core/codespec-workflow` Core 模块、Markdown Skill 模板。

**Spec:** `docs/superpowers/specs/2026-09-02-code-spec-three-skills-architecture-design.md`

## Global Constraints

- 根目录最终只生成三个 CodeSpec Skill 目录：`skills/codespec-workflow`、`skills/codespec-rebase-change`、`skills/codespec-archive-change`。
- `codespec-workflow` 是正常开发唯一入口；不得再生成或推荐 `new`、`propose`、`explore`、`continue`、`apply`、`update`、`ff`、`verify`、`onboard` 等独立入口。
- `codespec-rebase-change` 是 STALE、多 Change 冲突或基线需要重建时的唯一入口；不得把 `detect-stale`、`resolveChange`、`captureBaseline` 等能力暴露为独立 Skill。
- `codespec-archive-change` 是 Current Specification 提交的唯一入口；不得把 `sync-specs` 或 `bulk-archive-change` 作为并列 Skill 继续生成。
- 公开 Skill 不直接修改 Current Specification；所有领域写入必须经过 CodeSpec Core 的现有事务 API 或新增的内部适配层。
...
- [x] 将 `config.ts` 的 `CODESPEC_SKILL_NAMES` 改为三个公开 Skill 名称；单独提供 legacy skill 名称集合供迁移和清理使用，避免清理逻辑依赖公开集合。
...
- [x] 将 `getSkillTemplates()` 改为只注册 `codespec-workflow`、`codespec-rebase-change` 和 `codespec-archive-change`。
...
## 3. 把 `codespec-workflow` 扩展为唯一开发编排入口

**Files:**

- Modify: `src/core/templates/workflows/codespec-workflow.ts`
- Modify: `src/core/templates/skill-templates.ts`
- Tests: `test/core/templates/codespec-workflow.test.ts`, `test/core/shared/skill-generation.test.ts`
...
- [x] 明确在进入实现前检查 Change、模块、基线和状态；发现 STALE 或不可安全合并时只路由到 `codespec-rebase-change`。
- [x] 明确 workflow 不直接写 Current Specification；完成实现后只能把归档意图交给 `codespec-archive-change`。
...
```bash
pnpm vitest run test/core/templates/codespec-workflow.test.ts
```

测试必须覆盖：单入口路由、Superpowers 委派、STALE 转交、Current Specification 写入边界、Core 内部函数不作为 Skill 名称出现。

## 4. 新增 `codespec-rebase-change` Skill 入口
...
- Modify: `src/core/templates/skill-templates.ts`
- Modify: `src/core/shared/skill-generation.ts`
- Tests: `test/core/templates/rebase-change.test.ts`, `test/core/codespec-workflow/stale-rebase.test.ts`, `test/cli-e2e/codespec-workflow-journeys.test.ts`
...
- [x] 明确 rebase 成功后的状态契约：Change 回到设计阶段，revision 增加，基线、tasks、verification 和 archive 标记按现有 Core 实现重置，随后回到 `codespec-workflow`。
...
```bash
pnpm vitest run test/core/templates/rebase-change.test.ts test/core/codespec-workflow/stale-rebase.test.ts test/cli-e2e/codespec-workflow-journeys.test.ts
```

## 5. 将 `codespec-archive-change` 固化为唯一归档事务入口

**Files:**

- Modify: `src/core/templates/workflows/archive-change.ts`
- Modify: `src/core/templates/workflows/codespec-workflow.ts`
- Review/modify only where required: `src/core/codespec-workflow/archive-transaction.ts`
- Tests: `test/core/archive.test.ts`, `test/core/codespec-workflow/archive-transaction.test.ts` if present, `test/cli-e2e/codespec-workflow-journeys.test.ts`

- [x] 将 archive Skill 改为 canonical CodeSpec 路径和 Current Specification 写入协议，移除对 `codespec-sync-specs` 等旧 Skill 的依赖或推荐。
...
- [x] 处理 legacy `codespec/specs` 测试/调用路径与 canonical `archive/specs` 路径的兼容关系：新 Skill 只输出 canonical 路径，旧 CLI 行为按现有兼容约束保留。

**Verification:**

```bash
pnpm vitest run test/core/archive.test.ts test/cli-e2e/codespec-workflow-journeys.test.ts
```
...
- [x] 更新任何仍声称 `codespec-sync-specs`、`codespec-apply-change` 或其他旧 Skill 是必需入口的文档链接和示例。
- [x] 不把尚未实现的完整 SDD policy 写成现有用户承诺；只说明 SDD 级别由 Core 决定并被 workflow 消费。

**Verification:**

```bash
rg -n "codespec-(apply-change|bulk-archive-change|continue-change|explore|ff-change|new-change|onboard|propose|sync-specs|update-change|verify-change)" skills README.md docs src --glob '!docs/superpowers/specs/**'
```
...
- Generated: `skills/codespec-workflow/SKILL.md`
- Generated: `skills/codespec-rebase-change/SKILL.md`
- Generated: `skills/codespec-archive-change/SKILL.md`
