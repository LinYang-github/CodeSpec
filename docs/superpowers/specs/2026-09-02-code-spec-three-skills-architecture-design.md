...
CodeSpec 的用户可见 CodeSpec Skill 收敛为三个：
...
- `codespec-workflow`：正常开发唯一入口；
- `codespec-rebase-change`：STALE Change 和多 Change 冲突后的恢复入口；
- `codespec-archive-change`：Current Specification 更新和 Change 归档唯一入口。

其余 CodeSpec 能力不再作为独立 Skill 暴露。领域治理由 CodeSpec Core 负责，工程方法由现有 Superpowers Skill 负责。`code-spec` 是唯一默认协议，`codespec` 保留为 CLI、包和 Skill 前缀。
...
4. `spec-driven`、slug Change、`.codespec.yaml` 和 `codespec/specs/` 等历史模型仍出现在部分 Skill 中。
5. Superpowers 方法论已经存在，CodeSpec Skill 不需要复制 brainstorming、planning、TDD、debugging 或 review 流程。
...
codespec-rebase-change
    │
    └── CodeSpec Core: semantic rebase → DESIGN
...
codespec-archive-change
    │
    └── CodeSpec Core: validate → conflict check → apply delta → archive transaction
```

### 5.1 CodeSpec Skill 层
...
Superpowers 负责如何分析和实施工程工作：理解问题、选择设计、编写计划、执行 TDD、调试、验证和代码审查。CodeSpec 将 canonical 上下文注入这些方法论，但不复制或改写其核心流程。

## 6. 三个 Skill 的职责

### 6.1 `codespec-workflow`
...
`codespec-workflow` 不直接修改 Current Specification，也不绕过 archive 入口。

### 6.2 `codespec-rebase-change`
...
Rebase 后不得自动重新实现、自动验证或自动归档。用户需要通过 `codespec-workflow` 继续处理。

### 6.3 `codespec-archive-change`
...
- `codespec/changes/CHG-YYYYMMDD-NNN/metadata.yaml`。

不得回退到 slug Change 或旧 `.codespec.yaml`。
...
`codespec-workflow` 负责正常状态推进，`codespec-rebase-change` 负责 STALE 恢复，`codespec-archive-change` 负责最终状态提交。Skill 不得直接修改状态字段绕过 Core。
...
```text
src/core/templates/workflows/codespec-workflow.ts
...
`codespec-rebase-change` 需要新增对应模板、生成注册和必要的 parity 测试。

### 9.2 生成集合

`getSkillTemplates()` 最终只生成三个 CodeSpec Skill。旧的 `new`、`propose`、`continue`、`ff`、`apply`、`verify`、`onboard`、`update`、`sync` 和 `bulk-archive` 不再作为独立 Skill 输出。
...
- 找不到唯一 Change 时，`codespec-workflow` 必须要求用户选择，不得猜测。
...
- 生成集合只有三个 CodeSpec Skill；
- 生成结果与模板保持 parity；
- 旧 Skill 目录不会残留；
- 保留的 Skill 不含 `spec-driven`、slug、`.codespec.yaml` 或旧 `codespec/specs/` 引用。
...
1. 通过 `codespec-workflow` 创建并完成一个 Change；
2. 两个 Change 互不影响时正常归档；
3. 归档后相关 Change 被标记 STALE；
4. 通过 `codespec-rebase-change` 恢复 STALE Change；
5. 通过 `codespec-archive-change` 更新 Current Specification 并完成归档；
6. 归档冲突或验证失败时没有部分写入；
...
1. 将 `codespec-workflow.ts` 定义为三 Skill 共用的 canonical 协议和上下文契约。
2. 在 Core 中确认 Change、Baseline、Validation、Rebase 和 Archive 的服务边界。
3. 新增 `rebase-change` Skill 模板和生成入口。
4. 将 `codespec-workflow` 扩展为创建、规划、实现和验证的完整编排入口。
...
- `skills/` 只包含 `codespec-workflow`、`codespec-rebase-change` 和 `codespec-archive-change`；
