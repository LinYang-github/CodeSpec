...
CodeSpec exposes three AI entries. Use them in your assistant's chat.

| Entry | When to use it |
|---|---|
| `codespec-workflow` | Every normal development request |
| `codespec-rebase-change` | Core reports STALE, baseline drift, or a multi-Change conflict |
| `codespec-archive-change` | The Change is implemented and verified |
...
```text
/codespec:workflow add-dark-mode
```
