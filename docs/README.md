...
> **还不确定要构建什么时，先运行 `/codespec:workflow`。**它会把 brainstorming 和 planning 交给 Superpowers，再进入 CodeSpec Change 流程。

## 选择入口

**第一次使用：**从[快速入门](getting-started.md)开始，再阅读[核心概念](overview.md)。遇到术语或疑问时，查看[FAQ](faq.md)和[术语表](glossary.md)。

**有问题但没有计划：**使用 `/codespec:workflow`，让 Superpowers 在同一个开发入口内完成 brainstorming 和 planning。

**I have a big existing codebase.** You don't document all of it. [Using CodeSpec in an Existing Project](existing-projects.md) shows how to start on real, brownfield code without boiling the ocean.

**只想先运行起来：**阅读[安装](installation.md)，运行 `codespec init`，再读[命令如何工作](how-commands-work.md)。也可以使用[AI 辅助安装提示词](installation.md#install-with-your-ai-assistant)。
...
**I work on a team.** [CodeSpec on a Team](team-workflow.md) shows how a change maps onto a branch and a pull request, and how teammates review a plan before the code.
...
| [CodeSpec on a Team](team-workflow.md) | How changes fit branches, pull requests, and review |
| [Using CodeSpec in an Existing Project](existing-projects.md) | Adopting CodeSpec on a large brownfield codebase |
| [Editing & Iterating on a Change](editing-changes.md) | Update artifacts, go back, reconcile manual edits |
| [Commands](commands.md) | Reference for every `/codespec:*` slash command |
| [CLI](cli.md) | Reference for every `codespec` terminal command |
...
| [CodeSpec Workflow](codespec.md) | Why the workflow is fluid instead of phase-locked, plus an architecture deep dive |
...
| [Supported Tools](supported-tools.md) | The 30+ AI tools CodeSpec integrates with, and where files land |
...
| [Migration Guide](migration-guide.md) | Moving from the legacy workflow to CodeSpec |
...
```text
1. Install        npm install -g @hrhy-ai/codespec@latest
2. Initialize     cd your-project && codespec init
3. Develop        (in your AI chat)  /codespec:workflow add-dark-mode
4. Recover        (in your AI chat)  /codespec:rebase              ← only when STALE
5. Archive        (in your AI chat)  /codespec:archive
```
...
- **GitHub Issues:** [github.com/LinYang-github/CodeSpec/issues](https://github.com/LinYang-github/CodeSpec/issues) for bugs and feature requests.
- **`codespec feedback "your message"`** sends feedback straight from your terminal (it opens a GitHub issue).
