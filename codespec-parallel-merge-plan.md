# CodeSpec Parallel Delta Remediation Plan
...
4. **Document interim manual mitigation.**
   - Update `codespec/AGENTS.md` and docs so contributors know to rerun `codespec change sync` (see Phase 1) whenever another change lands.
...
1. **Introduce `codespec change sync <id>` (or `rebase`).**
...
2. **Enrich validator messages.**
   - `codespec validate` should flag unresolved conflict markers or fingerprint mismatches so errors appear early in the workflow.
...
3. **Integrate with Git directly.**
   - Offer optional `codespec branch` scaffolding that aligns spec changes with Git branches, letting teams leverage Git’s conflict editor for the markdown IR.

_Outcome:_ CodeSpec graduates from replace-based updates to a resilient, intent-preserving spec management platform.
...
1. Prototype fingerprint capture during `codespec change validate` and block archive on mismatches.
2. Ship `codespec change sync` with line-based diff3 merging and conflict markers.
