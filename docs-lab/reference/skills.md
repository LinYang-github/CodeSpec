...
> Every CodeSpec skill: arguments, what it creates, and what it responds with.
...
legacy /codespec:* commands (help/legacy/migration.md). Source: old docs/commands.md
...
| Skill | Job | Type |
|---|---|---|
| [codespec-explore](#codespec-explore) | Think through an idea before it becomes a change proposal | Core |
| [codespec-propose](#codespec-propose) | Create a change proposal with all its planning artifacts in one step | Core |
| [codespec-apply-change](#codespec-apply-change) | Implement a change proposal's tasks | Core |
| [codespec-update-change](#codespec-update-change) | Revise a change proposal's plan | Core |
| [codespec-sync-specs](#codespec-sync-specs) | Merge a change proposal's spec updates into `specs/` | Core |
| [codespec-archive-change](#codespec-archive-change) | Move a finished change proposal to the archive | Core |
| [codespec-new-change](#codespec-new-change) | Start a change proposal as an empty scaffold | Optional |
| [codespec-continue-change](#codespec-continue-change) | Create the next planning artifact, one at a time | Optional |
| [codespec-ff-change](#codespec-ff-change) | Create a change proposal with every artifact implementation needs, in one pass | Optional |
| [codespec-verify-change](#codespec-verify-change) | Check the implementation matches the plan | Optional |
| [codespec-bulk-archive-change](#codespec-bulk-archive-change) | Archive several change proposals at once | Optional |
| [codespec-onboard](#codespec-onboard) | Learn the workflow by doing one real change proposal end to end | Optional |
...
| **Creates** | Nothing by default. It reads and investigates only. On request it captures insights: a new change proposal under `codespec/changes/<name>/`, or updates to an existing one's proposal, design, specs, or tasks. Never code. |
...
| **Creates** | `codespec/changes/<name>/` with every artifact the schema defines, in dependency order (spec-driven: proposal, spec deltas, design, tasks). Never code. |
| **Response** | The created artifacts, ready for review, and the next step. Stops there; implementation waits for `codespec-apply-change`. |
...
| **Response** | Progress per task, then an overall count (N/M tasks complete). All done: suggests `codespec-archive-change`. Blocked by missing artifacts: points to `codespec-continue-change`. Unclear tasks or errors: pauses and asks. |
...
| **Creates** | Nothing new. Edits only artifact files that already exist. Missing artifacts are `codespec-continue-change`'s job. Never code. |
| **Response** | Shows each proposed revision and writes it only after you confirm, one artifact at a time. Ends with what was revised and the next step; implementation waits for `codespec-apply-change`. |
...
| **Creates** | Edits or creates `codespec/specs/<capability-path>/spec.md` for each delta spec, merging added, modified, removed, and renamed requirements into the main spec. Never code. |
| **Response** | A per-capability summary of requirements added, modified, removed, or renamed, after the updated specs validate. The change proposal stays active; archiving waits for `codespec-archive-change`. |
...
| **Creates** | Moves the change proposal folder to `codespec/changes/archive/YYYY-MM-DD-<name>/` (no date added if the name already starts with one). With your approval it first syncs outstanding delta specs via `codespec-sync-specs`. Never code. |
...
| **Creates** | `codespec/changes/<name>/` as an empty scaffold: no artifacts yet, never code. |
| **Response** | The scaffold's name and location, the workflow's artifact sequence, status (0/N complete), and the first artifact's template. Drafting artifacts waits for `codespec-continue-change`. |
...
| **Response** | The created artifact, progress (N of M complete), and which artifacts that unlocked. When planning is complete it says so; implementation moves to `codespec-apply-change`. |
...
| **Creates** | `codespec/changes/<name>/` and every planning artifact implementation requires, in dependency order (spec-driven: proposal, specs, design, tasks), leaving out only artifacts marked skipped or conditional. Never code. |
| **Response** | The change proposal's name and location, each artifact created, and any conditional artifact skipped and why. Stops there; implementation waits for `codespec-apply-change`. |
...
| **Creates** | `codespec/changes/archive/YYYY-MM-DD-<name>/` per archived change proposal (already-dated names keep their prefix). Each one's spec deltas sync first via `codespec-sync-specs`. Never code. |
...
| **Response** | A narrated walkthrough of the full cycle with pauses for your input: explore, create, build each artifact, implement, archive. Ends with a recap and a pointer to `codespec-propose`. Takes about 15 to 20 minutes. |
