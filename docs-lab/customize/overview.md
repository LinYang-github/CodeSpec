...
> Your options for customizing CodeSpec.

CodeSpec supports multiple customization options. This page shows what each one changes and when to use it.
...
| [Schemas](schemas.md) | What CodeSpec produces: the artifacts, their order, and their templates | Changes should produce different planning files, sections, or formats |
...
*"Fork" here means the `codespec schema fork` command, not forking a git repo. [Schemas](schemas.md) has the details.*
...
flowchart LR
    ...
    schema["Fork a schema<br/>(codespec schema fork)"]
```
