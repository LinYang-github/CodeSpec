...
/**
 * Tools whose command files live in an `codespec/` directory, so the tool
 * namespaces the command and registers `/codespec:<id>`. Every other registered
 * adapter writes `codespec-<id>` as the filename and therefore registers
 * `/codespec-<id>`.
 *
...
const sampleContent: CommandContent = {
  ...
  category: 'Workflow',
  tags: ['codespec'],
  body: 'Run /codespec:archive when done. See /codespec:continue for the next artifact.',
};

describe('command-generation/invocation', () => {
  describe('getInvocationStyleForPath', () => {
    it('classifies an codespec- prefixed filename as flat', () => {
      expect(getInvocationStyleForPath(path.join('.cursor', 'commands', 'codespec-apply.md'))).toBe('flat');
      expect(getInvocationStyleForPath(path.join('.github', 'prompts', 'codespec-apply.prompt.md'))).toBe('flat');
    });

    it('classifies a file inside an codespec/ directory as namespaced', () => {
      expect(getInvocationStyleForPath(path.join('.claude', 'commands', 'codespec', 'apply.md'))).toBe('namespaced');
      expect(getInvocationStyleForPath(path.join('.gemini', 'commands', 'codespec', 'apply.toml'))).toBe('namespaced');
    });
  });
  ...
  describe('formatCommandInvocation', () => {
    it('spells each shape the way the tool registers it', () => {
      expect(formatCommandInvocation({ style: 'namespaced', prefix: '/' }, 'apply')).toBe('/codespec:apply');
      expect(formatCommandInvocation({ style: 'flat', prefix: '/' }, 'apply')).toBe('/codespec-apply');
      expect(formatCommandInvocation({ style: 'flat', prefix: '@' }, 'bulk-archive')).toBe(
        '@codespec-bulk-archive'
      );
    });
  ...
  describe('generateCommand', () => {
    it('rewrites command references to the names a flat tool registers', () => {
      for (const toolId of ['cursor', 'github-copilot', 'devin', 'opencode', 'qwen']) {
        ...
        expect(fileContent, toolId).toContain('/codespec-archive');
        expect(fileContent, toolId).toContain('/codespec-continue');
        expect(fileContent, toolId).not.toContain('/codespec:');
      }
    });

    it("writes Amazon Q's prompt-library form, not a slash command", () => {
      // .amazonq/prompts/codespec-<id>.md is a prompt, invoked with @ — a body
      // telling the user to type /codespec-archive names nothing Amazon Q registers.
      const adapter = CommandAdapterRegistry.get('amazon-q')!;
      const { fileContent } = generateCommand(sampleContent, adapter);
      expect(fileContent).toContain('@codespec-archive');
      expect(fileContent).toContain('@codespec-continue');
      expect(fileContent).not.toContain('/codespec-');
      expect(fileContent).not.toContain('/codespec:');
    });
    ...
    it('leaves command references alone for namespaced tools', () => {
      for (const toolId of NAMESPACED_TOOLS) {
        ...
        const { fileContent } = generateCommand(sampleContent, adapter);
        expect(fileContent, toolId).toContain('/codespec:archive');
        expect(fileContent, toolId).not.toContain('/codespec-archive');
      }
    });

    it('rewrites nothing but the command references', () => {
      const adapter = CommandAdapterRegistry.get('cursor')!;
      const plain = { ...sampleContent, body: 'Plain body. See docs/codespec.md and codespec/changes/.' };
      const { fileContent } = generateCommand(plain, adapter);
      expect(fileContent).toContain('Plain body. See docs/codespec.md and codespec/changes/.');
    });
    ...
    it('leaves the adapters themselves as pure formatters', () => {
      ...
      for (const toolId of ['bob', 'oh-my-pi', 'opencode', 'pi', 'qwen', 'cursor', 'devin']) {
        const adapter = CommandAdapterRegistry.get(toolId)!;
        expect(adapter.formatFile(sampleContent), toolId).toContain('/codespec:archive');
      }
    });
  });
});
