...
describe('CodeSpec UI web assets', () => {
  it('uses CodeSpec paths and branding without OpenSpec runtime remnants', async () => {
    ...
    expect(app).toContain('renderBusinessOverview');
    expect(app).toContain('renderBusinessTable');
    expect(app).toContain('renderBusinessCanvas');
    expect(app).not.toContain('BUSINESS MANAGEMENT');
    expect(app).not.toContain('业务总览');
    expect(app).toContain('module-document-tab');
    ...
    expect(app).toContain('document-tab-copy');
    expect(app).not.toContain('/api/reveal/');
    ...
    expect(styles).toContain('.workspace { min-width: 0; min-height: 0; flex: 1 1 auto; overflow: auto;');
