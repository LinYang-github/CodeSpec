...
describe('BashInstaller', () => {
  ...
  beforeEach(async () => {
    ...
    testHomeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'codespec-bash-test-'));
  ...
  describe('install', () => {
    ...
    it('should handle paths with spaces in .bashrc config', async () => {
      ...
      const testHomeDirWithSpaces = await fs.mkdtemp(path.join(os.tmpdir(), 'codespec bash test '));
  ...
  describe('removeBashrcConfig', () => {
    ...
    it('should remove markers and config when present', async () => {
      ...
      expect(newContent).not.toContain('CodeSpec shell completions configuration');
