...
import type { ArchivePlan, ChangeId } from '../../../src/core/codespec-workflow/types.js';
...
} from '../../../src/core/codespec-workflow/schemas.js';
...
describe('codespec workflow contracts', () => {
  it('records a level and affected areas in canonical metadata', () => {
    const metadata = parseChangeMetadata({
      ...validMetadata(),
      change: { ...validMetadata().change, sdd_level: 3 },
      impact: { ...validMetadata().impact, affected_areas: ['auth/login'] },
    });

    expect(metadata.change.sdd_level).toBe(3);
  ...
  it('rejects unsafe configured paths', async () => {
    ...
    expect(() => getWorkspacePaths('/tmp/project/codespec', config)).toThrow(/\.\.|relative|path/i);
  });
