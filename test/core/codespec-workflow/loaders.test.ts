...
describe('codespec workflow loaders', () => {
  ...
  it('loads a generic spec-driven workspace using its configured paths', async () => {
    ...
    expect(workspace.paths.currentSpecs).toBe(path.join(fixture.codespecDir, 'generic', 'specs'));
  ...
  it('loads canonical configured paths before reading business and change data', async () => {
    ...
    expect(workspace.paths.changeIndex).toBe(path.join(fixture.codespecDir, 'nav', 'index.yaml'));
  ...
  it('requires config.yaml for canonical workspace loading', async () => {
    ...
    await fs.rename(
      path.join(fixture.codespecDir, 'config.yaml'),
      path.join(fixture.codespecDir, 'config.yml')
    );

    await expect(loadWorkspace(fixture.codespecDir)).rejects.toThrow(/config\.yaml/i);
  });
