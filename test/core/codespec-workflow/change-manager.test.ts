...
import {
  ...
  resumeChange,
} from '../../../src/core/codespec-workflow/change-manager.js';
...
describe('codespec workflow change management', () => {
  ...
  it('keeps Level 1 design inline in spec.md and does not create design.md', async () => {
    ...
    const created = await createCanonicalChange(workspace, {
      ...
      summary: 'A focused Level 1 change',
      mode: 'bugfix',
      sddLevel: 1,
    });
  ...
  it('scaffolds the extra Level 3 design sections', async () => {
    ...
    const created = await createCanonicalChange(workspace, {
      ...
      summary: 'Move the authentication boundary',
      mode: 'feature',
      sddLevel: 3,
    });

    const design = await fs.readFile(path.join(fixture.paths.changes, created.changeId, 'design.md'), 'utf8');
    for (const section of ['SDD 分级依据', '架构', '接口契约', '迁移', '回滚', '发布']) {
