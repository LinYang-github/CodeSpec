...
describe('canonical code-spec legacy rejection', () => {
  it('rejects a slug Change with OpenSpec metadata and names the canonical command', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codespec-legacy-'));
    const changeDir = path.join(root, 'codespec', 'changes', 'add-login');
    fs.mkdirSync(changeDir, { recursive: true });
    fs.writeFileSync(path.join(root, 'codespec', 'config.yaml'), 'schema: code-spec\n');
    fs.writeFileSync(path.join(changeDir, '.openspec.yaml'), 'schema: spec-driven\n');

    expect(() => readChangeMetadata(changeDir, root)).toThrow(/CHG-|codespec new change/i);
  });

  it('does not resolve a canonical Change through the old spec-driven fallback', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codespec-canonical-'));
    const changeDir = path.join(root, 'codespec', 'changes', 'CHG-20260901-001');
    fs.mkdirSync(changeDir, { recursive: true });
    fs.writeFileSync(path.join(changeDir, '.openspec.yaml'), 'schema: spec-driven\n');
  ...
  it('never writes legacy metadata for a canonical code-spec workspace', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codespec-write-'));
    const changeDir = path.join(root, 'codespec', 'changes', 'CHG-20260901-001');
    fs.mkdirSync(changeDir, { recursive: true });
  ...
  it('fails explicitly for a present invalid canonical config', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'codespec-invalid-'));
    fs.mkdirSync(path.join(root, 'codespec'), { recursive: true });
    fs.writeFileSync(path.join(root, 'codespec', 'config.yaml'), 'version: 1\npaths:\n  changes: [\nschema: code-spec\n');
