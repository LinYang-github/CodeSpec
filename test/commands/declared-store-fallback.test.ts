...
describe('declared store fallback (3.2)', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.realpathSync.native(
      fs.mkdtempSync(path.join(os.tmpdir(), 'codespec-declared-'))
    );
    env = {
    ...
    fs.writeFileSync(
      path.join(pointerRepo, 'codespec', 'config.yaml'),
      'store: team-context\n'
    );
  });
  ...
  it('runs the externalized-planning journey without --store anywhere', async () => {
    ...
    expect(statusHuman.stderr).toContain('使用 CodeSpec 根目录：team-context');
    ...
    const changeDir = path.join(storeRoot, 'codespec', 'changes', 'billing-rework');
    fs.writeFileSync(
    ...
    expect(
      fs.existsSync(path.join(storeRoot, 'codespec', 'changes', 'billing-rework'))
    ).toBe(true);
    expect(
      fs.readdirSync(path.join(storeRoot, 'codespec', 'changes', 'archive'))
    ).toEqual([]);
  ...
  it('composes with 3.1: the declared root surfaces the store own references', async () => {
    ...
    createCodeSpecRoot(upstreamRoot);
  ...
  it('refuses init in a pointer repo and creates nothing, then converts cleanly', async () => {
    ...
    fs.writeFileSync(path.join(pointerRepo, 'codespec', 'config.yaml'), 'schema: spec-driven\n');
  ...
  it('refuses init for malformed pointers and from pointer-repo subdirectories', async () => {
    ...
    expect(fs.existsSync(path.join(pointerRepo, 'codespec', 'specs'))).toBe(false);
    ...
    expect(fs.existsSync(path.join(subdir, 'codespec'))).toBe(false);
  });
  ...
  it('keeps real-root stdout byte-identical when a pointer is present, with one warning', async () => {
    ...
    for (const [label, config] of [
      ...
      fs.writeFileSync(path.join(realRepo, 'codespec', 'config.yaml'), config);
      const result = await runCLI(['list', '--json'], { cwd: realRepo, env });
      expect(result.exitCode).toBe(0);
      runs[label] = {
        stdout: result.stdout,
        warnings: (result.stderr.match(/该声明已忽略/g) ?? []).length,
      };
    }
