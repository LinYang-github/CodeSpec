 ...
 * and `codespec set change` is gone. Existing initiative metadata from the
...
describe('legacy repo-local change initiative metadata', () => {
  ...
  beforeEach(() => {
    tempDir = fs.realpathSync.native(
      fs.mkdtempSync(path.join(os.tmpdir(), 'codespec-change-initiative-link-'))
    );
    env = {
    ...
    fs.writeFileSync(path.join(tempDir, 'codespec', 'config.yaml'), 'schema: spec-driven\n');
  });
  ...
  function changeDir(id: string): string {
    return path.join(tempDir, 'codespec', 'changes', id);
  }
  ...
  it('keeps reading existing initiative metadata without modifying it', async () => {
    ...
    const metadataPath = path.join(dir, '.codespec.yaml');
  ...
  it('no longer provides codespec set change', async () => {
