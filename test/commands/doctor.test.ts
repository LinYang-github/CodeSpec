...
describe('codespec doctor (3.6)', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.realpathSync.native(fs.mkdtempSync(path.join(os.tmpdir(), 'codespec-doctor-')));
    env = {
  ...
  it('prints taxonomy errors in human mode instead of stack traces', async () => {
    ...
    expect(result.stderr).toContain('错误：当前目录及其父目录中未找到 CodeSpec 根目录');
