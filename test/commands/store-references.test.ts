...
describe('store references in instructions (3.1)', () => {
  ...
  beforeEach(async () => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codespec-store-refs-'));
    env = {
  ...
  it('omits the references field entirely when none are declared', async () => {
    fs.writeFileSync(path.join(appRepo, 'codespec', 'config.yaml'), 'schema: spec-driven\n');
  ...
  it('completes the PM-to-dev layered flow end to end', async () => {
    ...
    const changeDir = path.join(appRepo, 'codespec', 'changes', 'billing-rework');
    fs.writeFileSync(
