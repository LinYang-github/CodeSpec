...
async function confirmRemove(id: string, root: string, options: StoreRemoveOptions): Promise<void> {
  ...
  const { confirm } = await import('@inquirer/prompts');
  const confirmed = await confirm({
    message: `删除本地 Store 目录 ${formatPathForHuman(root)}？`,
    default: false,
  });
...
function printCleanupHuman(title: string, payload: StoreCleanupOutput): void {
  ...
  if (payload.files.deleted_path) {
    ...
    console.log(`保留文件位置：${formatPathForHuman(payload.files.left_on_disk)}`);
...
export function registerStoreCommand(program: Command): void {
  ...
  store.action(() => {
        ...
        status: [
          {
            ...
            message,
            fix: '运行 Store 子命令，或使用带 --store <id> 的生命周期命令。',
          },
        ],
      });
      process.exitCode = 1;
    ...
    console.error(
      '要在 Store 中创建或处理 Change，请在普通命令中传入 --store，例如：'
    );
