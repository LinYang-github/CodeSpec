...
2. **确定工作流 schema**
   ...
   - 用户要求查看可用工作流：先在当前目录运行 \`codespec context --json\` 解析权威根目录；若用户明确选择了已注册 Store，使用 \`codespec context --json --store "<store-id>"\`。然后在返回的 \`root.path\` 中运行 \`codespec schemas --json\` 供用户选择。该流程保留本地 \`store:\` 指针与全局 \`defaultStore\` 的解析结果；明确选择 Store 时，也在 \`codespec schemas --json\` 后附加 \`--store "<store-id>"\`。只有 context 返回 \`no_codespec_root\` 时，才改为在当前目录运行 \`codespec schemas --json\`；无效或不可用的 Store 不得使用该回退。

   其他情况下省略 \`--schema\`，以保留配置的默认值。

3. **创建 Change 目录**

   从下列形式中选择其一。若已选择注册的 Store，在该命令以及后续支持 \`--store\` 的 CodeSpec 命令后附加 \`--store "<store-id>"\`。
   ...
   使用用户显式指定的 schema：
   \`\`\`bash
   codespec new change "<name>" --schema "<schema-name>"
   \`\`\`

   CLI 会依据 \`.codespec.yaml\` 解析出的规划目录创建骨架。若同名 Change 已存在，询问用户是继续现有 Change 还是新建名称。
...
5. **创建所需集合中的全部工件**
   ...
   对每个状态为 \`ready\` 的工件：
   - 运行 \`codespec instructions <artifact-id> --change "<name>" --json\` 获取说明。
   ...
   每创建一个工件后重新运行 \`codespec status --change "<name>" --json\`。所需集合是 \`applyRequires\` 加上沿 \`status --json\` 的 \`requires\` 边可达的全部传递依赖；不要创建集合外工件。
   ...
   - 运行 \`codespec instructions <artifact-id> --change "<name>" --json\` 后，其 \`instruction\` 明确标记为可选（例如“仅当……时创建”）。不得自行判断为可跳过；应告知用户且不再反复考虑。
...
完成后汇总 Change 名称与位置、已创建工件及简述、跳过的条件工件及原因，并说明：“实现所需的全部规划产物已就绪。”最后提示：“产物已可评审。准备实现时，运行 \`/codespec:apply\`。”

**工件创建准则**

- 每类工件始终遵循 \`codespec instructions\` 返回的 \`instruction\`；即使名称熟悉，它仍是权威指导。
...
export function getCodespecProposeSkillTemplate(): SkillTemplate {
  return {
    name: 'codespec-propose',
    description: '一次性提出 Change 并生成完整规划产物。',
    instructions: withCodeSpecWorkflowGuidance(
...
export function getCodespecProposeCommandTemplate(): CommandTemplate {
  return {
    name: 'CODESPEC: Propose',
    description: '提出 Change 并一次性生成全部规划产物',
    category: 'Workflow',
    tags: ['workflow', 'artifacts', 'experimental'],
    content: renderProposeWorkflow('紧随 \'/codespec:propose\' 的参数应为 Change 名称（kebab-case）或用户希望构建内容的描述。'),
  };
}
