...
7. 成功后检查 Current Specification，汇总 Requirement merge、验证证据和已确认的警告。Current 保留 \`specs/<模块>/{spec.md,interface.yaml,api.yaml}\` 及根级 \`business.yaml\`、\`configuration.yaml\`。活动 Change 已移除，不能再用活动 status 查询它。
