...
export function getStageAdapterGuidance(stage: WorkflowStage): string {
  ...
  return `### ${stage} stage adapter\n\nThis is the ${action} stage. Resolve canonical context before acting: run \`openspec context --json\`, resolve one explicit \`CHG-YYYYMMDD-NNN\` (fail explicitly on missing or ambiguous context), then run \`openspec status --change "<CHG-ID>" --json\` and load the declared \`metadata.yaml\` and artifact paths. Substitute the resolved Change ID, status, baseline, Requirement IDs, Scenario IDs, Task IDs, test/evidence references, and canonical paths into the ${action} prompt. Refresh status after the action and record traceability in the canonical artifact.`;
}
