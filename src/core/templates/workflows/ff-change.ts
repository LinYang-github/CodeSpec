...
默认使用 \`code-spec\` schema。活动 Change 的工件和依赖顺序必须以 \`codespec status --change "<name>" --json\` 的实际输出为准；六个 canonical 工件为 \`metadata.yaml\`、\`analysis.yaml\`、\`design.md\`、\`spec.md\`、\`tasks.yaml\` 和 \`verification.yaml\`。PLAN 是计划审批阶段，不创建额外文件。
...
2. **创建 Change 目录**
   ...
   若用户明确指定了其他 schema，使用 \`codespec new change "<name>" --schema "<schema-name>"\`。若已选择注册的 Store，在所有支持的命令后附加 \`--store "<store-id>"\`。CLI 会在由配置解析的规划目录创建骨架；同名 Change 存在时，询问用户是否继续。
...
4. **创建所需集合中的全部工件**
   ...
   - 运行 \`codespec instructions <artifact-id> --change "<name>" --json\`。
   ...
   每创建一个工件后重跑 \`codespec status --change "<name>" --json\`。所需集合是 \`applyRequires\` 加上沿 \`status --json\` 的 \`requires\` 边可达的全部传递依赖；不要创建集合外工件。

   \`status\` 只反映文件存在性。\`applyRequires\` 工件显示 \`done\` **不代表**依赖存在；必须根据每个工件的 \`requires\` 边而非 \`status\` 建立所需集合，即使 \`done\` 工件仍会列出其依赖。

   仅可跳过已显示 \`status: "skipped"\` 的工件（其文件不得存在），或在运行 \`codespec instructions <artifact-id> --change "<name>" --json\` 后、\`instruction\` 明确标记为可选的工件。不得自行判断跳过；应告知用户且不再反复考虑。依赖是启用条件而非阻塞门槛：若工件仅因跳过可选依赖而 \`blocked\`，仍应创建它。直到所需集合的每项均为 \`done\`、\`skipped\` 或已明确跳过才停止。
...
汇总 Change 名称、位置、已创建工件、跳过原因，并说明：“实现所需的全部规划产物已就绪。准备实现时，运行 \`/codespec:apply\`。”

**护栏**

- 每类工件以 \`codespec instructions\` 的 \`instruction\` 为权威指导；schema 决定工件内容，\`template\` 决定文件结构。
...
export function getFfChangeSkillTemplate(): SkillTemplate {
  return {
    ...
    description: '快速创建开始实现所需的全部 CodeSpec 规划工件。',
...
export function getCodespecFfCommandTemplate(): CommandTemplate {
  return {
    name: 'CODESPEC: 快进',
    description: '一次性创建 Change 并生成实现所需的全部规划工件',
    category: 'Workflow',
    tags: ['workflow', 'artifacts', 'experimental'],
    content: renderFastForwardWorkflow('紧随 \'/codespec:ff\' 的参数应为 Change 名称（kebab-case）或用户希望构建内容的描述。'),
  };
}
