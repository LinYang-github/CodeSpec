...
import type { ChangeMetadata } from '../../codespec-workflow/types.js';
...
export function getStageAdapterGuidance(stage: WorkflowStage): string {
  ...
  return `### ${stage} 阶段适配器\n\n当前是 ${action} 阶段。执行前解析 canonical 上下文：运行 \`codespec context --json\`，解析一个明确的 \`CHG-YYYYMMDD-NNN\`（上下文缺失或有歧义时明确停止），然后运行 \`codespec status --change "<CHG-ID>" --json\` 并加载声明的 \`metadata.yaml\` 和产物路径。将解析出的 Change ID、status、mode、baseline hash、Requirement ID、准确的 Scenario ID、Task ID、测试/证据引用、必需验证命令和 canonical 路径注入 ${action} 提示。方法论路由：${methodology}。动作完成后刷新状态，并在 canonical 产物中记录追踪关系。`;
}
...
/** Canonical adapter guidance shared by generated CodeSpec workflow skills. */
export const CODESPEC_WORKFLOW_GUIDANCE = `
...
export function getCodeSpecWorkflowSkillTemplate() {
  return {
    ...
    description: '将 CodeSpec code-spec 工作路由到 canonical Change 工作流。',
    instructions: `${withCodeSpecWorkflowGuidance(DEVELOPMENT_ORCHESTRATION)}\n\n${STORE_SELECTION_GUIDANCE}`,
    license: 'MIT',
...
export function getCodespecWorkflowCommandTemplate(): CommandTemplate {
  return {
    name: 'CODESPEC: Workflow',
    description: '统一处理 CodeSpec Change 的开发生命周期',
    category: 'Workflow',
    tags: ['workflow', 'code-spec', 'canonical'],
    content: `${withCodeSpecWorkflowGuidance(`
统一处理一个 CodeSpec Change 的开发请求。
