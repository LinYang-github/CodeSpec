...
`;
  ...
  private generatePositionalSpec(positionalType?: string): string {
    switch (positionalType) {
      ...
      case 'schema-name':
        return "'*: :_codespec_complete_schemas'";
      case 'path':
        return "'*:path:_files'";
      case 'shell':
  ...
  private generateIndexedPositionalSpec(
    ...
    switch (positional.type) {
      case 'change-id':
        return `'${index}${separator}${name}:_codespec_complete_changes'`;
      case 'spec-id':
        return `'${index}${separator}${name}:_codespec_complete_specs'`;
      case 'change-or-spec-id':
        return `'${index}${separator}${name}:_codespec_complete_items'`;
      case 'schema-name':
        return `'${index}${separator}${name}:_codespec_complete_schemas'`;
      case 'path':
