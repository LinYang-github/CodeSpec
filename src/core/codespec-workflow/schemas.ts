...
const controlledVerificationKindSchema = z.enum([
  'unit', 'typecheck', 'build', 'lint', 'bdd', 'integration',
  'security', 'migration', 'performance', 'archive-regression',
]);
const verificationCommandDeclarationSchema = z.object({
  command: nonEmptyString.optional(),
  not_applicable_reason: nonEmptyString.optional(),
}).strict().superRefine((value, context) => {
  if (Boolean(value.command) === Boolean(value.not_applicable_reason)) {
    context.addIssue({ code: 'custom', message: 'exactly one of command or not_applicable_reason is required' });
  }
});
...
const workspaceConfigSchema = z
  .object({
    version: z.literal(1),
    schema: z.enum(['code-spec', 'spec-driven']),
    project: z
      .object({
        name: nonEmptyString,
      })
