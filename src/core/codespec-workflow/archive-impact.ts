...
export async function validateChangeArchiveImpact(workspace: WorkspaceContext, artifacts: ChangeArtifacts, state = artifacts.metadata.change.status): Promise<{
  impact: ArchiveImpact; deltas: RequirementDelta[]; current: Map<string, string>; issues: string[]; richDelta?: CurrentSpecDeltaDocument;
}> {
  if (!artifacts.metadata.artifacts.proposal) {
    ...
    return {
      impact, deltas: [], richDelta: check.delta, current: new Map([[check.delta.module, check.current ?? '']]),
