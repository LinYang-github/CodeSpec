...
export interface TraceRow {
  /** Absent only on historical proposal-bearing evidence. */
...
export function validateChangeTraceability(artifacts: ChangeArtifacts, requireEvidence = false): TraceabilityResult {
  if (!artifacts.metadata.artifacts.proposal) return validateAcceptanceTraceability(artifacts, requireEvidence);
