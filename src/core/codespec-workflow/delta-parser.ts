...
function parseRequirement(action: RequirementDelta['action'], lines: string[]): RequirementDelta {
  ...
  const flushScenario = () => {
    ...
    const scenarioKey = `${currentScenarioLabel}/${parsed.id}`;
    if (scenarioIds.has(scenarioKey)) throw new Error(`Duplicate Scenario ID ${parsed.id}`);
    scenarioIds.add(scenarioKey); scenarios.push(parsed);
...
export function parseDeltaSpec(content: string): ParsedDeltaSpec {
  const sections: Array<{ action: RequirementDelta['action']; lines: string[] }> = [];
  for (const section of documentSections(content)) {
    if (INLINE_DESIGN_SECTIONS.has(section.title)) continue;
    const action = `## ${section.title}`.match(ACTION);
    if (!action) throw new Error(`Unknown delta action section: ## ${section.title}`);
    sections.push({ action: action[1] as RequirementDelta['action'], lines: section.body.split('\n') });
  }
