...
// The Purpose `codespec archive` writes into a main spec it creates when the
...
export const VALIDATION_MESSAGES = {
  ...
  CHANGE_SKIP_SPECS_CONFLICT:
    '已在 .codespec.yaml 中设置 skip_specs，但 specs/ 下仍存在 Spec 文件。请移除 skip_specs 或删除 delta Spec 文件',
  CHANGE_SKIP_SPECS_ACCEPTED:
    '已在 .codespec.yaml 中设置 skip_specs：Change 声明不包含 Spec 层行为变更，接受零个 delta',
  CHANGE_SKIP_SPECS_INVALID_METADATA:
    '已设置 skip_specs，但 .codespec.yaml 不是有效的 Change 元数据，因此不会采用该标记。请修复元数据',
  ...
  PURPOSE_IS_PLACEHOLDER:
    'Purpose 部分仍是占位内容，而不是实际编写的 Purpose（可能是 `codespec archive` 为新 capability 写入的句子，或遗留的 `TBD`/`TODO` 标记）。请直接编辑主 Spec，改为说明 capability 的用途：delta 中的 `## Purpose` 只会在创建 capability 时读取，不能替代主 Spec 中的 Purpose。',
